...
use neo_primitives::{Nep6Wallet, Wallet as CoreWallet};
use neo_system::Node as NeoSystem;
use neo_network::p2p::channels_config::ChannelsConfig;
use neo_config::ProtocolSettings;
use neo_native_contracts::ContractManagement;
use neo_state_service::{state_store::StateServiceSettings, verification::StateServiceVerification};
use neo_wallets::WalletProvider;
// RPC server types are gated behind the `server` feature of `neo-rpc`.
// The full RPC stack is being re-built outside the kill-neo-core refactor;
// for now we just keep the imports commented out so the binary compiles.
#[allow(unused_imports)]
use neo_rpc as _rpc_placeholder;
...
pub(crate) async fn start_rpc_server_if_enabled(
    ...
    let tls_config = Ok::<_, anyhow::Error>(Default::default());
    {
        Ok(config) => config,
        Err(err) => {
