...
pub(in crate::node) async fn run_startup_imports(
    mut ctx: StartupImportContext<'_>,
...
async fn run_chain_acc_import(
    ctx: &mut StartupImportContext<'_>,
) -> anyhow::Result<StartupImportOutcome> {
...
fn finish_stop_height_import(ctx: &mut StartupImportContext<'_>) -> anyhow::Result<()> {
    flush_state_service_for_shutdown(ctx.node)?;
    restore_durable_store_mode(ctx.node.storage().as_ref(), ctx.durable_service_stores)
