...
impl RunningNode {
    ...
    #[cfg(test)]
    pub(super) fn node(&self) -> &Arc<ComposedNode> {
        &self.node
    }

    #[cfg(test)]
    pub(super) fn network(&self) -> &neo_network::NetworkHandle {
        &self.network
    }

    #[cfg(test)]
    pub(super) fn services(&self) -> &Arc<NodeServiceHandles<RuntimeStore>> {
        &self.services
    }

    #[cfg(test)]
    pub(super) fn durable_service_stores(&self) -> &[Arc<RuntimeStore>] {
        &self.durable_service_stores
    }

    #[cfg(test)]
    pub(super) async fn abort_for_test(mut self) {
