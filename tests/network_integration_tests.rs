...
fn test_network_config_validation() {
    ...
    assert!(
        config.p2p_config.max_peers > 0,
        "Max peers should be positive"
    );
...
fn test_protocol_version_compatibility() {
    ...
    assert!(
        v3_6_0.is_compatible(&v3_5_0),
        "v3.6.0 should be compatible with v3.5.0"
    );
    assert!(
        v3_7_0.is_compatible(&v3_6_0),
        "v3.7.0 should be compatible with v3.6.0"
    );
    assert!(
        v3_7_0.is_compatible(&v3_5_0),
        "v3.7.0 should be compatible with v3.5.0"
    );

    // Same version should be compatible
    assert!(
        v3_6_0.is_compatible(&v3_6_0),
        "Same version should be compatible"
    );

    // Different major versions should not be compatible
    assert!(
        !v4_0_0.is_compatible(&v3_7_0),
        "v4.0.0 should not be compatible with v3.7.0"
    );
    assert!(
        !v3_6_0.is_compatible(&v4_0_0),
        "v3.6.0 should not be compatible with v4.0.0"
    );

    // Older minor version should not be compatible with newer
    assert!(
        !v3_5_0.is_compatible(&v3_6_0),
        "v3.5.0 should not be compatible with v3.6.0"
    );
...
fn test_network_messages() {
    ...
    let pong_message = Message::new(magic, MessageType::Pong, Payload::Pong(pong_payload));
...
async fn test_network_server_integration() {
    ...
    assert!(
        network_server.is_ok(),
        "Should create network server successfully"
    );
...
fn test_message_validation() {
    ...
    let valid_message = Message::new(magic, MessageType::Version, Payload::Version(valid_version));

    assert!(
        valid_message.validate().is_ok(),
    ...
    assert!(
        invalid_magic_message.validate().is_err(),
    ...
    let addr_message = Message::new(magic, MessageType::Addr, Payload::Addr(addr_payload));

    assert!(
        addr_message.validate().is_ok(),
        "Addr message should be valid"
    );
...
async fn test_network_performance() {
    ...
    for i in 0..100 {
        ...
        let message = Message::new(magic, MessageType::Ping, Payload::Ping(ping_payload));
    ...
    assert!(
        elapsed.as_secs() < 2,
        "Network performance test should be fast"
    );
}
...
async fn test_comprehensive_network_integration() {
    ...
    assert_eq!(
        network_server.state(),
        neo_network::NetworkServerState::Stopped
    );
    ...
    assert!(
        version_message.validate().is_ok(),
        "Version message should be valid"
    );
