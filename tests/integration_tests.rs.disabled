...
use neo_vm::{ExecutionEngine, OpCode, Script, TriggerType, VMState};
use std::sync::Arc;
...
async fn test_vm_smart_contract_integration() {
    ...
    let script_bytes = vec![
        ...
        0xf6,
        0x67,
        0xce,              // SYSCALL System.Storage.Put
...
async fn test_blockchain_ledger_integration() {
    ...
    assert!(
        add_result.is_ok(),
        "Should be able to add block to blockchain"
    );
...
async fn test_native_contract_integration() {
    ...
    let balance_result = neo_contract.invoke(&mut engine, "balanceOf", &[account.clone()]);
    ...
    let gas_balance_result = gas_contract.invoke(&mut engine, "balanceOf", &[account.clone()]);
...
async fn test_consensus_integration() {
    ...
    assert!(
        consensus_service.is_ok(),
        "Consensus service should be created successfully"
    );
    ...
    assert_eq!(
        service.state(),
        neo_consensus::ConsensusServiceState::Stopped
    );
...
async fn test_network_protocol_integration() {
    ...
    assert!(
        version1.is_compatible(&version2),
...
async fn test_storage_persistence_integration() {
    ...
    // Test key existence
    assert!(
        storage.contains_key(key).unwrap(),
        "Storage should contain the key"
    );
    ...
    assert!(
        !storage.contains_key(key).unwrap(),
        "Storage should not contain deleted key"
    );
...
async fn test_cryptography_integration() {
    ...
    assert!(
        verify_result.is_ok(),
        "Signature verification should not crash"
    );
...
async fn test_interop_services_integration() {
    ...
    assert_eq!(
        time_result.unwrap().len(),
        8,
        "Time should be 8 bytes (u64)"
    );
    ...
    assert_eq!(
        sha256_result.unwrap().len(),
        32,
        "SHA256 should produce 32-byte hash"
    );
...
async fn test_end_to_end_integration() {
    ...
    assert_eq!(
        vm_engine.result_stack().len(),
        1,
        "Contract should return 1 value"
    );

    let contract_result = vm_engine.result_stack().peek(0).unwrap();
    assert_eq!(
        contract_result.as_int().unwrap().to_string(),
        "42",
...
async fn test_module_interoperability_matrix() {
    ...
    assert!(
        storage.get(b"test").unwrap().is_some(),
        "Ledger-Storage integration"
    );
    ...
    assert!(
        consensus_config.view_timeout > 0,
