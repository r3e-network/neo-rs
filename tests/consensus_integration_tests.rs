...
use neo_consensus::{
    ...
    validators::{ValidatorManager, ValidatorSet, Validator, ValidatorPerformance},
    proposal::{ProposalManager, ProposalConfig, BlockProposal, TransactionSelectionStrategy},
    context::{ConsensusContext, TimerType},
    recovery::{RecoveryManager, RecoveryConfig, RecoveryReason, RecoveryStatus},
    messages::{ConsensusMessage, ConsensusMessageData, PrepareRequest, PrepareResponse, Commit, RecoveryRequest},
};
use neo_core::{UInt160, UInt256, Transaction, Signer, WitnessScope, Witness};
use neo_ledger::{Blockchain, Block, BlockHeader, VerifyResult};
...
fn test_consensus_config_validation() {
    ...
    assert!(config.max_block_size > 0, "Max block size should be positive");
    assert!(config.max_transactions_per_block > 0, "Max transactions should be positive");
    
    // Test invalid configurations
    config.view_timeout = 0;
    assert!(!config.is_valid(), "Config with zero view timeout should be invalid");
    
    config = ConsensusConfig::default();
    config.block_time = 0;
    assert!(!config.is_valid(), "Config with zero block time should be invalid");
    
    config = ConsensusConfig::default();
    config.min_validators = 0;
    assert!(!config.is_valid(), "Config with zero min validators should be invalid");
    
    config = ConsensusConfig::default();
    config.max_validators = config.min_validators - 1;
    assert!(!config.is_valid(), "Config with max_validators < min_validators should be invalid");
...
async fn test_validator_manager() {
    ...
    // Test initial state
    assert_eq!(validator_manager.get_all_validators().len(), 0, "Should start with no validators");
    assert_eq!(validator_manager.get_active_validators().len(), 0, "Should start with no active validators");
    ...
    assert!(register_result1.is_ok(), "Should register validator 1 successfully");
    
    let register_result2 = validator_manager.register_validator(
        validator2_hash,
        vec![2u8; 33],
        20_000_000,
        0,
    );
    assert!(register_result2.is_ok(), "Should register validator 2 successfully");
    
    let register_result3 = validator_manager.register_validator(
        validator3_hash,
        vec![3u8; 33],
        15_000_000,
        0,
    );
    assert!(register_result3.is_ok(), "Should register validator 3 successfully");
    
    // Test validator retrieval
    assert_eq!(validator_manager.get_all_validators().len(), 3, "Should have 3 validators");
    
    let validator1 = validator_manager.get_validator(&validator1_hash);
    assert!(validator1.is_some(), "Should find validator 1");
    assert_eq!(validator1.unwrap().stake, 10_000_000, "Validator 1 stake should match");
    
    // Test validator set creation
    let validator_set = validator_manager.create_validator_set(100);
    assert!(validator_set.is_ok(), "Should create validator set successfully");
    
    let set = validator_set.unwrap();
    assert!(set.validators().len() <= config.max_validators as usize, "Validator set should not exceed max");
...
async fn test_consensus_context() {
    ...
    assert_eq!(context.primary_index(), 0, "Should start with primary index 0");
...
async fn test_block_proposal() {
    ...
    assert!(proposal_result.is_ok(), "Should create proposal successfully");
    
    let proposal = proposal_result.unwrap();
    assert!(proposal.transactions.len() <= transactions.len(), "Proposal should not exceed input transactions");
    assert!(proposal.total_network_fee >= 0, "Total network fee should be non-negative");
    assert!(proposal.total_system_fee >= 0, "Total system fee should be non-negative");
...
async fn test_consensus_messages() {
    ...
    assert!(prepare_request.validate().is_ok(), "PrepareRequest should be valid");
    
    // Test PrepareResponse
    let prepare_response = PrepareResponse::new(block_hash);
    assert_eq!(prepare_response.block_hash, block_hash);
    assert!(prepare_response.validate().is_ok(), "PrepareResponse should be valid");
...
async fn test_consensus_recovery() {
    ...
    assert!(recovery_result.is_ok(), "Should initiate recovery successfully");
    ...
    assert!(recovery_request.validate().is_ok(), "Recovery request should be valid");
...
async fn test_consensus_round_simulation() {
    ...
    for i in 0..4 {
        ...
        assert!(service.is_ok(), "Should create consensus service for node {}", i);
        nodes.push(service.unwrap());
    }
    
    // Verify all nodes are created and in correct initial state
    for (i, node) in nodes.iter().enumerate() {
        assert_eq!(node.state(), ConsensusServiceState::Stopped, "Node {} should start stopped", i);
    }
...
async fn test_consensus_blockchain_integration() {
    ...
    let register_result = validator_manager.register_validator(
        validator_hash,
        vec![1u8; 33],
        1_000_000,
        0,
    ...
    assert_eq!(validator_set.validators().len(), 1, "Should have 1 validator");
    ...
    assert_eq!(validation_result, VerifyResult::Succeed, "Block should be valid");
    ...
    println!("   🔸 Blockchain height: {}", blockchain.current_block_height());
    println!("   🔸 Block count: {}", blockchain.block_count());
    println!("   🔸 Validator count: {}", validator_set.validators().len());
}
...
async fn test_consensus_performance() {
    ...
    for block_height in 0..10 {
        ...
        assert!(validator_set.is_ok(), "Should create validator set for block {}", block_height);
    ...
    assert!(elapsed.as_secs() < 1, "Consensus performance test should be fast");
}
...
async fn test_consensus_error_handling() {
    ...
    assert!(invalid_result.is_err(), "Should reject validator with low stake");
    
    // Test duplicate validator registration
    let validator_hash = UInt160::from_bytes(&[1; 20]).unwrap();
    let first_result = validator_manager.register_validator(
        validator_hash,
        vec![1u8; 33],
        10_000_000,
        0,
    ...
    assert!(duplicate_result.is_err(), "Should reject duplicate validator");
    ...
    assert!(invalid_prepare.validate().is_err(), "Should reject invalid prepare request");
...
async fn test_consensus_state_persistence() {
    ...
    assert_eq!(context2.view_number(), saved_view, "View should be restored");
    assert_eq!(context2.block_index(), saved_block, "Block index should be restored");
    
    println!("✅ Consensus state persistence test passed");
    println!("   🔸 Saved state: view={}, block={}", saved_view, saved_block);
    println!("   🔸 Restored state: view={}, block={}", context2.view_number(), context2.block_index());
} 