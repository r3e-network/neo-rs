...
async fn test_ecdsa_key_operations() {
    ...
    assert!(
        private_key.is_ok(),
        "Should generate secp256r1 private key successfully"
    );
    ...
    assert!(
        restored_private_key.is_ok(),
        "Should restore private key from bytes"
    );

    let restored_public_key = PublicKey::from_compressed_bytes(&public_key_compressed);
    assert!(
        restored_public_key.is_ok(),
        "Should restore public key from compressed bytes"
    );
    ...
    assert!(
        secp256k1_private.is_ok(),
        "Should generate secp256k1 private key successfully"
    );
    ...
    for (i, message) in test_messages.iter().enumerate() {
        ...
        if !wrong_message.is_empty() {
            ...
            let wrong_verification =
                ECDsa::verify_secp256r1(&wrong_message, &signature, &public_key);
            assert!(wrong_verification.is_ok(), "Verification should not fail");
            assert!(
                !wrong_verification.unwrap(),
        ...
        let wrong_sig_verification =
            ECDsa::verify_secp256r1(message, &wrong_signature, &public_key);
...
async fn test_hash_function_operations() {
    ...
    for (i, data) in test_data.iter().enumerate() {
        println!(
            "  Testing hash functions with data {}: {} bytes",
            i + 1,
            data.len()
        );
        ...
        assert_eq!(
            hash256_hash.len(),
            32,
            "Hash256 should produce 32-byte hash"
        );

        // Verify Hash256 is double SHA256
        let manual_double_sha256 = Sha256::hash(&sha256_hash).unwrap();
        assert_eq!(
            hash256_hash, manual_double_sha256,
            "Hash256 should equal double SHA256"
        );
...
async fn test_neo_address_operations() {
    ...
    for (i, script_hash) in test_script_hashes.iter().enumerate() {
        ...
        assert!(
            ...
            "Neo N3 address should start with 'N'"
        );
        ...
        assert_eq!(
            decoded[0], 0x35,
            "Address version should be 0x35 for Neo N3"
        );

        // Convert back from address
        let restored_script_hash = UInt160::from_address(&address);
        assert!(
            restored_script_hash.is_ok(),
            "Should restore script hash from address"
        );
        assert_eq!(
            restored_script_hash.unwrap(),
            *script_hash,
            "Restored script hash should match original"
        );
    ...
    for (i, script) in test_scripts.iter().enumerate() {
        println!(
            "  Testing script hash generation {}: {} bytes",
            i + 1,
            script.len()
        );
        ...
        assert_eq!(
            script_hash, manual_script_hash,
            "Script hash should equal Hash160 of script"
        );
        ...
        assert_eq!(
            restored_hash, script_hash,
            "Round-trip conversion should preserve script hash"
        );
    }
...
async fn test_cryptographic_edge_cases() {
    ...
    for (i, invalid_sig) in invalid_signatures.iter().enumerate() {
        println!(
            "  Testing invalid signature {}: {} bytes",
            i + 1,
            invalid_sig.len()
        );
        ...
        if invalid_sig.len() == 64 {
            let verify_result =
                ECDsa::verify_secp256r1(test_message, invalid_sig, &valid_public_key);
...
async fn test_multi_signature_operations() {
    ...
    for (i, (private_key, public_key)) in key_pairs.iter().enumerate() {
        ...
        assert!(
            verification,
            "Individual signature {} should be valid",
            i + 1
        );
    ...
    for i in 0..key_count {
        for j in 0..key_count {
            if i != j {
                let result =
                    ECDsa::verify_secp256r1(message, &signatures[i], &key_pairs[j].1).unwrap();
                assert!(
                    !result,
    ...
    for subset_size in 1..=key_count {
        println!(
            "  Testing {}-of-{} signature verification",
            subset_size, key_count
        );
        ...
        assert_eq!(
            valid_count, subset_size,
            "All signatures in subset should be valid"
        );

        // Check if threshold is met
        let threshold_met = valid_count >= threshold;
        println!(
            "    Threshold met: {} (need {}, have {})",
            threshold_met, threshold, valid_count
        );
    }
...
async fn test_cryptographic_performance() {
    ...
    println!(
        "  Key generation: {} iterations in {:?} (avg: {:?})",
    ...
    println!(
        "  Verification: {} iterations in {:?} (avg: {:?})",
        iterations, verification_time, avg_verification_time
    );
    assert_eq!(
        verification_count, iterations,
    ...
    assert!(
        avg_key_gen_time.as_millis() < 100,
        "Key generation should be reasonably fast"
    );
    assert!(
        avg_signing_time.as_millis() < 50,
        "Signing should be reasonably fast"
    );
    assert!(
        avg_verification_time.as_millis() < 50,
        "Verification should be reasonably fast"
    );
    assert!(
        avg_hash_time.as_micros() < 1000,
...
async fn test_neo_blockchain_compatibility() {
    ...
    let known_test_cases = vec![
        ...
        (
            ...
            "NKhNscREBRvyQ7X2eCW8M4Nqz8jyqC7YhE",
        ),
        (
            "0102030405060708090a0b0c0d0e0f1011121314",
            "NKrPUhGTVKAeEkhpMpuwKBXgWHUhPGJRc9",
        ),
    ];
    ...
    for (script_hash_hex, expected_address) in known_test_cases {
        ...
        assert!(
            generated_address.starts_with('N'),
            "Address should start with 'N'"
        );
        assert!(
            generated_address.len() > 30,
        ...
        assert_eq!(
            restored_script_hash, script_hash,
    ...
    assert_eq!(
        compressed_pub_key.len(),
        33,
        "Neo public key should be 33 bytes (compressed)"
    );
    assert!(
        compressed_pub_key[0] == 0x02 || compressed_pub_key[0] == 0x03,
    ...
    assert!(
        contract_address.starts_with('N'),
        "Contract address should start with 'N'"
    );
...
async fn test_concurrent_cryptographic_operations() {
    ...
    let key_gen_tasks = (0..concurrent_operations)
        .map(|i| {
    ...
    let signing_tasks = (0..concurrent_operations)
        .map(|i| {
    ...
    let hashing_tasks = (0..concurrent_operations)
        .map(|i| {
            tokio::spawn(async move {
                ...
                (
                    i,
                    sha256_result.is_ok() && hash256_result.is_ok() && hash160_result.is_ok(),
                )
            })
        })
        .collect::<Vec<_>>();
