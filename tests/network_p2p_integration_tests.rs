...
use neo_network::{
    NetworkConfig, P2PConfig, RpcConfig, NetworkMessage, MessageType,
    NetworkServer, P2PNode, PeerManager, NetworkStats, ProtocolVersion,
    messages::{VersionMessage, VerAckMessage, PingMessage, PongMessage, GetBlocksMessage},
};
...
async fn test_network_configuration() {
    ...
    assert_eq!(mainnet_config.magic, 0x334f454e, "MainNet magic should be correct");
    assert!(!mainnet_config.seed_nodes.is_empty(), "Should have seed nodes");
    assert!(mainnet_config.p2p_config.max_peers > 0, "Should allow peers");
    assert!(mainnet_config.rpc_config.is_some(), "Should have RPC config");
    ...
    assert_eq!(testnet_config.magic, 0x3554454e, "TestNet magic should be correct");
    assert_ne!(mainnet_config.magic, testnet_config.magic, "MainNet and TestNet should have different magic");
    assert_ne!(mainnet_config.listen_address.port(), testnet_config.listen_address.port(), "Should use different ports");
...
async fn test_protocol_version_compatibility() {
    ...
    assert!(version_3_6_0.is_compatible(&version_3_5_0), "Newer minor version should be compatible with older");
    assert!(version_3_6_1.is_compatible(&version_3_6_0), "Newer patch version should be compatible");
    assert!(!version_4_0_0.is_compatible(&version_3_6_0), "Different major versions should not be compatible");
    
    // Test version string representation
    assert_eq!(version_3_6_0.to_string(), "3.6.0", "Version string should be formatted correctly");
    
    // Test version parsing
    let parsed_version = ProtocolVersion::from_string("3.6.0");
    assert!(parsed_version.is_ok(), "Should parse valid version string");
    assert_eq!(parsed_version.unwrap(), version_3_6_0, "Parsed version should match original");
...
async fn test_network_message_serialization() {
    ...
    assert!(serialized.is_ok(), "Version message should serialize successfully");
    ...
    assert!(deserialized.is_ok(), "Version message should deserialize successfully");
    let deserialized_msg = deserialized.unwrap();
    
    // Verify data integrity
    assert_eq!(deserialized_msg.version, version_msg.version, "Version should match");
    assert_eq!(deserialized_msg.user_agent, version_msg.user_agent, "User agent should match");
    assert_eq!(deserialized_msg.start_height, version_msg.start_height, "Start height should match");
    ...
    assert_eq!(ping_deserialized.height, ping_msg.height, "Ping height should match");
    ...
    assert_eq!(pong_deserialized.timestamp, pong_msg.timestamp, "Pong timestamp should match");
...
async fn test_peer_management() {
    ...
    assert!(!known_peers_after_removal.contains(&peer2), "Should not contain removed peer");
    assert_eq!(known_peers_after_removal.len(), 2, "Should have 2 peers after removal");
...
async fn test_p2p_node_functionality() {
    ...
    assert!(p2p_node_result.is_ok(), "P2P node should be created successfully");
    ...
    assert!(node_info.listen_address.port() > 0, "Should have valid listen address");
    assert_eq!(node_info.protocol_version.major, 3, "Should use protocol version 3");
    ...
    assert!(broadcast_result.is_ok(), "Should prepare broadcast successfully");
...
async fn test_network_message_handling() {
    ...
    assert!(network_server_result.is_ok(), "Network server should be created successfully");
    ...
    let handle_result = network_server.handle_message(
        "127.0.0.1:10333".parse().unwrap(),
        version_network_msg
    ).await;
    
    assert!(handle_result.is_ok(), "Should handle version message successfully");
    ...
    let get_blocks_result = network_server.handle_message(
        "127.0.0.1:10333".parse().unwrap(),
        get_blocks_network_msg
    ).await;
    
    assert!(get_blocks_result.is_ok(), "Should handle GetBlocks message successfully");
    
    // Test invalid message handling
    let invalid_msg = NetworkMessage::new(MessageType::Version, vec![0xFF, 0xFF]); // Invalid payload
    
    let invalid_result = network_server.handle_message(
        "127.0.0.1:10333".parse().unwrap(),
        invalid_msg
    ).await;
...
async fn test_network_statistics() {
    ...
    // Test initial state
    assert_eq!(network_stats.total_connections(), 0, "Should start with 0 connections");
    assert_eq!(network_stats.active_connections(), 0, "Should start with 0 active connections");
    assert_eq!(network_stats.messages_sent(), 0, "Should start with 0 messages sent");
    assert_eq!(network_stats.messages_received(), 0, "Should start with 0 messages received");
    ...
    assert_eq!(network_stats.total_connections(), 3, "Should have 3 total connections");
    assert_eq!(network_stats.active_connections(), 2, "Should have 2 active connections");
    assert_eq!(network_stats.failed_connections(), 1, "Should have 1 failed connection");
    ...
    assert_eq!(network_stats.messages_sent(), 15, "Should have sent 15 messages");
    assert_eq!(network_stats.messages_received(), 15, "Should have received 15 messages");
    ...
    assert_eq!(pong_stats.received, 10, "Should have received 10 pong messages");
    
    // Test bandwidth statistics
    network_stats.record_bytes_sent(1024);
    network_stats.record_bytes_received(2048);
    
    assert_eq!(network_stats.total_bytes_sent(), 1024, "Should have sent 1024 bytes");
    assert_eq!(network_stats.total_bytes_received(), 2048, "Should have received 2048 bytes");
...
async fn test_concurrent_network_operations() {
    ...
    let node_creation_tasks = (0..5).map(|i| {
    ...
    for result in message_results.iter() {
        ...
        assert!(*success, "Message {} should be processed successfully", msg_id);
    }
...
async fn test_network_error_handling() {
    ...
    assert!(connection_result.is_err(), "Connection to invalid peer should fail");
    ...
    assert!(deserialize_result.is_err(), "Invalid message data should fail to deserialize");
    ...
    assert_eq!(network_stats.failed_connections(), 10, "Should record all failures");
    assert_eq!(network_stats.active_connections(), 5, "Should record recovery connections");
...
async fn test_network_protocol_handshake() {
    ...
    assert_eq!(received_version.nonce, node1_version.nonce, "Version nonce should match");
    ...
    assert_eq!(received_node2_version.nonce, node2_version.nonce, "Node2 version nonce should match");
    ...
    assert_eq!(received_version.version, received_node2_version.version, "Versions should be compatible");
    assert_ne!(received_version.nonce, received_node2_version.nonce, "Nonces should be different");
    ...
    assert_eq!(received_pong.timestamp, ping.timestamp, "Pong timestamp should match ping");
...
async fn test_network_performance_under_load() {
    ...
    let avg_serialization_time = serialization_times.iter().sum::<Duration>() / message_count as u32;
    println!("  Average message serialization time: {:?}", avg_serialization_time);
    ...
    for i in 0..1000 {
        let peer: SocketAddr = format!("192.168.{}.{}:10333", i / 255, i % 255).parse().unwrap();
        peer_manager.add_known_peer(peer);
    }
    ...
    assert!(avg_serialization_time.as_micros() < 1000, "Message serialization should be fast");
    assert!(stats_time.as_millis() < 100, "Stats recording should be fast");
