...
use neo_core::unwrap_migration::{UnwrapMigrator, migration_patterns};
use neo_core::witness_safe::{SafeWitnessOperations, SafeWitnessBuilder};
...
fn test_safe_error_handling_integration() {
    ...
    assert!(result.unwrap_err().to_string().contains("Expected value was missing"));
    ...
    println!("   ✅ Migration tracking: {} migrated, {} in tests", 
    ...
    assert!(validation.unwrap_err().to_string().contains("Invocation script cannot be empty"));
    ...
    println!("   ✅ Batch processing: {} witnesses processed", processed.len());
...
fn test_performance_impact() {
    ...
    println!("Safe operations: {:?} for {} iterations", safe_duration, ITERATIONS);
    println!("Unsafe operations: {:?} for {} iterations", unsafe_duration, ITERATIONS);
    
    let overhead = if safe_duration > unsafe_duration {
        ((safe_duration.as_nanos() as f64 - unsafe_duration.as_nanos() as f64) 
         / unsafe_duration.as_nanos() as f64) * 100.0
    } else {
        0.0
    };
    ...
    assert!(overhead < 10.0, "Performance overhead too high: {:.2}%", overhead);
