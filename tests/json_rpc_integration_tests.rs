...
use neo_rpc_server::{RpcServer, RpcServerConfig, JsonRpcRequest, JsonRpcResponse, JsonRpcError};
...
async fn test_rpc_server_initialization() {
    ...
    assert!(http_server_result.is_ok(), "HTTP RPC server should be created successfully");
    
    let http_server = http_server_result.unwrap();
    assert_eq!(http_server.config().max_connections, 100, "Max connections should match config");
    ...
    assert!(ws_server_result.is_ok(), "WebSocket RPC server should be created successfully");
    
    let ws_server = ws_server_result.unwrap();
    assert!(ws_server.config().ws_address.is_some(), "WebSocket address should be configured");
    assert!(ws_server.config().enable_auth, "Authentication should be enabled");
    assert!(ws_server.config().auth_token.is_some(), "Auth token should be configured");
...
async fn test_json_rpc_serialization() {
    ...
    for (i, request) in test_requests.iter().enumerate() {
        ...
        assert!(!serialized_str.is_empty(), "Serialized request should not be empty");
        
        // Deserialize request
        let deserialized: Result<JsonRpcRequest, _> = serde_json::from_str(&serialized_str);
        assert!(deserialized.is_ok(), "Request should deserialize successfully");
        let deserialized_request = deserialized.unwrap();
        
        // Verify data integrity
        assert_eq!(deserialized_request.jsonrpc, request.jsonrpc, "JSON-RPC version should match");
        assert_eq!(deserialized_request.method, request.method, "Method should match");
        assert_eq!(deserialized_request.params, request.params, "Params should match");
    ...
    for (i, response) in test_responses.iter().enumerate() {
        ...
        assert!(deserialized.is_ok(), "Response should deserialize successfully");
        let deserialized_response = deserialized.unwrap();
        
        // Verify data integrity
        assert_eq!(deserialized_response.jsonrpc, response.jsonrpc, "JSON-RPC version should match");
        assert_eq!(deserialized_response.id, response.id, "ID should match");
        
        if response.result.is_some() {
            assert_eq!(deserialized_response.result, response.result, "Result should match");
        }
        
        if response.error.is_some() {
            assert!(deserialized_response.error.is_some(), "Error should be present");
            ...
            assert_eq!(deser_error.message, orig_error.message, "Error message should match");
        }
    }
...
async fn test_blockchain_data_json_serialization() {
    ...
    assert!(script_hash_json.is_string(), "UInt160 should serialize as string");
    
    let script_hash_str = script_hash_json.as_str().unwrap();
    assert!(script_hash_str.starts_with("0x"), "UInt160 should start with 0x");
    assert_eq!(script_hash_str.len(), 42, "UInt160 string should be 42 characters (0x + 40 hex)");
    ...
    assert!(block_hash_json.is_string(), "UInt256 should serialize as string");
    
    let block_hash_str = block_hash_json.as_str().unwrap();
    assert!(block_hash_str.starts_with("0x"), "UInt256 should start with 0x");
    assert_eq!(block_hash_str.len(), 66, "UInt256 string should be 66 characters (0x + 64 hex)");
    ...
    assert!(transaction_json.is_ok(), "Transaction should serialize to JSON");
    let tx_json = transaction_json.unwrap();
    
    assert!(tx_json.is_object(), "Transaction should serialize as object");
    let tx_obj = tx_json.as_object().unwrap();
    
    assert!(tx_obj.contains_key("nonce"), "Transaction should have nonce field");
    assert!(tx_obj.contains_key("networkfee"), "Transaction should have networkfee field");
    assert!(tx_obj.contains_key("systemfee"), "Transaction should have systemfee field");
    assert!(tx_obj.contains_key("validuntilblock"), "Transaction should have validuntilblock field");
    ...
    assert!(block_json_value.is_object(), "Block should serialize as object");
    let block_obj = block_json_value.as_object().unwrap();
    
    assert!(block_obj.contains_key("header"), "Block should have header field");
    assert!(block_obj.contains_key("transactions"), "Block should have transactions field");
...
async fn test_rpc_client_functionality() {
    ...
    assert_eq!(client.endpoint(), "http://127.0.0.1:10332", "Endpoint should match config");
    assert_eq!(client.config().timeout, 30, "Timeout should match config");
    assert_eq!(client.config().max_retries, 3, "Max retries should match config");
    ...
    assert_eq!(getblockcount_request.method, "getblockcount", "Method should match");
    assert!(getblockcount_request.params.is_empty(), "Params should be empty");
    ...
    for (method, params) in invalid_requests {
        ...
        assert_eq!(request.method, method, "Method should be preserved even if invalid");
    }
...
async fn test_rpc_method_handlers() {
    ...
    assert!(getblockcount_response.is_ok(), "getblockcount should be handled successfully");
    
    let response = getblockcount_response.unwrap();
    assert!(response.result.is_some(), "getblockcount should return a result");
    assert!(response.error.is_none(), "getblockcount should not return an error");
    ...
    assert!(getbestblockhash_response.is_ok(), "getbestblockhash should be handled successfully");
    
    let response = getbestblockhash_response.unwrap();
    assert!(response.result.is_some(), "getbestblockhash should return a result");
    
    // Test getblock handler with hash parameter
    let getblock_request = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        method: "getblock".to_string(),
        params: vec![json!("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef")],
        id: Some(json!(3)),
    };
    
    let getblock_response = rpc_server.handle_request(getblock_request).await;
    assert!(getblock_response.is_ok(), "getblock should be handled successfully");
    ...
    assert!(getblock_index_response.is_ok(), "getblock with index should be handled successfully");
    
    // Test getrawtransaction handler
    let getrawtransaction_request = JsonRpcRequest {
        jsonrpc: "2.0".to_string(),
        method: "getrawtransaction".to_string(),
        params: vec![json!("0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890")],
        id: Some(json!(5)),
    };
    
    let getrawtransaction_response = rpc_server.handle_request(getrawtransaction_request).await;
    assert!(getrawtransaction_response.is_ok(), "getrawtransaction should be handled successfully");
    ...
    assert!(invalid_response.is_ok(), "Invalid method should be handled gracefully");
    
    let response = invalid_response.unwrap();
    assert!(response.error.is_some(), "Invalid method should return an error");
    assert_eq!(response.error.unwrap().code, -32601, "Should return 'Method not found' error");
...
async fn test_rpc_error_handling() {
    ...
    let response = rpc_server.handle_request(invalid_version_request).await.unwrap();
    assert!(response.error.is_some(), "Invalid JSON-RPC version should return error");
    ...
    let response = rpc_server.handle_request(invalid_params_request).await.unwrap();
    ...
    let response = rpc_server.handle_request(missing_params_request).await.unwrap();
    assert!(response.error.is_some(), "Missing required params should return error");
    ...
    let response = rpc_server.handle_request(too_many_params_request).await.unwrap();
    ...
    let response = rpc_server.handle_request(internal_error_request).await.unwrap();
    // Should return method not found or internal error
    assert!(response.error.is_some(), "Non-existent method should return error");
...
async fn test_rpc_performance() {
    ...
    println!("  Processed {} requests in {:?} (avg: {:?})", 
             request_count, processing_time, avg_time);
    
    // Performance assertions
    assert!(avg_time.as_millis() < 100, "Average request processing should be fast");
    assert!(processing_time.as_secs() < 10, "Total processing time should be reasonable");
    
    // Test concurrent request handling
    let concurrent_requests = 50;
    let concurrent_start = std::time::Instant::now();
    
    let concurrent_tasks = (0..concurrent_requests).map(|i| {
    ...
    for (i, result) in concurrent_results.iter().enumerate() {
        ...
        assert!(response.error.is_none(), "Concurrent request {} should succeed", i);
    }
    
    println!("  Processed {} concurrent requests in {:?}", 
    ...
    assert!(large_response.is_ok(), "Large response should be handled successfully");
...
async fn test_websocket_rpc_functionality() {
    ...
    assert!(subscription_result.is_ok(), "WebSocket subscription should be handled successfully");
    ...
    assert!(subscription_response.contains("subscription_id"), "Should return subscription ID");
    assert!(subscription_response.contains("block_added"), "Should confirm subscription topic");
    ...
    assert!(notification_test.is_ok(), "WebSocket notifications should be delivered");
    
    println!("  ✅ WebSocket subscription established: {} -> {}", ws_request.method, subscription_response);
    ...
    assert!(ws_notification.id.is_none(), "Notification should have no ID");
...
async fn test_rpc_authentication() {
    ...
    assert!(auth_server.config().enable_auth, "Authentication should be enabled");
    assert!(auth_server.config().auth_token.is_some(), "Auth token should be configured");
    ...
    let auth_headers = {
        let mut headers = HashMap::new();
        headers.insert("Authorization".to_string(), "Bearer super_secret_token_123".to_string());
        headers
    };
    
    // Simulate authentication check
    let is_authenticated = auth_headers.get("Authorization")
        .map(|auth| auth == "Bearer super_secret_token_123")
        .unwrap_or(false);
    
    assert!(is_authenticated, "Valid token should authenticate successfully");
    ...
    let is_invalid_auth = invalid_auth_headers.get("Authorization")
        .map(|auth| auth == "Bearer super_secret_token_123")
        .unwrap_or(false);
...
async fn test_rpc_batch_requests() {
    ...
    for i in 0..3 {
        ...
        assert_eq!(response.id, Some(json!(i + 1)), "Response ID should match request ID");
        assert!(response.error.is_none(), "Valid methods should not return errors");
    }
    
    // Fourth should fail (non-existent method)
    let error_response = &batch_responses[3];
    assert_eq!(error_response.id, Some(json!(4)), "Error response ID should match");
    assert!(error_response.error.is_some(), "Invalid method should return error");
    assert_eq!(error_response.error.as_ref().unwrap().code, -32601, "Should be method not found error");
    ...
    assert_eq!(large_batch_responses.len(), large_batch_size, "Should process all batch requests");
    println!("  Processed batch of {} requests in {:?}", large_batch_size, batch_time);
