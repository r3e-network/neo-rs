...
use crate::test_mocks::{
    ledger::{Block, BlockHeader, Blockchain},
    network::{
        p2p::Node,
        sync::{SyncManager, SyncState, SyncStrategy},
    },
    MockPeer, Transaction,
};
...
async fn test_initial_block_download() {
    ...
    assert_eq!(
        ...
        "Not all blocks were synchronized"
    );

    // Verify block hashes match
    for height in 0..=source_height {
        let source_block = source_blockchain
            .get_block_by_index(height)
            .await
            .unwrap()
            .unwrap();
        let target_block = target_blockchain
            .get_block_by_index(height)
            .await
            .unwrap()
            .unwrap();
...
async fn test_parallel_block_download() {
    ...
    for (peer_id, stat) in stats {
        println!(
            "Peer {} downloaded {} blocks",
            peer_id, stat.blocks_downloaded
        );
        assert!(
            stat.blocks_downloaded > 50,
            "Peer {} didn't participate enough",
            peer_id
        );
    }
}
...
async fn test_chain_reorganization() {
    ...
    let sync_manager = Arc::new(SyncManager::new(main_chain.clone(), SyncStrategy::FastSync));
    ...
    assert_eq!(
        current_height, 110,
        "Chain didn't reorganize to longer chain"
    );

    // Verify blocks 81-110 match alternative chain
    for height in 81..=110 {
        let main_block = main_chain
            .get_block_by_index(height)
            .await
            .unwrap()
            .unwrap();
        ...
        assert_eq!(
            main_block.hash(),
            alt_block.hash(),
            "Block mismatch after reorg at height {}",
            height
        );
    }
}
...
async fn test_checkpoint_sync() {
    ...
    assert!(
        validation_stats.full_validations < total_height / 2,
...
async fn test_sync_recovery() {
    ...
    assert_eq!(
        sync_stats.blocks_processed, 500,
...
async fn test_slow_peer_handling() {
    ...
    let slow_peer = create_slow_mock_peer(blockchain.clone(), Duration::from_secs(2)).await;
    ...
    assert!(
        fast_peer_blocks > slow_peer_blocks * 2,
...
async fn create_empty_blockchain() -> Arc<Blockchain> {
    let blockchain = Blockchain::new(NetworkType::TestNet, "/tmp/neo-test-blockchain-empty")
        .await
        .unwrap();
    Arc::new(blockchain)
}
...
async fn create_slow_mock_peer(blockchain: Arc<Blockchain>, delay: Duration) -> MockPeer {
    MockPeer {
        id: 1,
        blockchain,
