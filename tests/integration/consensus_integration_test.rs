...
use crate::test_mocks::{
    consensus::{
        messages::{ChangeView, Commit, ConsensusMessage, PrepareRequest, PrepareResponse},
        ConsensusContext, ConsensusPhase, DbftConfig, DbftEngine,
    },
    ledger::{Block, Blockchain, MemoryPool},
    Transaction,
};
...
async fn test_consensus_happy_path() {
        ...
        let handle = tokio::spawn(async move {
            ...
            while let Some(msg) = rx.recv().await {
                ...
                for (j, other_engine) in engines_clone.iter().enumerate() {
                    if i != j {
                        other_engine
                            .handle_consensus_message(msg.clone())
                            .await
                            .unwrap();
                    }
                }
    ...
    assert!(
        block_hashes.iter().all(|h| h == first_hash),
...
async fn test_consensus_recovery() {
    ...
    assert_eq!(
        recovered_context.get_phase().await,
...
async fn test_consensus_performance() {
    ...
    println!(
        "Consensus completed in {:?} for {} transactions",
        elapsed, tx_count
    );
...
async fn test_consensus_persistence() {
    ...
    let (engine1, _) = create_test_consensus_engine_with_storage(validator_id, 4, &data_path).await;
    ...
    let (engine2, _) = create_test_consensus_engine_with_storage(validator_id, 4, &data_path).await;
...
async fn create_test_consensus_engine(
    ...
    let engine = Arc::new(
        DbftEngine::new(config, validator_id, blockchain, mempool, tx)
            .await
            .unwrap(),
    );
...
async fn create_test_blockchain() -> Arc<Blockchain> {
    let blockchain = Blockchain::new(NetworkType::TestNet, "/tmp/neo-test-blockchain")
        .await
        .unwrap();
    Arc::new(blockchain)
}
