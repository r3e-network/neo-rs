...
use crate::test_mocks::{Signer, Transaction, Witness, WitnessScope};
...
async fn test_full_consensus_network() {
    ...
    for i in 0..validator_count {
        let config =
            create_validator_node_config(i, 30333 + i as u16, 40332 + i as u16, validators.clone());
    ...
    for i in 0..validator_count {
        for j in 0..validator_count {
            if i != j {
                nodes[i]
                    .connect_peer(&format!("127.0.0.1:{}", 30333 + j))
                    .await
                    .ok();
            }
        }
    ...
    assert!(
        latest_blocks.iter().all(|b| b.hash() == first_hash),
    ...
    log::info!(
        "{} out of {} transactions included in blocks",
        included_count,
        tx_count
    );
    assert!(
        included_count > tx_count / 2,
...
async fn test_network_fault_tolerance() {
    ...
    for i in 0..validator_count {
        let config =
            create_validator_node_config(i, 31333 + i as u16, 41332 + i as u16, validators.clone());
    ...
    for i in validator_count..total_nodes {
        let config = create_regular_node_config(31333 + i as u16, 41332 + i as u16);
    ...
    assert_eq!(
        recovered_height, network_height,
...
async fn test_high_throughput_processing() {
    ...
    for i in 0..4 {
        let config =
            create_validator_node_config(i, 32333 + i as u16, 42332 + i as u16, validators.clone());
    ...
    assert!(
        total_processed > total_submitted / 2,
...
async fn test_smart_contract_state_consistency() {
    ...
    for i in 0..3 {
        let config =
            create_validator_node_config(i, 33333 + i as u16, 43332 + i as u16, validators.clone());
    ...
    for i in 0..invocations {
        ...
        // Increment counter
        let result = invoke_contract_method(&client, &contract_hash, "increment", vec![]).await;
    ...
    for i in 0..3 {
        ...
        let value = query_contract_state(&client, &contract_hash, "getCounter").await;
    ...
    assert!(
        counter_values.iter().all(|v| *v == counter_values[0]),
        "Nodes have different contract states: {:?}",
        counter_values
    );

    // Value should match number of successful invocations
    assert!(
        counter_values[0] >= invocations / 2,
...
async fn test_transaction_propagation() {
    ...
    for i in 0..3 {
        let config =
            create_validator_node_config(i, 34333 + i as u16, 44332 + i as u16, validators.clone());
    ...
    // Start regular nodes
    for i in 3..5 {
        let config = create_regular_node_config(34333 + i as u16, 44332 + i as u16);

        let node = Node::new(config).await.unwrap();

        // Connect to validators
        for j in 0..3 {
            node.connect_peer(&format!("127.0.0.1:{}", 34333 + j))
                .await
                .ok();
        }
...
async fn query_contract_state(client: &RpcClient, contract: &UInt160, method: &str) -> u64 {
