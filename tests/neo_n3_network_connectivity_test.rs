...
use neo_network::{NetworkConfig, P2PNode, P2PConfig, ProtocolVersion, NodeInfo};
    ...
    match timeout(Duration::from_secs(120), service.start()).await {
        Ok(Ok(_)) => {
            ...
            if let Some(p2p_node) = service.p2p_node() {
                ...
                assert!(peer_count > 0, "Should connect to at least one MainNet peer");
    ...
    match timeout(Duration::from_secs(120), service.start()).await {
        Ok(Ok(_)) => {
            ...
            if let Some(p2p_node) = service.p2p_node() {
                ...
                assert!(peer_count > 0, "Should connect to at least one TestNet peer");
...
async fn test_neo_n3_protocol_compatibility() {
    ...
    assert_eq!(testnet_magic, 0x3554334E, "TestNet magic should match C# Neo");
    ...
    assert!(!node_info.capabilities.is_empty(), "Should have capabilities");
