...
use neo_primitives::{UInt160, UInt256, error::PrimitiveResult};
...
// `SerializablePayload` impl for `Header`.
//
// `SerializablePayload` is defined in `neo-primitives` (Layer 0). The
// orphan rule requires the impl to live in a crate that owns either
// the trait or the type. `neo-primitives` does not depend on
// `neo-ledger-types` (Layer 0 cannot depend on Layer 1), so the impl
// must live here — `Header` is local to this crate.
