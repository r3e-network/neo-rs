...
impl StdLib {
    ...
    pub fn new() -> Self {
        ...
        let hash = UInt160::from_bytes(&[
            0xac, 0xce, 0x6f, 0xd8, 0x0d, 0x44, 0xe1, 0x79, 0x6a, 0xa0,
            0xc2, 0xc6, 0x25, 0xe9, 0xb4, 0xcf, 0xd2, 0xfa, 0x31, 0xa6,
        ]).unwrap();
    ...
    fn atoi(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("atoi requires string argument".to_string()));
        }
    ...
    fn itoa(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() || args[0].len() != 8 {
            return Err(Error::NativeContractError("itoa requires integer argument".to_string()));
        }
    ...
    fn base64_encode(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("base64Encode requires data argument".to_string()));
        }
    ...
    fn base64_decode(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("base64Decode requires string argument".to_string()));
        }
        ...
        use base64::{Engine as _, engine::general_purpose};
        let decoded = general_purpose::STANDARD.decode(&string_data)
    ...
    fn json_serialize(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("jsonSerialize requires data argument".to_string()));
        }
        
        // For simplicity, treat the input as a string and create a JSON string
        let string_data = String::from_utf8(args[0].clone())
            .unwrap_or_else(|_| hex::encode(&args[0]));
    ...
    fn json_deserialize(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("jsonDeserialize requires JSON string argument".to_string()));
        }
        ...
        let json_value: JsonValue = serde_json::from_str(&json_string)
            .map_err(|e| Error::NativeContractError(format!("JSON deserialization error: {}", e)))?;
    ...
    fn memory_compare(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.len() < 2 {
            return Err(Error::NativeContractError("memoryCompare requires two data arguments".to_string()));
        }
    ...
    fn memory_search(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.len() < 2 {
            return Err(Error::NativeContractError("memorySearch requires data and pattern arguments".to_string()));
        }
    ...
    fn string_split(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.len() < 2 {
            return Err(Error::NativeContractError("stringSplit requires string and delimiter arguments".to_string()));
        }
    ...
    fn string_len(&self, args: &[Vec<u8>]) -> Result<Vec<u8>> {
        if args.is_empty() {
            return Err(Error::NativeContractError("stringLen requires string argument".to_string()));
        }
...
mod tests {
    ...
    fn test_base64_encode_decode() {
        ...
        // Decode
        let decoded_result = std_lib.base64_decode(&[encoded_string.into_bytes()]).unwrap();
