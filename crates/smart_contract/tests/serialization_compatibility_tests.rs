...
fn test_variable_length_encoding_compatibility() {
    ...
    for (value, expected) in test_cases {
        ...
        assert_eq!(serialized, expected, "VarInt encoding failed for value: {}", value);
        
        // Test deserialization
        let mut reader = BinaryReader::new(&serialized);
        let deserialized = reader.read_var_int().unwrap();
        assert_eq!(value, deserialized, "VarInt decoding failed for value: {}", value);
    }
}
...
fn test_string_serialization_compatibility() {
    ...
    for test_string in test_strings {
        ...
        expected_writer.write_var_int(utf8_bytes.len() as u64).unwrap();
        expected.extend_from_slice(&expected_writer.to_bytes());
        expected.extend_from_slice(utf8_bytes);
        
        assert_eq!(serialized, expected, "String serialization failed for: {}", test_string);
        
        // Test deserialization
        let mut reader = BinaryReader::new(&serialized);
        let deserialized = reader.read_string().unwrap();
        assert_eq!(test_string, deserialized, "String deserialization failed for: {}", test_string);
    }
}
...
fn test_byte_array_serialization_compatibility() {
    ...
    for test_array in test_arrays {
        ...
        let mut expected = Vec::new();
        let mut expected_writer = BinaryWriter::new();
        expected_writer.write_var_int(test_array.len() as u64).unwrap();
        expected.extend_from_slice(&expected_writer.to_bytes());
        expected.extend_from_slice(&test_array);
        
        assert_eq!(serialized, expected, "Byte array serialization failed for array of length: {}", test_array.len());
        
        // Test deserialization
        let mut reader = BinaryReader::new(&serialized);
        let deserialized = reader.read_bytes().unwrap();
        assert_eq!(test_array, deserialized, "Byte array deserialization failed for array of length: {}", test_array.len());
    }
}
...
fn test_biginteger_serialization_compatibility() {
    ...
    for (value, expected) in test_cases {
        ...
        assert_eq!(serialized, expected, "BigInteger serialization failed for value: {}", value);
        
        // Test deserialization
        let mut reader = BinaryReader::new(&serialized);
        let deserialized = BigInteger::deserialize(&mut reader).unwrap();
        assert_eq!(bigint, deserialized, "BigInteger deserialization failed for value: {}", value);
    }
}
...
fn test_endianness_compatibility() {
    ...
    assert_eq!(&serialized[6..14], &[0xf0, 0xde, 0xbc, 0x9a, 0x78, 0x56, 0x34, 0x12]); // u64
