...
use crate::{RpcClient, Nep17Api, Utility};
use neo_core::{
    Contract, KeyPair, NativeContract, Signer, Transaction, UInt160, WitnessScope
};
...
impl WalletApi {
    ...
    pub async fn get_unclaimed_gas(&self, account: &str) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    pub async fn get_unclaimed_gas_from_hash(&self, account: &UInt160) -> Result<f64, Box<dyn std::error::Error>> {
        let script_hash = NativeContract::neo().hash();
        let block_count = self.rpc_client.get_block_count().await?;
        
        let result = self.nep17_api.contract_client
            .test_invoke(
                &script_hash,
                "unclaimedGas",
                vec![
        ...
        let stack_item = result.stack.first()
    ...
    pub async fn get_neo_balance(&self, account: &str) -> Result<u32, Box<dyn std::error::Error>> {
        let balance = self.get_token_balance(&NativeContract::neo().hash().to_string(), account).await?;
    ...
    pub async fn get_gas_balance(&self, account: &str) -> Result<f64, Box<dyn std::error::Error>> {
        let balance = self.get_token_balance(&NativeContract::gas().hash().to_string(), account).await?;
        let gas_factor = NativeContract::gas().factor();
        Ok(balance.to_f64().unwrap_or(0.0) / gas_factor as f64)
    }
    ...
    pub async fn get_token_balance(
        ...
        self.nep17_api.balance_of(&token_script_hash, &account_hash).await
    }
    ...
    pub async fn claim_gas_from_account(
        ...
        let neo_balance = self.nep17_api
        ...
        self.nep17_api.create_transfer_tx_with_from(
    ...
    pub async fn transfer(
        ...
        let tx = self.nep17_api.create_transfer_tx(
            &token_script_hash,
            key,
            &to,
            amount,
    ...
    pub async fn get_account_state(
        ...
        let gas_balance = self.nep17_api
        ...
        Ok(AccountState {
            ...
            gas_balance: gas_balance.to_f64().unwrap_or(0.0) / NativeContract::gas().factor() as f64,
            unclaimed_gas,
        })
    }
}
