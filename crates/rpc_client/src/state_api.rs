...
use crate::RpcClient;
use crate::models::{RpcStateRoot, RpcFoundStates};
...
impl StateApi {
    ...
    pub async fn get_state_root(&self, index: u32) -> Result<RpcStateRoot, Box<dyn std::error::Error>> {
    ...
    pub async fn get_proof(
        ...
        let proof_str = result.as_string()
            .ok_or("Invalid response format")?;
            
        base64::decode(proof_str)
            .map_err(|e| e.into())
    }
    ...
    pub async fn verify_proof(
        ...
        let value_str = result.as_string()
            .ok_or("Invalid response format")?;
    ...
    pub async fn get_state_height(&self) -> Result<(Option<u32>, Option<u32>), Box<dyn std::error::Error>> {
        let result = self.rpc_client
        ...
        let local_root_index = obj.get("localrootindex")
            .and_then(|v| v.as_number())
            .map(|n| n as u32);
            
        let validated_root_index = obj.get("validatedrootindex")
            .and_then(|v| v.as_number())
            .map(|n| n as u32);
    ...
    pub async fn find_states(
        ...
        let result = self.rpc_client
            .rpc_send_async("findstates", params)
            .await?;
            
        let obj = result.as_object()
            .ok_or("Invalid response format")?;
            
        RpcFoundStates::from_json(obj)
            .map_err(|e| e.into())
    }
