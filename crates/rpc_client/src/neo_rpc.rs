...
use crate::{RpcClient, RpcResult, RpcError};
...
impl RpcClient {
    ...
    pub async fn get_best_block_hash(&self) -> RpcResult<UInt256> {
        let result = self.call_raw("getbestblockhash".to_string(), json!([])).await?;
    ...
    pub async fn get_block_header(&self, hash_or_index: Value, verbose: Option<bool>) -> RpcResult<Value> {
        let params = if let Some(v) = verbose {
            json!([hash_or_index, v])
        } else {
            json!([hash_or_index])
    ...
    pub async fn get_block_count(&self) -> RpcResult<u32> {
        let result = self.call_raw("getblockcount".to_string(), json!([])).await?;
    ...
    pub async fn get_block_hash(&self, index: u32) -> RpcResult<UInt256> {
        let result = self.call_raw("getblockhash".to_string(), json!([index])).await?;
        self.parse_uint256(&result)
    }

    /// Gets the connection count (matches C# GetConnectionCountAsync)
    pub async fn get_connection_count(&self) -> RpcResult<u32> {
        let result = self.call_raw("getconnectioncount".to_string(), json!([])).await?;
        self.parse_u32(&result)
    }
    ...
    pub async fn get_next_block_validators(&self) -> RpcResult<Vec<Value>> {
        let result = self.call_raw("getnextblockvalidators".to_string(), json!([])).await?;
    ...
    pub async fn get_raw_transaction(&self, tx_hash: UInt256, verbose: Option<bool>) -> RpcResult<Value> {
        let params = if let Some(v) = verbose {
    ...
    pub async fn get_storage(&self, contract_hash: UInt160, key: &str) -> RpcResult<Option<String>> {
        let result = self.call_raw("getstorage".to_string(), 
            json!([contract_hash.to_string(), key])).await?;
    ...
    pub async fn get_transaction_height(&self, tx_hash: UInt256) -> RpcResult<u32> {
        let result = self.call_raw("gettransactionheight".to_string(), 
            json!([tx_hash.to_string()])).await?;
        self.parse_u32(&result)
    }
    ...
    pub async fn get_state_root(&self, index: u32) -> RpcResult<Value> {
        self.call_raw("getstateroot".to_string(), json!([index])).await
    }

    /// Gets the proof (matches C# GetProofAsync)
    pub async fn get_proof(&self, root_hash: UInt256, contract_hash: UInt160, key: &str) -> RpcResult<Value> {
        self.call_raw("getproof".to_string(), 
            json!([root_hash.to_string(), contract_hash.to_string(), key])).await
    }

    /// Verifies proof (matches C# VerifyProofAsync)
    pub async fn verify_proof(&self, root_hash: UInt256, proof: &str) -> RpcResult<Value> {
        self.call_raw("verifyproof".to_string(), 
            json!([root_hash.to_string(), proof])).await
    }
    ...
    pub async fn invoke_function(
        ...
        self.call_raw("invokefunction".to_string(), json!(rpc_params)).await
    }

    /// Invokes a script (matches C# InvokeScriptAsync)
    pub async fn invoke_script(&self, script: &str, signers: Option<Vec<Value>>) -> RpcResult<Value> {
        ...
        self.call_raw("invokescript".to_string(), json!(params)).await
    }
    ...
    pub async fn get_contract_state(&self, contract_hash: UInt160) -> RpcResult<Value> {
        self.call_raw("getcontractstate".to_string(), 
            json!([contract_hash.to_string()])).await
    }

    /// Gets NEP-17 balances (matches C# GetNep17BalancesAsync)
    pub async fn get_nep17_balances(&self, address: &str) -> RpcResult<Value> {
        self.call_raw("getnep17balances".to_string(), json!([address])).await
    }

    /// Gets NEP-17 transfers (matches C# GetNep17TransfersAsync)
    pub async fn get_nep17_transfers(&self, address: &str, timestamp: Option<u64>) -> RpcResult<Value> {
    ...
    pub async fn get_nep11_balances(&self, address: &str) -> RpcResult<Value> {
        self.call_raw("getnep11balances".to_string(), json!([address])).await
    }

    /// Gets NEP-11 transfers (matches C# GetNep11TransfersAsync)
    pub async fn get_nep11_transfers(&self, address: &str, timestamp: Option<u64>) -> RpcResult<Value> {
        let params = if let Some(ts) = timestamp {
            json!([address, ts])
        } else {
            json!([address])
    ...
    pub async fn get_nep11_properties(&self, contract_hash: UInt160, token_id: &str) -> RpcResult<Value> {
        self.call_raw("getnep11properties".to_string(), 
            json!([contract_hash.to_string(), token_id])).await
    }
    ...
    pub async fn dump_priv_key(&self, address: &str) -> RpcResult<String> {
        let result = self.call_raw("dumpprivkey".to_string(), json!([address])).await?;
    ...
    pub async fn open_wallet(&self, path: &str, password: &str) -> RpcResult<Value> {
        self.call_raw("openwallet".to_string(), json!([path, password])).await
    }
    ...
    pub async fn send_from(
        ...
        self.call_raw("sendfrom".to_string(), 
            json!([asset_id, from, to, value])).await
    }
    ...
    pub async fn send_to_address(&self, asset_id: &str, to: &str, value: &str) -> RpcResult<Value> {
        self.call_raw("sendtoaddress".to_string(), json!([asset_id, to, value])).await
    }

    /// Sends many (matches C# SendManyAsync)
    pub async fn send_many(&self, outputs: Vec<Value>) -> RpcResult<Value> {
        self.call_raw("sendmany".to_string(), json!([outputs])).await
    }
    ...
    pub async fn get_application_log(&self, tx_hash: UInt256) -> RpcResult<Value> {
        self.call_raw("getapplicationlog".to_string(), 
            json!([tx_hash.to_string()])).await
    }
    ...
    fn parse_uint256(&self, value: &Value) -> RpcResult<UInt256> {
        ...
        UInt256::from_hex_string(&str_val)
            .map_err(|e| RpcError::Parse(format!("Invalid UInt256: {}", e)))
    }

    /// Parses a UInt160 from JSON value
    fn parse_uint160(&self, value: &Value) -> RpcResult<UInt160> {
        let str_val = self.parse_string(value)?;
        UInt160::from_hex_string(&str_val)
            .map_err(|e| RpcError::Parse(format!("Invalid UInt160: {}", e)))
    }
