...
use neo_core::{
    Contract, ContractParametersContext, ECPoint, KeyPair, 
    Signer, Transaction, TransactionAttribute, Witness
};
...
impl TransactionManager {
    ...
    pub async fn make_transaction(
        ...
        factory.make_transaction(script, &signers.unwrap_or_default()).await
    }
    ...
    pub async fn make_transaction_with_fee(
        ...
        let factory = TransactionManagerFactory::new(rpc_client);
        let mut manager = factory.make_transaction(script, &signers.unwrap_or_default()).await?;
    ...
    pub fn add_signature(&mut self, key: &KeyPair) -> Result<&mut Self, Box<dyn std::error::Error>> {
    ...
    pub fn add_witness(&mut self, contract: Contract) -> Result<&mut Self, Box<dyn std::error::Error>> {
    ...
    pub async fn sign(&mut self) -> Result<Transaction, Box<dyn std::error::Error>> {
        ...
        for item in &self.sign_store {
            for key in &item.key_pairs {
                ...
                self.context.add_signature(&item.contract, &key.public_key(), signature)?;
            }
        }
    ...
    fn get_contract(&self, script_hash: &neo_core::UInt160) -> Result<Contract, Box<dyn std::error::Error>> {
