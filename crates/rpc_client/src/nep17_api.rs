...
use neo_core::{
    Contract, KeyPair, Signer, Transaction, UInt160, WitnessScope
};
...
impl Nep17Api {
    ...
    pub async fn balance_of(
        ...
        let result = self.contract_client
    ...
    pub async fn symbol(&self, script_hash: &UInt160) -> Result<String, Box<dyn std::error::Error>> {
        let result = self.contract_client
    ...
    pub async fn total_supply(&self, script_hash: &UInt160) -> Result<BigInt, Box<dyn std::error::Error>> {
        let result = self.contract_client
    ...
    pub async fn get_token_info(
        ...
        // Get contract state
        let contract_state = self.rpc_client
        ...
        Ok(RpcNep17TokenInfo {
            ...
            symbol: stack.get(0)
                ...
                .unwrap_or_default(),
            decimals: stack.get(1)
                .and_then(|s| s.get_integer().ok())
                .and_then(|i| i.to_u8())
                .unwrap_or(0),
            total_supply: stack.get(2)
                .and_then(|s| s.get_integer().ok())
    ...
    pub async fn get_token_info_with_balance(
        ...
        let account = UInt160::from_address(address, self.rpc_client.protocol_settings.address_version)?;
    ...
    pub async fn create_transfer_tx(
        ...
        let from = Contract::create_signature_redeem_script(&key.public_key())
            .to_script_hash();
            
        self.create_transfer_tx_with_from(script_hash, &from, key, to, amount, data).await
    }
    ...
    pub async fn get_nep17_transfers(
        ...
        self.rpc_client.get_nep17_transfers(address, start_time, end_time).await
    }
