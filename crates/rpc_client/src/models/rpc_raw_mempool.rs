...
impl RpcRawMemPool {
    ...
    pub fn to_json(&self) -> JObject {
        ...
        let verified_array: Vec<JToken> = self.verified
            .iter()
            .map(|h| JToken::String(h.to_string()))
            .collect();
        json.insert("verified".to_string(), JToken::Array(JArray::from(verified_array)));
        
        let unverified_array: Vec<JToken> = self.unverified
            .iter()
            .map(|h| JToken::String(h.to_string()))
            .collect();
        json.insert("unverified".to_string(), JToken::Array(JArray::from(unverified_array)));
    ...
    pub fn from_json(json: &JObject) -> Result<Self, String> {
        ...
        let unverified = json.get("unverified")
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
                    .filter_map(|item| item.as_string())
