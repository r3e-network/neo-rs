...
impl RpcApplicationLog {
    ...
    pub fn to_json(&self) -> JObject {
        ...
        if let Some(ref block_hash) = self.block_hash {
            json.insert("blockhash".to_string(), JToken::String(block_hash.to_string()));
        }
        
        let executions_array: Vec<JToken> = self.executions
            .iter()
            .map(|e| JToken::Object(e.to_json()))
            .collect();
        json.insert("executions".to_string(), JToken::Array(JArray::from(executions_array)));
    ...
    pub fn from_json(json: &JObject, protocol_settings: &ProtocolSettings) -> Result<Self, String> {
        ...
        let executions = json.get("executions")
...
impl Execution {
    ...
    pub fn to_json(&self) -> JObject {
        let mut json = JObject::new();
        json.insert("trigger".to_string(), JToken::String(self.trigger.to_string()));
        json.insert("vmstate".to_string(), JToken::String(self.vm_state.to_string()));
        ...
        let stack_array: Vec<JToken> = self.stack
            .iter()
            .filter_map(|s| s.to_json().ok())
            .map(JToken::Object)
            .collect();
        json.insert("stack".to_string(), JToken::Array(JArray::from(stack_array)));
        
        let notifications_array: Vec<JToken> = self.notifications
            .iter()
            .map(|n| JToken::Object(n.to_json()))
            .collect();
        json.insert("notifications".to_string(), JToken::Array(JArray::from(notifications_array)));
    ...
    /// Matches C# FromJson
    pub fn from_json(json: &JObject, protocol_settings: &ProtocolSettings) -> Result<Self, String> {
        let trigger_str = json.get("trigger")
            .and_then(|v| v.as_string())
        ...
        let vm_state_str = json.get("vmstate")
            .and_then(|v| v.as_string())
        ...
        let exception_message = json.get("exception")
            .and_then(|v| v.as_string())
            .map(|s| s.to_string());
        ...
        let notifications = json.get("notifications")
            .and_then(|v| v.as_array())
            .map(|arr| {
                arr.iter()
                    .filter_map(|item| item.as_object())
...
impl RpcNotifyEventArgs {
    ...
    pub fn to_json(&self) -> JObject {
        let mut json = JObject::new();
        json.insert("contract".to_string(), JToken::String(self.contract.to_string()));
        json.insert("eventname".to_string(), JToken::String(self.event_name.clone()));
    ...
    /// Creates from JSON
    /// Matches C# FromJson
    pub fn from_json(json: &JObject, _protocol_settings: &ProtocolSettings) -> Result<Self, String> {
        let contract = json.get("contract")
            .and_then(|v| v.as_string())
            .and_then(|s| UInt160::parse(s).ok())
            .ok_or("Missing or invalid 'contract' field")?;
            
        let event_name = json.get("eventname")
            .and_then(|v| v.as_string())
            .ok_or("Missing or invalid 'eventname' field")?
            .to_string();
            
        let state_json = json.get("state")
