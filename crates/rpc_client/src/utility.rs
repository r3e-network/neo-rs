...
impl Utility {
    ...
    pub fn to_script_hash(value: &JToken, protocol_settings: &ProtocolSettings) -> Result<UInt160, String> {
        let address_or_script_hash = value.as_string()
        ...
        if address_or_script_hash.len() < 40 {
            ...
            UInt160::parse(address_or_script_hash)
                .map_err(|e| e.to_string())
        }
    }
    ...
    pub fn get_script_hash(account: &str, protocol_settings: &ProtocolSettings) -> Result<UInt160, String> {
        if account.is_empty() {
    ...
    pub fn block_to_json(block: &Block, protocol_settings: &ProtocolSettings) -> JObject {
        ...
        json.insert("previousblockhash".to_string(), JToken::String(block.prev_hash.to_string()));
        json.insert("merkleroot".to_string(), JToken::String(block.merkle_root.to_string()));
        json.insert("time".to_string(), JToken::Number(block.timestamp as f64));
        json.insert("nonce".to_string(), JToken::String(format!("{:016x}", block.nonce)));
        json.insert("index".to_string(), JToken::Number(block.index as f64));
        json.insert("primary".to_string(), JToken::Number(block.primary_index as f64));
        
        if let Some(ref next_consensus) = block.next_consensus {
            json.insert("nextconsensus".to_string(), JToken::String(next_consensus.to_address(protocol_settings.address_version)));
        }
        
        // Add witness
        json.insert("witnesses".to_string(), JToken::Array(
            block.witness.iter()
        ...
        // Add transactions
        json.insert("tx".to_string(), JToken::Array(
    ...
    pub fn block_from_json(json: &JObject, protocol_settings: &ProtocolSettings) -> Result<Block, String> {
    ...
    pub fn transaction_to_json(tx: &Transaction, protocol_settings: &ProtocolSettings) -> JObject {
        ...
        json.insert("sender".to_string(), JToken::String(tx.sender().to_address(protocol_settings.address_version)));
        json.insert("sysfee".to_string(), JToken::String(tx.system_fee.to_string()));
        json.insert("netfee".to_string(), JToken::String(tx.network_fee.to_string()));
        json.insert("validuntilblock".to_string(), JToken::Number(tx.valid_until_block as f64));
        
        // Add signers
        json.insert("signers".to_string(), JToken::Array(
            tx.signers().iter()
        ...
        // Add attributes
        json.insert("attributes".to_string(), JToken::Array(
            tx.attributes().iter()
        ...
        // Add script
        json.insert("script".to_string(), JToken::String(base64::encode(&tx.script())));
    ...
    pub fn transaction_from_json(json: &JObject, protocol_settings: &ProtocolSettings) -> Result<Transaction, String> {
...
fn witness_to_json(witness: &neo_core::Witness) -> JObject {
    let mut json = JObject::new();
    json.insert("invocation".to_string(), JToken::String(base64::encode(&witness.invocation_script)));
    json.insert("verification".to_string(), JToken::String(base64::encode(&witness.verification_script)));
    json
}

fn signer_to_json(signer: &neo_core::Signer, protocol_settings: &ProtocolSettings) -> JObject {
    let mut json = JObject::new();
    json.insert("account".to_string(), JToken::String(signer.account.to_address(protocol_settings.address_version)));
    json.insert("scopes".to_string(), JToken::String(signer.scopes.to_string()));
    
    if !signer.allowed_contracts.is_empty() {
        json.insert("allowedcontracts".to_string(), JToken::Array(
            signer.allowed_contracts.iter()
    ...
    if !signer.allowed_groups.is_empty() {
        json.insert("allowedgroups".to_string(), JToken::Array(
            signer.allowed_groups.iter()
    ...
    if !signer.rules.is_empty() {
        json.insert("rules".to_string(), JToken::Array(
            signer.rules.iter()
...
fn attribute_to_json(attr: &neo_core::TransactionAttribute) -> JObject {
    let mut json = JObject::new();
    json.insert("type".to_string(), JToken::String(attr.get_type().to_string()));
...
fn rule_to_json(rule: &neo_core::WitnessRule) -> JObject {
    let mut json = JObject::new();
    json.insert("action".to_string(), JToken::String(rule.action.to_string()));
    json.insert("condition".to_string(), JToken::Object(condition_to_json(&rule.condition)));
    json
}
