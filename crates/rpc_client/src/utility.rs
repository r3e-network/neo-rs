...
use neo_core::{
    Block, BlockHeader, Contract, ECPoint, KeyPair, ProtocolSettings, Transaction, UInt160,
    UInt256, Witness,
};
...
impl Utility {
    ...
    pub fn to_script_hash(
        ...
        if address_or_script_hash.len() < 40 {
            UInt160::from_address(&address_or_script_hash).map_err(|e| e.to_string())
        } else {
    ...
    pub fn get_script_hash(
        ...
        match account.len() {
            34 => {
                // Address
                UInt160::from_address(account).map_err(|e| e.to_string())
            }
            40 => {
                // Script hash
            ...
            66 => {
                ...
                let point =
                    ECPoint::decode_compressed(&key_bytes).map_err(|err| err.to_string())?;
...
mod tests {
    ...
    use neo_core::{
        neo_config::ADDRESS_SIZE, Signer, TransactionAttribute, UInt256, WitnessCondition,
        WitnessScope,
    };
    use neo_json::JArray;
    ...
    fn to_script_hash_accepts_address() {
        ...
        let parsed =
            Utility::to_script_hash(&token, &ProtocolSettings::default_settings()).unwrap();
        assert_eq!(parsed, hash);
        assert_eq!(
            Utility::get_script_hash(&hash.to_string(), &ProtocolSettings::default_settings())
                .unwrap(),
            hash
        );
    }
    ...
    fn get_script_hash_accepts_public_key_hex() {
        ...
        let parsed =
            Utility::get_script_hash(&pubkey_hex, &ProtocolSettings::default_settings()).unwrap();
    ...
    fn witness_roundtrip_from_json() {
        ...
        let witness = Utility::witness_from_json(&obj).expect("witness");
    ...
    fn transaction_roundtrip_json() {
        ...
        let json = Utility::transaction_to_json(&tx, &ProtocolSettings::default_settings());
        let parsed = Utility::transaction_from_json(&json, &ProtocolSettings::default_settings())
    ...
    fn block_roundtrip_json() {
        ...
        let json = Utility::block_to_json(&block, &ProtocolSettings::default_settings());
        let parsed =
            Utility::block_from_json(&json, &ProtocolSettings::default_settings()).unwrap();
    ...
    fn transaction_roundtrip_with_custom_signer() {
        use neo_core::network::p2p::payloads::witness_rule_action::WitnessRuleAction;
    ...
    fn stack_item_parses_array_and_struct() {
        ...
        let item_array = Utility::stack_item_from_json(&array_obj).unwrap();
        ...
        let item_struct = Utility::stack_item_from_json(&struct_obj).unwrap();
    ...
    fn parses_oracle_response_code_variants() {
        ...
        assert_eq!(
            crate::utility::parsing::oracle_response_code_to_str(OracleResponseCode::Timeout),
            "Timeout"
        );
    }
}
