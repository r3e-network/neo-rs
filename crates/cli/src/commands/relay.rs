...
pub async fn execute(client: &RpcClient, transaction: &str) -> CommandResult {
    ...
    let tx_hex = if transaction.chars().all(|c| c.is_ascii_hexdigit() || c == 'x') {
        transaction.strip_prefix("0x").unwrap_or(transaction).to_string()
    } else {
        // Assume base64
        let decoded = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, transaction)
            .map_err(|e| anyhow::anyhow!("Invalid transaction encoding: {}", e))?;
        hex::encode(decoded)
    };

    let params = vec![JToken::String(tx_hex)];
    let result = client.rpc_send_async("sendrawtransaction", params).await
