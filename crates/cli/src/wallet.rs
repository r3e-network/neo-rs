...
use neo_wallets::{
    Nep6Wallet, 
    wallet_account::{WalletAccount, StandardWalletAccount}, 
    KeyPair, 
...
impl WalletManager {
    ...
    pub async fn open_wallet(&mut self, path: &Path, password: &str) -> WalletResult<()> {
        ...
        if !path.exists() {
            return Err(Box::new(WalletError(format!("Wallet file not found: {}", path.display()))));
        }

        // 2. Read wallet file
        let wallet_data = fs::read_to_string(path).await
        ...
        let version = wallet_json.get("version")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Box::new(WalletError("Missing wallet version".to_string())))?;

        if version != "1.0" {
            return Err(Box::new(WalletError(format!("Unsupported wallet version: {}", version))));
        }

        // 5. Load wallet accounts (matches C# account loading exactly)
        let accounts_json = wallet_json.get("accounts")
        ...
        let wallet_name = wallet_json.get("name")
            .and_then(|v| v.as_str())
    ...
    pub async fn create_wallet(&mut self, path: &Path, password: &str, name: Option<&str>) -> WalletResult<()> {
        // Production-ready wallet creation (matches C# Neo wallet creation exactly)
        
        // 1. Check if wallet file already exists
        if path.exists() {
            return Err(Box::new(WalletError(format!("Wallet file already exists: {}", path.display()))));
        }
        ...
        let account = wallet.create_account(&key_pair.private_key()).await
    ...
    pub fn default_account(&self) -> Option<&Arc<dyn WalletAccount>> {
        self.default_account.and_then(|addr| self.accounts.get(&addr))
    }
    ...
    pub async fn import_private_key(&mut self, private_key_hex: &str, _password: &str) -> WalletResult<UInt160> {
        if let Some(wallet) = &mut self.current_wallet {
            ...
            if private_key_bytes.len() != 32 {
                return Err(Box::new(WalletError("Private key must be 32 bytes".to_string())));
            }

            let account = wallet.create_account(&private_key_bytes).await
                .map_err(|e| Box::new(WalletError(format!("Failed to create account: {}", e))))?;
            ...
            if self.accounts.contains_key(&address) {
                return Err(Box::new(WalletError("Account already exists in wallet".to_string())));
            }
    ...
    pub async fn export_private_key(&self, address: &UInt160, _password: &str) -> WalletResult<String> {
        ...
        let wif = account.export_wif().await
    ...
    pub async fn get_balance(&self, address: &UInt160, asset_id: &UInt256) -> WalletResult<i64> {
        if let Some(wallet) = &self.current_wallet {
            let _account = self.accounts.get(address)
                .ok_or_else(|| Box::new(WalletError("Account not found".to_string())))?;

            // Production-ready balance retrieval (matches C# balance checking exactly)
            let balance = wallet.get_available_balance(asset_id).await
    ...
    async fn load_wallet_account(
        ...
        let address_str = account_json.get("address")
            .and_then(|v| v.as_str())
        ...
        if let Some(key_str) = account_json.get("key").and_then(|v| v.as_str()) {
            ...
            let account = StandardWalletAccount::new_from_encrypted(script_hash, encrypted_key, None);
            Ok(Arc::new(account))
        } else {
            // Watch-only account
    ...
    pub async fn total_neo_balance(&self) -> WalletResult<i64> {
        ...
        for (address, _account) in &self.accounts {
            match self.get_neo_balance_for_account(&rpc_client, neo_contract_hash, address).await {
                Ok(balance) => {
                    total_balance = total_balance.saturating_add(balance);
                }
                Err(e) => {
                    // Log error but continue with other accounts (matches C# error handling)
                    eprintln!("Warning: Failed to get NEO balance for account {}: {}", address, e);
                }
            }
        }
    ...
    pub async fn total_gas_balance(&self) -> WalletResult<i64> {
        ...
        // 3. Query balance for each account (matches C# wallet account iteration exactly)
        for (address, _account) in &self.accounts {
            match self.get_gas_balance_for_account(&rpc_client, gas_contract_hash, address).await {
                Ok(balance) => {
                    total_balance = total_balance.saturating_add(balance);
                }
                Err(e) => {
                    // Log error but continue with other accounts (matches C# error handling)
                    eprintln!("Warning: Failed to get GAS balance for account {}: {}", address, e);
                }
            }
        }
    ...
    fn parse_balance_result(&self, result: &serde_json::Value, decimals: u8) -> WalletResult<i64> {
        ...
        if state != "HALT" {
            return Err(Box::new(WalletError(format!("Smart contract execution failed: {}", state))));
        }
        ...
        let balance_type = balance_item.get("type")
            .and_then(|t| t.as_str())
            .ok_or_else(|| Box::new(WalletError("Missing type in balance result".to_string())))?;

        if balance_type != "Integer" {
            return Err(Box::new(WalletError(format!("Expected Integer type, got: {}", balance_type))));
        }

        let balance_value = balance_item.get("value")
            .and_then(|v| v.as_str())
        ...
        let raw_balance = balance_value.parse::<i64>()
    ...
    pub async fn get_nep17_balances(&self) -> WalletResult<Vec<TokenBalance>> {
        ...
        for (address, _account) in &self.accounts {
            match self.get_account_nep17_balances(&rpc_client, address).await {
                ...
                Err(e) => {
                    eprintln!("Warning: Failed to get NEP-17 balances for account {}: {}", address, e);
                }
            }
        }
    ...
    async fn get_account_nep17_balances(
        ...
        // Parse the response
        let balances_array = result.get("balance")
    ...
    fn parse_token_balance(&self, item: &serde_json::Value, address: &UInt160) -> WalletResult<TokenBalance> {
        let asset_hash = item.get("assethash")
            .and_then(|h| h.as_str())
        ...
        let last_updated_block = item.get("lastupdatedblock")
            .and_then(|b| b.as_u64())
            .unwrap_or(0) as u32;
    ...
    pub async fn total_unclaimed_gas(&self) -> WalletResult<i64> {
        ...
        for (address, _account) in &self.accounts {
            match self.get_unclaimed_gas_for_account(&rpc_client, neo_contract_hash, address).await {
                Ok(unclaimed) => {
                    total_unclaimed = total_unclaimed.saturating_add(unclaimed);
                }
                Err(e) => {
                    // Log error but continue with other accounts (matches C# error handling)
                    eprintln!("Warning: Failed to get unclaimed GAS for account {}: {}", address, e);
                }
            }
        }
    ...
    async fn get_unclaimed_gas_for_account(
        ...
        let result = rpc_client
            .invoke_function(neo_contract_hash.to_string(), "unclaimedGas".to_string(), params)
            .await
            .map_err(|e| Box::new(WalletError(format!("RPC call failed: {}", e))))?;
