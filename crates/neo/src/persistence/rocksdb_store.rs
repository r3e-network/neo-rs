...
use crate::neo_config::MAX_BLOCK_SIZE;
use crate::{
    Error, IReadOnlyStore, IStore, IStoreSnapshot, IWriteStore, Result, SeekDirection,
    StorageConfig,
};
use rocksdb::{
    Direction, FlushOptions, IteratorMode, Options, ReadOptions, WriteBatch, WriteOptions, DB,
};
...
impl BatchProcessor {
    ...
    pub fn should_flush(&self) -> bool {
        let size_threshold =
            self.pending_writes.len() + self.pending_deletes.len() >= self.batch_threshold;
...
impl RocksDBStore {
    ...
    pub async fn batch_write_optimized(
        &self,
        ...
        if batch_processor.should_flush() {
            ...
            batch_processor
                .flush_to_batch(&mut write_batch)
    ...
    fn estimate_num_keys(&self) -> Result<u64> {
        match self.db.property_value("rocksdb.estimate-num-keys") {
            Ok(Some(value)) => value
                .parse()
                .map_err(|e| Error::DatabaseError(format!("Invalid key count: {}", e))),
            Ok(None) => Ok(0),
            Err(e) => Err(Error::DatabaseError(format!(
                "Failed to get key count: {}",
                e
            ))),
        }
    }
    ...
    fn get_approximate_size(&self) -> Result<u64> {
        match self.db.property_value("rocksdb.total-sst-files-size") {
            Ok(Some(value)) => value
                .parse()
                .map_err(|e| Error::DatabaseError(format!("Invalid size: {}", e))),
            Ok(None) => Ok(0),
    ...
    fn get_compaction_level(&self) -> Result<u32> {
        match self.db.property_value("rocksdb.num-files-at-level0") {
            Ok(Some(value)) => value
                .parse()
                .map_err(|e| Error::DatabaseError(format!("Invalid level: {}", e))),
            Ok(None) => Ok(0),
...
impl IReadOnlyStore<Vec<u8>, Vec<u8>> for RocksDBStore {
    ...
    fn try_get(&self, key: &Vec<u8>) -> Option<Vec<u8>> {
        debug!(
            "Getting key: {:?}",
            hex::encode(&key[..std::cmp::min(key.len(), 8)])
        );
    ...
    fn find(
        ...
        debug!(
            "Finding entries with prefix: {:?}, direction: {:?}",
            key_or_prefix.map(|k| hex::encode(&k[..std::cmp::min(k.len(), 8)])),
            direction
        );
        ...
        Box::new(db_iter.filter_map(move |result| match result {
            Ok((key, value)) => {
...
impl IWriteStore<Vec<u8>, Vec<u8>> for RocksDBStore {
    ...
    fn put_sync(&mut self, key: Vec<u8>, value: Vec<u8>) {
        debug!(
            "Putting key synchronously: {:?}, value length: {}",
            hex::encode(&key[..std::cmp::min(key.len(), 8)]),
            value.len()
        );
        ...
        match self.db.put_opt(&key, &value, &write_options) {
            ...
            Err(e) => {
                error!(
                    "CRITICAL: Failed to put key-value pair synchronously: {}",
                    e
                );
                ...
                warn!(
                    "Storage write operation failed - this may indicate disk issues or corruption"
                );
...
impl IStore for RocksDBStoreReference {
    fn get_snapshot(&self) -> Box<dyn IStoreSnapshot> {
        ...
        error!(
            "get_snapshot called on store reference - this should not happen in normal operation"
        );
...
impl IReadOnlyStore<Vec<u8>, Vec<u8>> for RocksDBSnapshotWrapper {
    ...
    fn find(
        ...
        debug!(
            "Finding entries in snapshot {} with prefix: {:?}",
            self.snapshot.id,
            key_or_prefix.map(|k| hex::encode(&k[..std::cmp::min(k.len(), 8)]))
        );
...
impl IWriteStore<Vec<u8>, Vec<u8>> for RocksDBSnapshotWrapper {
    ...
    fn put(&mut self, key: Vec<u8>, value: Vec<u8>) {
        debug!(
            "Putting key in snapshot {}: {:?}, value length: {}",
            self.snapshot.id,
            hex::encode(&key[..std::cmp::min(key.len(), 8)]),
            value.len()
        );
...
mod tests {
    ...
    fn test_rocksdb_store_creation() {
        ...
        let store =
            RocksDBStore::new(config).map_err(|e| StorageError::OperationFailed(e.to_string()))?;
