...
impl MemoryPool {
    ...
    pub fn verified_and_unverified_transactions(&self) -> (Vec<Transaction>, Vec<Transaction>) {
        (
            self.verified_transactions_vec(),
            self.unverified_transactions_vec(),
        )
    }

    /// Removes transactions that were committed in the provided block and moves the
    /// remaining verified transactions to the unverified set, matching the C# logic.
    pub fn update_pool_for_block_persisted(
        ...
        for tx in &block.transactions {
            if let Some(item) = self.try_remove_verified(tx.hash()) {
                ...
                removed.push(item.transaction);
            } else if let Some(item) = self.try_remove_unverified(tx.hash()) {
                removed.push(item.transaction);
            }
        }
