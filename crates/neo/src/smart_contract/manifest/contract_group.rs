...
impl ContractGroup {
    ...
    pub fn from_stack_item_value(stack_item: &StackItem) -> Self {
        ...
        if items.len() < 2 {
            panic!("ContractGroup stack item must contain two elements");
        }

        let pub_key_bytes = items[0]
            .as_bytes()
            .expect("ContractGroup public key must be byte string");
        let signature_bytes = items[1]
            .as_bytes()
            .expect("ContractGroup signature must be byte string");

        let pub_key = ECPoint::from_bytes(&pub_key_bytes)
            .unwrap_or_else(|e| panic!("Failed to decode ECPoint: {}", e));
...
impl IInteroperable for ContractGroup {
    fn from_stack_item(&mut self, stack_item: StackItem) {
        *self = Self::from_stack_item_value(&stack_item);
    }
    ...
    fn to_stack_item(&self) -> StackItem {
        let pub_key_bytes = self
            .pub_key
            .encode_point(true)
            .unwrap_or_else(|e| panic!("Failed to encode ECPoint: {}", e));
