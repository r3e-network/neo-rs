...
pub trait IStoreSnapshot: Send + Sync {
    ...
    S: IStoreSnapshot,
{
    store: Arc<S>,
    prefix: u8,
    entries: HashMap<UInt256, Trackable>,
}
    ...
    S: IStoreSnapshot,
{
    ...
    pub fn commit(&mut self) -> MptResult<()> {
        let keys: Vec<UInt256> = self.entries.keys().copied().collect();
        for hash in keys {
            let entry = self
                .entries
                .get(&hash)
                .expect("cache key collected but missing entry");
            match entry.state {
                TrackState::None => {}
    ...
    fn resolve_internal(&mut self, hash: &UInt256) -> MptResult<&mut Trackable> {
        if !self.entries.contains_key(hash) {
            let node = self.load_from_store(hash)?;
            self.entries.insert(*hash, Trackable::new(node));
        }
        Ok(self
            .entries
            .get_mut(hash)
