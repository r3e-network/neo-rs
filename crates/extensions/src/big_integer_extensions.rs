...
impl BigIntegerExtensions for BigInt {
    ...
    fn sqrt(&self) -> Result<BigInt, String> {
        ...
        let mut x = BigInt::from(1) << (((self - 1).get_bit_length() + 1) / 2);
    ...
    fn get_bit_length(&self) -> i64 {
        ...
        if bytes.len() == 1 || (bytes.len() == 2 && bytes[1] == 0) {
            return bit_count(if self.is_positive() { bytes[0] } else { 255 - bytes[0] }) as i64;
        }
        
        let last_byte = if self.is_positive() { bytes[bytes.len() - 1] } else { 255 - bytes[bytes.len() - 1] };
