...
impl MemoryReader {
    ...
    pub fn read_int16(&mut self) -> Result<i16> {
        ...
        let result = i16::from_le_bytes(bytes.try_into().unwrap());
        self.pos += 2;
        Ok(result)
    }

    /// Reads a 16-bit signed integer in big-endian format.
    pub fn read_int16_big_endian(&mut self) -> Result<i16> {
        self.ensure_position(2)?;
        let bytes = &self.span[self.pos..self.pos + 2];
        let result = i16::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 2;
        Ok(result)
    }
    ...
    pub fn read_uint16(&mut self) -> Result<u16> {
        self.ensure_position(2)?;
        let bytes = &self.span[self.pos..self.pos + 2];
        let result = u16::from_le_bytes(bytes.try_into().unwrap());
        self.pos += 2;
        Ok(result)
    }

    /// Reads a 16-bit unsigned integer in big-endian format.
    pub fn read_uint16_big_endian(&mut self) -> Result<u16> {
        self.ensure_position(2)?;
        let bytes = &self.span[self.pos..self.pos + 2];
        let result = u16::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 2;
        Ok(result)
    }
    ...
    pub fn read_int32_big_endian(&mut self) -> Result<i32> {
        self.ensure_position(4)?;
        let bytes = &self.span[self.pos..self.pos + 4];
        let result = i32::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 4;
        Ok(result)
    }
    ...
    pub fn read_uint32(&mut self) -> Result<u32> {
        self.ensure_position(4)?;
        let bytes = &self.span[self.pos..self.pos + 4];
        let result = u32::from_le_bytes(bytes.try_into().unwrap());
        self.pos += 4;
        Ok(result)
    }

    /// Reads a 32-bit unsigned integer in little-endian format (alias for compatibility).
    pub fn read_u32(&mut self) -> Result<u32> {
        self.read_uint32()
    }

    /// Reads a 32-bit unsigned integer in big-endian format.
    pub fn read_uint32_big_endian(&mut self) -> Result<u32> {
        self.ensure_position(4)?;
        let bytes = &self.span[self.pos..self.pos + 4];
        let result = u32::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 4;
        Ok(result)
    }
    ...
    pub fn read_int64_big_endian(&mut self) -> Result<i64> {
        self.ensure_position(8)?;
        let bytes = &self.span[self.pos..self.pos + 8];
        let result = i64::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 8;
        Ok(result)
    }
    ...
    pub fn read_uint64_big_endian(&mut self) -> Result<u64> {
        self.ensure_position(8)?;
        let bytes = &self.span[self.pos..self.pos + 8];
        let result = u64::from_be_bytes(bytes.try_into().unwrap());
        self.pos += 8;
        Ok(result)
    }
    ...
    pub fn read_memory(&mut self, count: usize) -> Result<Vec<u8>> {
    ...
    pub fn read_var_memory(&mut self, max: usize) -> Result<Vec<u8>> {
    ...
    pub fn read_var_bytes(&mut self, max: usize) -> Result<Vec<u8>> {
        let length = self.read_var_int(max as u64)? as usize;
...
mod tests {
    ...
    fn test_read_var_int() {
        ...
        // Test 2-byte value
        let data = vec![0xfd, 0x34, 0x12]; // 0x1234
        let mut reader = MemoryReader::new(&data);
