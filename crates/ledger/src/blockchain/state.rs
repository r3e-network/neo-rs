...
use super::storage::{StorageKey, StorageItem};
...
impl BlockchainState {
    ...
    fn initialize_native_contracts() -> HashMap<UInt160, NativeContractInfo> {
        ...
        let neo_hash = UInt160::from_bytes(&[
            0xef, 0x4f, 0x73, 0xa4, 0x85, 0x65, 0x05, 0x1d,
            0x82, 0xf3, 0xf9, 0x11, 0x73, 0xf7, 0x72, 0xf8,
            0xd6, 0x0f, 0xd0, 0xc1
        ]).unwrap();
        contracts.insert(neo_hash, NativeContractInfo {
            hash: neo_hash,
            name: "NeoToken".to_string(),
            methods: vec!["symbol".to_string(), "decimals".to_string(), "totalSupply".to_string()],
            service_level: 1,
        });

        // GAS Token Contract
        let gas_hash = UInt160::from_bytes(&[
            0xd2, 0xa4, 0xcf, 0xf3, 0x1f, 0x56, 0xe6, 0x8e,
            0xb0, 0xcc, 0xd7, 0xa3, 0x7c, 0x1b, 0x15, 0x11,
            0xe1, 0x2c, 0xce, 0x81
        ]).unwrap();
        ...
        let policy_hash = UInt160::from_bytes(&[
            0xcc, 0x5e, 0x4e, 0xdd, 0x78, 0xdf, 0xad, 0xd4,
            0x95, 0x12, 0x5b, 0xe9, 0xed, 0xc5, 0x8e, 0x78,
            0x1d, 0xda, 0x7c, 0xfc
        ]).unwrap();
        contracts.insert(policy_hash, NativeContractInfo {
            hash: policy_hash,
            name: "PolicyContract".to_string(),
            methods: vec!["getMaxTransactionsPerBlock".to_string(), "getMaxBlockSize".to_string()],
            service_level: 1,
        });
    ...
    fn create_native_contract_state(&self, native_info: &NativeContractInfo) -> ContractState {
        ContractState {
            ...
            manifest: ContractManifest {
                ...
                abi: ContractAbi {
                    methods: native_info.methods.iter().map(|method| {
    ...
    fn validate_transaction_basic_structure(&self, transaction: &Transaction) -> Result<()> {
        ...
        if transaction.valid_until_block() == 0 {
            return Err(Error::Validation("ValidUntilBlock cannot be zero".to_string()));
        }
        
        // Check system fee
        if transaction.system_fee() < 0 {
            return Err(Error::Validation("SystemFee cannot be negative".to_string()));
        }
        
        // Check network fee
        if transaction.network_fee() < 0 {
            return Err(Error::Validation("NetworkFee cannot be negative".to_string()));
        }
        
        // Check transaction size
        let tx_size = transaction.size();
        if tx_size > 102400 { // 100KB limit (matches C# MAX_TRANSACTION_SIZE)
            return Err(Error::Validation("Transaction too large".to_string()));
        }
        
        // Check signers count
        if transaction.signers().is_empty() {
            return Err(Error::Validation("Transaction must have at least one signer".to_string()));
        }
        ...
        if transaction.witnesses().len() != transaction.signers().len() {
            return Err(Error::Validation("Witness count must match signer count".to_string()));
        }
    ...
    async fn validate_transaction_policy(&self, transaction: &Transaction) -> Result<()> {
        ...
        if tx_size as u32 > self.policy_settings.max_block_size {
            return Err(Error::Validation("Transaction exceeds maximum block size".to_string()));
        }
        
        // Check against blocked accounts
        for signer in transaction.signers() {
            if self.policy_settings.blocked_accounts.contains(&signer.account) {
    ...
    async fn get_gas_balance(&self, account: &UInt160) -> Result<i64> {
        ...
        let balance_key = StorageKey::new(
            gas_hash.as_bytes().to_vec(),
            ...
            Some(item) => {
                ...
                if item.value.len() >= 8 {
                    let balance = i64::from_le_bytes([
                        ...
                        item.value[4], item.value[5], item.value[6], item.value[7],
                    ]);
                    Ok(balance)
                } else {
                    Ok(0)
        ...
        for attribute in transaction.attributes() {
            if !self.attribute_allows_multiple(attribute) {
                ...
                if seen_types.contains(&attr_type) {
                    return Err(Error::Validation("Duplicate attribute not allowed".to_string()));
                }
    ...
    fn validate_single_attribute(&self, attribute: &neo_core::TransactionAttribute) -> Result<()> {
        match attribute {
            ...
            neo_core::TransactionAttribute::Conflicts { hash } => {
                if hash.is_zero() {
                    return Err(Error::Validation("Conflict hash cannot be zero".to_string()));
                }
                Ok(())
            }
        }
    ...
    fn validate_transaction_signers(&self, transaction: &Transaction) -> Result<()> {
        ...
        for signer in transaction.signers() {
            if seen_accounts.contains(&signer.account) {
                return Err(Error::Validation("Duplicate signer accounts not allowed".to_string()));
            }
    ...
    fn validate_signer_scope(&self, signer: &neo_core::Signer) -> Result<()> {
        ...
        if signer.scopes.has_flag(neo_core::WitnessScope::CalledByEntry) {
        ...
        if signer.scopes.has_flag(neo_core::WitnessScope::CustomContracts) {
        ...
        match condition {
            ...
            neo_core::WitnessCondition::And { conditions } => {
                ...
                if conditions.is_empty() {
                    return Err(Error::Validation("AND condition requires at least one condition".to_string()));
                }
            ...
            neo_core::WitnessCondition::Or { conditions } => {
                // Validate all conditions
                if conditions.is_empty() {
                    return Err(Error::Validation("OR condition requires at least one condition".to_string()));
                }
                for cond in conditions {
                    self.validate_witness_condition(cond)?;
                }
            ...
            neo_core::WitnessCondition::ScriptHash { hash } => {
                ...
                if hash.is_zero() {
                    return Err(Error::Validation("ScriptHash condition cannot have zero hash".to_string()));
                }
                Ok(())
            },
            neo_core::WitnessCondition::Group { group } => {
                ...
                if group.len() != 33 {
                    return Err(Error::Validation("Invalid group key length in condition".to_string()));
                }
                if group[0] != 0x02 && group[0] != 0x03 {
                    return Err(Error::Validation("Invalid group key format in condition".to_string()));
                }
                Ok(())
            },
            ...
            neo_core::WitnessCondition::CalledByContract { hash } => {
                ...
                if hash.is_zero() {
                    return Err(Error::Validation("CalledByContract condition cannot have zero hash".to_string()));
                }
                Ok(())
            },
            neo_core::WitnessCondition::CalledByGroup { group } => {
                // Validate group public key
                if group.len() != 33 {
                    return Err(Error::Validation("Invalid group key length in CalledByGroup condition".to_string()));
                }
                if group[0] != 0x02 && group[0] != 0x03 {
                    return Err(Error::Validation("Invalid group key format in CalledByGroup condition".to_string()));
                }
                Ok(())
            },
        }
    ...
    fn validate_transaction_script(&self, transaction: &Transaction) -> Result<()> {
        ...
        if transaction.script().is_empty() {
            return Err(Error::Validation("Transaction script cannot be empty".to_string()));
        }
        
        if transaction.script().len() > 65535 {
            return Err(Error::Validation("Transaction script too large".to_string()));
        }
    ...
    async fn validate_transaction_witnesses(&self, transaction: &Transaction) -> Result<()> {
        ...
        for (index, witness) in transaction.witnesses().iter().enumerate() {
            ...
            if witness.invocation_script.len() > 1024 || witness.verification_script.len() > 1024 {
                return Err(Error::Validation(format!("Witness {} script too large", index)));
            }
            ...
            if witness.invocation_script.is_empty() || witness.verification_script.is_empty() {
                return Err(Error::Validation(format!("Witness {} has empty scripts", index)));
            }
        }
    ...
    async fn validate_transaction_state_dependent(&self, transaction: &Transaction) -> Result<()> {
        ...
        if self.persistence.get_transaction(&tx_hash).await?.is_some() {
            return Err(Error::Validation("Transaction already exists in blockchain".to_string()));
        }
        ...
        for attribute in transaction.attributes() {
            if let neo_core::TransactionAttribute::Conflicts { hash } = attribute {
                ...
                if self.persistence.get_transaction(hash).await?.is_some() {
                    return Err(Error::Validation("Conflicting transaction exists".to_string()));
                }
            }
        }
    ...
    pub async fn get_committee(&mut self) -> Result<Vec<neo_cryptography::ECPoint>> {
        ...
        if committee.len() != 21 { // Neo N3 standard committee size
        ...
        println!("Committee retrieved: {} members at block {}", 
                committee.len(), self.get_current_block_index());
    ...
    fn get_default_committee(&self) -> Vec<neo_cryptography::ECPoint> {
        ...
        if let Ok(Some(stored_committee)) = self.get_stored_committee() {
            if !stored_committee.is_empty() {
                println!("Using stored committee ({} members)", stored_committee.len());
        ...
        if let Ok(calculated_committee) = self.calculate_committee_from_votes() {
            if !calculated_committee.is_empty() {
                println!("Using calculated committee from votes ({} members)", calculated_committee.len());
        ...
        match self.get_genesis_committee() {
            Ok(genesis_committee) => {
                println!("Using genesis committee fallback ({} members)", genesis_committee.len());
                genesis_committee
            }
            Err(_) => {
    ...
    fn calculate_committee_from_votes(&self) -> Result<Vec<neo_cryptography::ECPoint>> {
        ...
        sorted_candidates.sort_by(|a, b| {
            b.vote_count.cmp(&a.vote_count) // Descending order
        ...
        if committee.len() < committee_size {
            println!("Warning: Not enough candidates ({}/{}), padding with genesis keys", 
            ...
            for genesis_member in genesis_committee {
                ...
                if !padded_committee.iter().any(|member| member == &genesis_member) {
        ...
        println!("Committee calculated from {} candidates, selected top {}", 
                candidate_count, committee.len());
    ...
    fn get_contract_storage_item(&self, contract_hash: &UInt160, key: &[u8]) -> Result<Option<Vec<u8>>> {
        // Production-ready contract storage access (matches C# Store.TryGet exactly)
        // This would query RocksDB for contract storage in production
        ...
        let storage_key_string = format!("{}{}", hex::encode(contract_hash.as_bytes()), hex::encode(key));
    ...
    fn deserialize_committee_from_storage(&self, data: &[u8]) -> Result<Vec<neo_cryptography::ECPoint>> {
        ...
        if member_count > 21 || member_count == 0 {
            return Err(Error::InvalidData(format!("Invalid committee size: {}", member_count)));
        }
        ...
        for _ in 0..member_count {
            if offset + 33 > data.len() {
                return Err(Error::InvalidData("Incomplete committee member data".to_string()));
            }
    ...
    fn get_all_candidates_from_neo_contract(&self) -> Result<Vec<CandidateWithVotes>> {
        ...
        for i in 0..256 { // NEO supports up to 256 candidates
            let candidate_key = format!("candidate:{}", i);
            if let Ok(Some(data)) = self.get_contract_storage_item(&neo_contract_hash, candidate_key.as_bytes()) {
                if let Ok(candidate) = self.deserialize_candidate_from_storage(&data) {
                    candidates.push(candidate);
                }
            }
        ...
        println!("Retrieved {} candidates from NEO contract storage", candidates.len());
    ...
    fn deserialize_candidate_from_storage(&self, data: &[u8]) -> Result<CandidateWithVotes> {
        ...
        if data.len() < 33 + 8 { // Minimum: 33 bytes (ECPoint) + 8 bytes (u64 votes)
        ...
        let vote_count = u64::from_le_bytes([
            vote_bytes[0], vote_bytes[1], vote_bytes[2], vote_bytes[3],
            vote_bytes[4], vote_bytes[5], vote_bytes[6], vote_bytes[7]
        ]);
        
        // 3. Validate candidate data (production validation)
        if key_bytes[0] != 0x02 && key_bytes[0] != 0x03 {
            return Err(Error::InvalidData("Invalid candidate ECPoint format".to_string()));
        }
    ...
    fn validate_secp256r1_point(&self, key_bytes: &[u8]) -> Result<bool> {
        ...
        let p = num_bigint::BigInt::parse_bytes(
            b"fffffffffffffffffffffffffffffffeffffffffffffffff", 16
    ...
    fn try_get_current_height_from_persistence(&self) -> Result<u32> {
        ...
        // This would query RocksDB for current blockchain height in production
        ...
        if safe_height > 0 {
            println!("Estimated blockchain height: {} (based on time since genesis)", safe_height);
        }
    ...
    fn get_genesis_committee(&self) -> Result<Vec<neo_cryptography::ECPoint>> {
        ...
        for key_hex in genesis_committee_keys {
            let key_bytes = hex::decode(key_hex)
                .map_err(|_| Error::InvalidCommittee("Invalid genesis committee key".to_string()))?;
            let ec_point = ECPoint::from_bytes(&key_bytes)
                .map_err(|_| Error::InvalidCommittee("Invalid ECPoint in genesis committee".to_string()))?;
            committee.push(ec_point);
        }
...
mod tests {
    use super::*;
    use crate::blockchain::storage::{Storage, RocksDBStorage};
