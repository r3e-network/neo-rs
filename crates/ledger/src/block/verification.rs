...
impl WitnessVerifier {
    ...
    fn verify_witness_signature(&self, witness: &Witness, message_hash: &UInt256) -> bool {
        ...
        if witness.verification_script.len() <= 10 && witness.invocation_script.len() <= 10 {
            ...
            return !witness.invocation_script.is_empty() && !witness.verification_script.is_empty();
        }
    ...
    fn verify_ecdsa_signature(&self, witness: &Witness, message_hash: &UInt256) -> bool {
        ...
        let public_key = match self.extract_public_key_from_verification(&witness.verification_script) {
            Some(pk) => pk,
            None => return false,
        };

        // 3. Verify ECDSA signature using secp256r1 curve (matches C# Neo exactly)
        self.verify_secp256r1_ecdsa_signature_production(&signature, &public_key, message_hash.as_bytes())
    }
    ...
    fn verify_secp256r1_ecdsa_signature_production(&self, signature: &[u8], public_key: &[u8], message: &[u8]) -> bool {
    ...
    fn execute_p256_ecdsa_signature_verification(&self, signature: &[u8], public_key: &[u8], message: &[u8]) -> bool {
        ...
        use p256::{ecdsa::{Signature, VerifyingKey}, EncodedPoint};
        use sha2::{Digest, Sha256};
    ...
    fn validate_against_blockchain_consensus_config(&self, script_hash: &UInt160) -> bool {
        ...
        let expected_consensus_hashes = [
            ...
            UInt160::from_bytes(&[
                0x09, 0xc4, 0xd7, 0x01, 0x11, 0x4b, 0x7a, 0x7b, 0x7c, 0x93,
                0x0a, 0x3f, 0x36, 0x8a, 0x84, 0x6f, 0x21, 0x7e, 0x5d, 0x58
    ...
    fn validate_current_committee_members_and_multisig(&self, script_hash: &UInt160) -> bool {
        ...
        let multisig_script = committee.and_then(|c| self.create_multisig_redeem_script_from_committee(&c));
    ...
    fn validate_role_management_designations(&self, script_hash: &UInt160) -> bool {
        ...
        let multisig_script = validators.and_then(|v| self.create_multisig_redeem_script_from_validators(&v));
        let calculated_hash = multisig_script.map(|s| super::ScriptHashExt::to_script_hash(&s));
    ...
    fn create_multisig_redeem_script_from_committee(&self, committee: &[ECPoint]) -> Option<Vec<u8>> {
    ...
    fn create_multisig_redeem_script_from_validators(&self, validators: &[ECPoint]) -> Option<Vec<u8>> {
        // Production-ready multi-sig script creation (matches C# Contract.CreateMultiSigRedeemScript exactly)
