...
use neo_core::{Transaction, TransactionAttribute, Signer, Witness, WitnessScope, UInt160, UInt256, Block, BlockHeader};
...
mod tests {
    ...
    fn test_transaction_sorting_by_fee_per_byte() {
        ...
        for i in 1..sorted_txs.len() {
            let prev_fee_per_byte = sorted_txs[i-1].network_fee() as f64 / sorted_txs[i-1].size() as f64;
            let curr_fee_per_byte = sorted_txs[i].network_fee() as f64 / sorted_txs[i].size() as f64;
