...
fn test_consensus_config_csharp_compatibility() {
    ...
    assert_eq!(config.validator_count, 7, "Validator count must match C# Neo default");
    assert_eq!(config.block_time_ms, 15000, "Block time must match C# Neo (15 seconds)");
    assert_eq!(config.view_timeout_ms, 20000, "View timeout must match C# Neo (20 seconds)");
    assert_eq!(config.max_view_changes, 6, "Max view changes must match C# Neo");
    assert_eq!(config.max_block_size, 1024 * 1024, "Max block size must match C# Neo (1MB)");
    assert_eq!(config.max_transactions_per_block, 512, "Max transactions must match C# Neo");
    
    // Test Byzantine fault tolerance calculation (same as C# Neo)
    assert_eq!(config.byzantine_threshold(), 2, "Byzantine threshold must be (N-1)/3");
    assert_eq!(config.required_signatures(), 5, "Required signatures must be N - f");
...
fn test_view_number_csharp_compatibility() {
    ...
    max_view.increment();
    assert_eq!(max_view.value(), 0, "View number must wrap at 255 like C# Neo");
...
fn test_prepare_request_validation_csharp_compatibility() {
    ...
    assert!(valid_request.validate().is_ok(), "Valid prepare request should pass");
    ...
    assert!(invalid_request.validate().is_err(), "Empty block data should fail validation");
    ...
    assert!(large_request.validate().is_err(), "Oversized block should fail validation");
    ...
    assert!(no_tx_request.validate().is_err(), "Block with no transactions should fail");
    ...
    assert!(many_tx_request.validate().is_err(), "Too many transactions should fail");
...
fn test_signature_threshold_csharp_compatibility() {
    let config_4 = ConsensusConfig { validator_count: 4, ..Default::default() };
    let config_7 = ConsensusConfig { validator_count: 7, ..Default::default() };
    let config_10 = ConsensusConfig { validator_count: 10, ..Default::default() };
