...
impl RecoveryManager {
    ...
    pub async fn start_recovery(
        ...
        if self.sessions.read().len() >= self.config.max_concurrent_recoveries {
            return Err(Error::Recovery("Too many concurrent recovery sessions".to_string()));
        }
        ...
        info!(
            ...
            session_id, block_index.value(), view_number.value(), reason
        );
    ...
    pub async fn handle_recovery_response(
        ...
        if let Some(session) = sessions.get_mut(session_id) {
            ...
            // Store response
            session.responses_received.insert(responder, response.clone());

            // Check if we have enough responses to proceed
            let validator_set = self.context.get_validator_set()
            ...
            if session.responses_received.len() >= required_responses {
                ...
                self.process_recovery_responses(session_id, &response).await?;
            }
        }
    ...
    pub async fn start_auto_recovery(&self) {
        ...
        let timer = interval(Duration::from_millis(self.config.recovery_retry_interval_ms));
    ...
    async fn execute_recovery(&self, session_id: &str) -> Result<()> {
        ...
        for validator in &validator_set.validators {
            ...
            self.sessions.write().get_mut(session_id)
                .unwrap()
                .validators_contacted.push(validator.public_key_hash);
        }
    ...
    async fn process_recovery_responses(
        ...
        if recovered_view <= self.context.get_current_round().view_number {
            return Err(Error::InvalidRecoveryMessage("Recovery view too old".to_string()));
        }
        
        if recovered_block_index != self.context.get_current_round().block_index {
            return Err(Error::InvalidRecoveryMessage("Recovery block index mismatch".to_string()));
        }
        ...
        self.context.get_current_round_mut().block_index = recovered_block_index;
        self.context.get_current_round_mut().view_number = recovered_view;
        self.context.get_current_round_mut().timestamp = recovered_timestamp;
        ...
        println!("dBFT recovery completed: view {} -> {}, block {}", 
                self.context.get_current_round().view_number - 1, self.context.get_current_round().view_number, recovered_block_index);
    ...
    pub fn cleanup_old_sessions(&self) {
        ...
        sessions.retain(|_, session| {
            now - session.started_at < history_limit
        });
    }
    ...
    fn mark_recovery_session_completed(&mut self, session_id: &str) -> Result<()> {
        ...
        if session.status != RecoveryStatus::InProgress {
            return Err(Error::InvalidRecoverySession("Session not in progress".to_string()));
        }
        ...
        // 4. Update recovery statistics (production metrics)
        {
            ...
            stats.avg_recovery_time_ms = 
                (stats.avg_recovery_time_ms * (total_completed - 1.0) + session_duration as f64 * 1000.0) / total_completed;
        }
        
        // 5. Log completion for monitoring (production logging)
        info!("Recovery session {} completed in {} seconds", session_id, session_duration);
...
mod tests {
    ...
    async fn test_recovery_manager() {
        ...
        let session_id = manager.start_recovery(
            BlockIndex::new(100),
            ViewNumber::new(1),
    ...
    async fn test_recovery_request_response() {
        ...
        let response = manager.handle_recovery_request(request, requester).await.unwrap();
