...
impl SafeConsensusState {
    ...
    pub fn new(node_index: u32, node_count: u32) -> ConsensusResult<Self> {
        ...
        if node_index >= node_count {
            return Err(ConsensusError::InvalidConfig(
                format!("Node index {} >= node count {}", node_index, node_count)
            ));
        }
    ...
    pub fn change_view(&mut self, new_view: u32) -> ConsensusResult<()> {
        ...
        if new_view < self.view {
            return Err(ConsensusError::InvalidView(
                format!("Cannot decrease view from {} to {}", self.view, new_view)
            ));
        }
        ...
        if new_view > self.view + MAX_VIEW_JUMP {
            return Err(ConsensusError::InvalidView(
                format!("View jump too large: {} -> {}", self.view, new_view)
            ));
        }
    ...
    pub fn record_error(&mut self) -> ConsensusResult<()> {
        ...
        if self.error_count > MAX_ERRORS {
            return Err(ConsensusError::CircuitBreakerOpen(
                format!("Too many errors: {}", self.error_count)
            ));
        }
    ...
    pub fn validate_proposal(
        ...
        if proposal_view < current_view {
            return Err(ConsensusError::InvalidProposal(
                format!("Proposal view {} < current view {}", proposal_view, current_view)
            ));
        }
        ...
        if proposal_height != current_height {
            return Err(ConsensusError::InvalidProposal(
                format!("Proposal height {} != current height {}", 
    ...
    pub fn validate_vote(
        ...
        if vote_view != current_view {
            return Err(ConsensusError::InvalidVote(
                format!("Vote view {} != current view {}", vote_view, current_view)
            ));
        }
        
        if vote_height != current_height {
            return Err(ConsensusError::InvalidVote(
                format!("Vote height {} != current height {}", vote_height, current_height)
            ));
        }
    ...
    pub fn attempt_recovery(&mut self) -> ConsensusResult<()> {
        ...
        if self.recovery_attempts >= self.max_attempts {
            return Err(ConsensusError::RecoveryFailed(
                format!("Exceeded maximum recovery attempts: {}", self.max_attempts)
            ));
        }
...
mod tests {
    ...
    fn test_timeout_manager() {
        let mut manager = SafeTimeoutManager::new(
