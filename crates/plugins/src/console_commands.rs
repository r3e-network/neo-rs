...
impl ConsoleCommandHandler for ExportBlocksCommand {
    async fn execute(
        ...
        let start = args.get(0).and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
        ...
        Ok(format!(
            "Exporting {} blocks starting from {}",
            count, start
        ))
    }
...
impl ConsoleCommandHandler for CreateWalletCommand {
    async fn execute(
        ...
        let wallet_path = args.get(0).ok_or("Missing wallet path parameter")?;
