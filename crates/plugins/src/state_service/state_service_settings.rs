    ...
    pub fn load(config: &serde_json::Value) -> Self {
        ...
        Self {
            path: plugin_config["Path"]
                ...
                .to_string(),
            full_state: plugin_config["FullState"].as_bool().unwrap_or(false),
            network: plugin_config["Network"].as_u64().unwrap_or(860833102) as u32,
            auto_verify: plugin_config["AutoVerify"].as_bool().unwrap_or(false),
            max_find_result_items: plugin_config["MaxFindResultItems"].as_i64().unwrap_or(100)
                as i32,
