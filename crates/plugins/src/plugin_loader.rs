...
impl PluginLoader {
    ...
    /// Returns the base directory used by the plugin loader
    ...
    pub async fn load_plugins(&mut self) -> ExtensionResult<()> {
        ...
        if !plugins_dir.exists() {
            tokio::fs::create_dir_all(&plugins_dir)
                .await
                .map_err(ExtensionError::IoError)?;
        }
    ...
    async fn load_configurations(&mut self) -> ExtensionResult<()> {
        ...
        if config_path.exists() {
            ...
            let config: serde_json::Value = serde_json::from_str(&config_content)
                .map_err(|e| ExtensionError::invalid_config(e.to_string()))?;
    ...
    async fn initialize_plugins(&mut self) -> ExtensionResult<()> {
        ...
        for (plugin_name, plugin) in plugins.iter_mut() {
            ...
            // Create plugin context aligned with neo-extensions
            let context = neo_extensions::plugin::PluginContext {
                neo_version: "3.6.0".to_string(),
                config_dir: self.base_directory.join("Config"),
                data_dir: self.base_directory.join("Data"),
