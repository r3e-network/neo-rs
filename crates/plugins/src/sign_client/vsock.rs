...
impl Vsock {
    ...
    pub async fn connect_async(&self, context: &SocketsHttpConnectionContext, cancellation: &tokio::time::Duration) -> Result<Box<dyn std::io::Read + Send + Sync>, VsockError> {
        if !cfg!(target_os = "linux") {
    ...
    pub fn create_channel(address: VsockAddress, service_config: ServiceConfig) -> Arc<dyn GrpcChannel> {
        ...
        Arc::new(VsockGrpcChannel::new(address_placeholder, service_config, sockets_http_handler))
    }
}
...
pub struct SocketsHttpHandler {
    pub connect_callback: Box<dyn Fn(&SocketsHttpConnectionContext, &tokio::time::Duration) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Box<dyn std::io::Read + Send + Sync>, VsockError>> + Send + Sync>> + Send + Sync>,
}
...
impl VsockGrpcChannel {
    pub fn new(address: String, service_config: ServiceConfig, http_handler: SocketsHttpHandler) -> Self {
        Self {
            address,
            service_config,
            http_handler,
