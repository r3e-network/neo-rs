...
impl LogStorageStore {
    ...
    pub fn put_engine_state(&mut self, state: &EngineLogState) -> IoResult<Uuid> {
        let id = Uuid::new_v4();
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_ENGINE)
    ...
    pub fn put_transaction_engine_state(
        ...
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_ENGINE_TRANSACTION)
            .add_uint256(hash)
            .into_vec();
        let value = serialize_serializable(state)?;
        self.write_raw(key, value)
    ...
    pub fn put_block_state(
        ...
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_BLOCK)
            .add_uint256(hash)
            .add_u8(trigger.bits())
            .into_vec();
        let value = serialize_serializable(state)?;
    ...
    pub fn put_notify_state(&mut self, state: &NotifyLogState) -> IoResult<Uuid> {
        let id = Uuid::new_v4();
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_NOTIFY)
            .add_bytes(&id.to_bytes_le())
            .into_vec();
        let value = serialize_serializable(state)?;
        self.write_raw(key, value)?;
    ...
    pub fn put_contract_state(
        ...
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_CONTRACT)
    ...
    pub fn put_execution_state(&mut self, state: &ExecutionLogState) -> IoResult<Uuid> {
        let id = Uuid::new_v4();
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_EXECUTION)
            .add_bytes(&id.to_bytes_le())
            .into_vec();
        let value = serialize_serializable(state)?;
        self.write_raw(key, value)?;
    ...
    pub fn put_execution_block_state(
        ...
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_EXECUTION_BLOCK)
    ...
    pub fn put_transaction_state(
        ...
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_TRANSACTION)
            .add_uint256(hash)
            .into_vec();
        let value = serialize_serializable(state)?;
        self.write_raw(key, value)
    ...
    pub fn put_stack_item_state(&mut self, item: &StackItem) -> IoResult<Uuid> {
        let id = Uuid::new_v4();
        let key = KeyBuilder::new(PREFIX_ID, PREFIX_STACK_ITEM)
            .add_bytes(&id.to_bytes_le())
            .into_vec();
    ...
    fn find_contract_state_internal(
        ...
        let base_prefix = KeyBuilder::new(PREFIX_ID, PREFIX_CONTRACT)
            .add_uint160(script_hash)
            .into_vec();

        let seek_bytes = KeyBuilder::new(PREFIX_ID, PREFIX_CONTRACT)
            .add_uint160(script_hash)
            .add_be_u64(u64::MAX)
            .into_vec();
