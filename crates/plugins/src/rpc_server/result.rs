    ...
    E: Error + 'static,
{
    match function() {
        ...
        Ok(None) => Err(RpcException::new(err)),
        Err(e) => {
            if with_data {
                Err(RpcException::new(err.with_data(e.to_string())))
            } else {
                Err(RpcException::new(err))
            }
        }
    }
}
    ...
    E: Error + 'static,
{
    match function() {
        ...
        Ok(_) | Err(_) => Err(RpcException::new(err)),
    }
}
...
impl<T> OptionRpcExt<T> for Option<T> {
    fn not_null_or(self, err: RpcError) -> Result<T, RpcException> {
        self.ok_or_else(|| RpcException::new(err))
    }
