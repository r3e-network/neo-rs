...
impl RpcServerWallet {
    ...
    fn dump_priv_key(server: &RpcServer, params: &[Value]) -> Result<Value, RpcException> {
        ...
        let account = wallet.get_account(&script_hash).ok_or_else(|| {
            RpcException::new(RpcError::unknown_account().with_data(script_hash.to_string()))
        })?;
        if !account.has_key() {
    ...
    fn open_wallet(server: &RpcServer, params: &[Value]) -> Result<Value, RpcException> {
        ...
        if !Path::new(&path).exists() {
            return Err(RpcException::new(RpcError::wallet_not_found()));
        }
    ...
    fn calculate_network_fee(server: &RpcServer, params: &[Value]) -> Result<Value, RpcException> {
        ...
        let raw = BASE64_STANDARD.decode(payload.trim()).map_err(|_| {
            RpcException::new(RpcError::invalid_params().with_data("Invalid transaction payload"))
        })?;
    ...
    fn wallet_failure(err: WalletError) -> RpcException {
        match err {
            WalletError::InvalidPassword => {
                RpcException::new(RpcError::wallet_not_supported().with_data("Invalid password."))
            }
            WalletError::WalletFileNotFound(path) => {
                RpcException::new(RpcError::wallet_not_found().with_data(path))
            }
            WalletError::AccountNotFound(hash) => {
                RpcException::new(RpcError::unknown_account().with_data(format!("{}", hash)))
            }
            WalletError::InsufficientFunds => {
                RpcException::new(RpcError::insufficient_funds_wallet())
            }
            WalletError::Io(err) => {
                RpcException::new(RpcError::internal_server_error().with_data(err.to_string()))
            }
            other => {
                RpcException::new(RpcError::wallet_not_supported().with_data(other.to_string()))
            }
        }
    }
    ...
    fn require_wallet(server: &RpcServer) -> Result<Arc<dyn CoreWallet>, RpcException> {
        server
            .wallet()
            .ok_or_else(|| RpcException::new(RpcError::no_opened_wallet()))
    }
    ...
    fn build_and_relay(
        ...
        if tx.network_fee() > server.settings().max_fee {
            return Err(RpcException::new(RpcError::wallet_fee_limit()));
        }
