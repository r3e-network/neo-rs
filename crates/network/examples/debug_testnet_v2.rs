...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        match stream.read(&mut buffer) {
            ...
            Ok(n) => {
                println!("\nReceived {} bytes at +{}ms:", n, start_time.elapsed().as_millis());
                let chunk = &buffer[..n];
...
fn analyze_chunk(chunk: &[u8], offset: usize) {
    ...
    for i in 0..chunk.len() {
        ...
        if i + 4 <= chunk.len() && &chunk[i..i+4] == b"N3T5" {
            println!("  Found 'N3T5' at chunk offset {} (absolute offset {})", i, offset + i);
        }
        ...
        if chunk[i] == b'/' {
            if i > 0 && i + chunk[i-1] as usize <= chunk.len() {
                ...
                if agent.iter().all(|&b| b.is_ascii_graphic() || b == b' ') {
                    println!("  Found user agent at chunk offset {}: '{}'", i-1, 
...
fn analyze_complete_response(data: &[u8]) {
    ...
    if data.len() >= 4 {
        println!("First 4 bytes as u32 (LE): {}", u32::from_le_bytes([data[0], data[1], data[2], data[3]]));
    }
    ...
    if data.len() >= 8 && &data[4..8] == b"N3T5" {
        ...
        if data.len() >= 40 {
            ...
            println!("  Version: {}", u32::from_le_bytes([data[offset], data[offset+1], data[offset+2], data[offset+3]]));
            offset += 4;
            
            println!("  Network: {}", String::from_utf8_lossy(&data[offset..offset+4]));
            offset += 4;
            
            println!("  Unknown field: 0x{:08x}", u32::from_le_bytes([data[offset], data[offset+1], data[offset+2], data[offset+3]]));
            offset += 4;
            
            println!("  Timestamp: {}", u32::from_le_bytes([data[offset], data[offset+1], data[offset+2], data[offset+3]]));
            offset += 4;
            
            println!("  Nonce: {}", u32::from_le_bytes([data[offset], data[offset+1], data[offset+2], data[offset+3]]));
            offset += 4;
            ...
            if offset < data.len() {
                ...
                if offset + agent_len <= data.len() {
                    println!("  User Agent: '{}'", String::from_utf8_lossy(&data[offset..offset+agent_len]));
                    offset += agent_len;
                }
            }
            
            if offset < data.len() {
                println!("  Remaining bytes from offset {}: {:?}", offset, &data[offset..]);
            }
        }
    }
}
