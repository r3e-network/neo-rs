...
use crate::{Error, Result, ProtocolMessage, NetworkMessage};
...
impl MessageValidator {
    ...
    fn serialize_payload(&self, payload: &ProtocolMessage) -> Result<Vec<u8>> {
        payload.to_bytes().map_err(|e| Error::Serialization(e.to_string()))
    }
    ...
    fn validate_payload(&self, payload: &ProtocolMessage) -> Result<()> {
        match payload {
            ProtocolMessage::Version { 
                ...
                self.validate_version_message(*version, *services, *timestamp, *port, *nonce, user_agent, *start_height, *relay)
            },
            ...
            ProtocolMessage::Addr { addresses } => {
                self.validate_addr_message(addresses)
            },
            
            ProtocolMessage::Ping { nonce } => {
                self.validate_ping_message(*nonce)
            },
            
            ProtocolMessage::Pong { nonce } => {
                self.validate_pong_message(*nonce)
            },
            ...
            ProtocolMessage::Headers { headers } => {
                self.validate_headers_message(headers)
            },
            ...
            ProtocolMessage::Tx { transaction } => {
                self.validate_transaction_message(transaction)
            },
            
            ProtocolMessage::Block { block } => {
                self.validate_block_message(block)
            },
            
            ProtocolMessage::Inv { inventory } => {
                self.validate_inventory_message(inventory, "inv")
            },
    ...
    fn validate_version_message(
        ...
        if port == 0 {
            return Err(Error::InvalidMessage("Version message has invalid port 0".to_string()));
        }
        ...
        if !user_agent.chars().all(|c| c.is_ascii() && !c.is_control()) {
            return Err(Error::InvalidMessage("User agent contains invalid characters".to_string()));
        }
        ...
        if nonce == 0 {
            return Err(Error::InvalidMessage("Version nonce cannot be zero".to_string()));
        }

        debug!("Version message validation passed: version={}, start_height={}, user_agent='{}'", 
    ...
    fn validate_addr_message(&self, addresses: &[SocketAddr]) -> Result<()> {
        ...
        for addr in addresses {
            ...
            if self.magic == 0x334f454e { // MainNet magic
    ...
    fn validate_ping_message(&self, nonce: u32) -> Result<()> {
        if nonce == 0 {
            return Err(Error::InvalidMessage("Ping nonce cannot be zero".to_string()));
        }
        Ok(())
    }
    ...
    fn validate_pong_message(&self, nonce: u32) -> Result<()> {
        if nonce == 0 {
            return Err(Error::InvalidMessage("Pong nonce cannot be zero".to_string()));
        }
        Ok(())
    }

    /// Validates get headers message
    fn validate_get_headers_message(&self, hash_start: &[UInt256], hash_stop: &UInt256) -> Result<()> {
        if hash_start.is_empty() {
            return Err(Error::InvalidMessage("GetHeaders must have at least one start hash".to_string()));
        }
        ...
        if all_zero && !hash_stop.is_zero() {
            return Err(Error::InvalidMessage("Invalid GetHeaders: all start hashes are zero but stop hash is not".to_string()));
        }
    ...
    fn validate_headers_message(&self, headers: &[BlockHeader]) -> Result<()> {
        ...
        for (i, header) in headers.iter().enumerate() {
            self.validate_block_header(header).map_err(|e| {
                Error::InvalidMessage(format!("Invalid header at index {}: {}", i, e))
            })?;
        }
    ...
    fn validate_get_blocks_message(&self, hash_start: &[UInt256], hash_stop: &UInt256) -> Result<()> {
    ...
    fn validate_get_block_by_index_message(&self, index_start: u32, count: u16) -> Result<()> {
        if count == 0 {
            return Err(Error::InvalidMessage("GetBlockByIndex count cannot be zero".to_string()));
        }

        if count > 500 {
            return Err(Error::InvalidMessage(format!(
                "GetBlockByIndex count too large: {} (max 500)",
                count
            )));
        }
    ...
    fn validate_transaction_message(&self, transaction: &Transaction) -> Result<()> {
        ...
        if tx_size > 102400 { // 100KB max transaction size
            return Err(Error::InvalidMessage(format!(
                "Transaction size {} exceeds maximum 102400 bytes",
                tx_size
            )));
        }
        ...
        if transaction.system_fee() < 0 || transaction.network_fee() < 0 {
            return Err(Error::InvalidMessage("Transaction fees cannot be negative".to_string()));
        }

        // 4. Validate script is not empty
        if transaction.script().is_empty() {
            return Err(Error::InvalidMessage("Transaction script cannot be empty".to_string()));
        }
        ...
        if transaction.witnesses().is_empty() {
            return Err(Error::InvalidMessage("Transaction must have at least one witness".to_string()));
        }
    ...
    fn validate_block_message(&self, block: &Block) -> Result<()> {
        ...
        if block.transactions.is_empty() {
            return Err(Error::InvalidMessage("Block cannot have zero transactions".to_string()));
        }
        ...
        if block_size > 1_048_576 { // 1MB max block size
            return Err(Error::InvalidMessage(format!(
                "Block size {} exceeds maximum 1MB",
                block_size
            )));
        }
        ...
        for (i, tx) in block.transactions.iter().enumerate() {
            self.validate_transaction_message(tx).map_err(|e| {
                Error::InvalidMessage(format!("Invalid transaction {} in block: {}", i, e))
            })?;
        }
        ...
        if calculated_merkle != block.header.merkle_root {
            return Err(Error::InvalidMessage("Block merkle root mismatch".to_string()));
        }

        debug!("Block message validation passed for block {}", block.index());
        Ok(())
    }

    /// Validates inventory message (inv, getdata, notfound)
    fn validate_inventory_message(&self, inventory: &[crate::InventoryItem], msg_type: &str) -> Result<()> {
    ...
    fn validate_block_header(&self, header: &BlockHeader) -> Result<()> {
        ...
        if header.merkle_root.is_zero() {
            return Err(Error::InvalidMessage("Block merkle root cannot be zero".to_string()));
        }

        // 3. Validate next consensus is not zero
        if header.next_consensus.is_zero() {
            return Err(Error::InvalidMessage("Block next consensus cannot be zero".to_string()));
        }

        // 4. Validate witnesses exist
        if header.witnesses.is_empty() {
            return Err(Error::InvalidMessage("Block header must have at least one witness".to_string()));
        }
        ...
        if header.index > 0 && header.previous_hash.is_zero() {
            return Err(Error::InvalidMessage("Non-genesis block cannot have zero previous hash".to_string()));
        }

        // 7. For genesis block, validate previous hash is zero
        if header.index == 0 && !header.previous_hash.is_zero() {
            return Err(Error::InvalidMessage("Genesis block must have zero previous hash".to_string()));
        }
...
mod tests {
    use super::*;
    use crate::{NetworkMessage, MessageCommand};
    use neo_core::{UInt256, BlockHeader, Witness, Transaction};
    ...
    fn create_test_block_header(index: u32) -> BlockHeader {
        BlockHeader {
            version: 0,
            previous_hash: if index == 0 { UInt256::zero() } else { UInt256::new([1; 32]) },
    ...
    fn test_addr_message_validation() {
        ...
        let invalid_addr = ProtocolMessage::Addr { addresses: many_addresses };
    ...
    fn test_verack_getaddr_validation() {
        ...
        assert!(validator.validate_payload(&ProtocolMessage::GetAddr).is_ok());
    }
    ...
    fn test_get_headers_message_validation() {
        ...
        let get_headers_msg = ProtocolMessage::GetHeaders { hash_start, hash_stop };
    ...
    fn test_headers_message_validation() {
        ...
        let invalid_headers_msg = ProtocolMessage::Headers { headers: many_headers };
        ...
        let invalid_headers_msg = ProtocolMessage::Headers { headers: vec![invalid_header] };
        assert!(validator.validate_payload(&invalid_headers_msg).is_err());
    }
    ...
    fn test_get_blocks_message_validation() {
        ...
        let hash_start = vec![UInt256::new([1; 32])];
        let hash_stop = UInt256::new([2; 32]);
        let get_blocks_msg = ProtocolMessage::GetBlocks { hash_start, hash_stop };
    ...
    fn test_get_block_by_index_message_validation() {
        ...
        let get_block_msg = ProtocolMessage::GetBlockByIndex { index_start: 0, count: 500 };
        assert!(validator.validate_payload(&get_block_msg).is_ok());

        // Invalid - count too high
        let invalid_count = ProtocolMessage::GetBlockByIndex { index_start: 0, count: 501 };
        assert!(validator.validate_payload(&invalid_count).is_err());

        // Invalid - index_start too high
        let invalid_index = ProtocolMessage::GetBlockByIndex { index_start: 1001, count: 1 };
        assert!(validator.validate_payload(&invalid_index).is_err());

        // Valid - zero count (allowed)
        let zero_count = ProtocolMessage::GetBlockByIndex { index_start: 0, count: 0 };
    ...
    fn test_block_message_validation() {
        ...
        let genesis_msg = ProtocolMessage::Block { block: genesis_block };
        assert!(validator.validate_payload(&genesis_msg).is_ok());

        // Invalid - block index too high
        let invalid_block = create_test_block(1001);
        let invalid_block_msg = ProtocolMessage::Block { block: invalid_block };
    ...
    fn test_inventory_message_validation() {
        ...
        let many_hashes: Vec<UInt256> = (0..50001).map(|i| {
