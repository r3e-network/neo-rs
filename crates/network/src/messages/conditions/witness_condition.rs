...
//!
//! The canonical implementation lives in `neo_core::WitnessCondition`; this module re-exports the
//! type and exposes the same constants as the C# hierarchy so the network crate can stay aligned
//! with the original layout.

pub use neo_core::WitnessCondition;
...
pub const MAX_SUBITEMS: usize = neo_core::WitnessCondition::MAX_SUBITEMS;
/// Maximum nesting depth for composite conditions (matches C# `MaxNestingDepth`).
pub const MAX_NESTING_DEPTH: usize = neo_core::WitnessCondition::MAX_NESTING_DEPTH;
