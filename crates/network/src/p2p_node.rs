...
use crate::{Error, Result, NetworkConfig, PeerManager, MessageHandler, NetworkMessage, NetworkCommand};
...
impl P2pNode {
    ...
    pub async fn send_message_to_peer(
        ...
        if !self.peers.read().await.contains_key(&peer) {
            return Err(Error::PeerNotConnected(peer.to_string()));
        }
        
        // 2. Send message through peer manager
        self.peer_manager.send_message(peer, message.clone()).await?;
        ...
        let _ = self.event_sender.send(NodeEvent::MessageSent {
            peer,
            message,
        });
    ...
    async fn handle_command(&self, command: NetworkCommand) -> Result<()> {
        match command {
            ...
            NetworkCommand::DisconnectPeer(address) => {
    ...
    async fn start_websocket_listener(&self) -> Result<()> {
        info!("Starting WebSocket listener on port {}", self.config.websocket_port);
    ...
    async fn connect_to_seed_nodes(&self) -> Result<()> {
        info!("🌐 Starting connection to {} Neo N3 seed nodes", self.config.seed_nodes.len());
        
        let mut successful_connections = 0;
        let mut failed_connections = 0;
        
        for (index, seed_node) in self.config.seed_nodes.iter().enumerate() {
            info!("📡 Attempting to connect to seed node #{} at {}", index + 1, seed_node);
        ...
        if successful_connections == 0 {
            error!("⚠️  No seed nodes could be reached! The node may not be able to sync with the Neo N3 network.");
        } else {
            info!("🎉 Connected to {} Neo N3 network peers", successful_connections);
        }
    ...
    async fn start_periodic_tasks(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if peer_count < 3 {
                    warn!("⚠️  Low peer count: {} peers connected. Attempting to find more peers...", peer_count);
...
mod tests {
    use super::*;
    use crate::{NetworkMessage, MessageCommand, ProtocolMessage};
    ...
    fn test_node_statistics_serialization() {
        ...
        assert_eq!(stats.outbound_connections, deserialized.outbound_connections);
    ...
    async fn test_send_message_to_peer() {
        ...
        let send_result = node.send_message_to_peer(peer_address, message.clone()).await;
    ...
    async fn test_node_statistics_updates_with_peers() {
        ...
        node.peers.write().await.insert(outbound_peer, outbound_info);
