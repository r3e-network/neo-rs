...
impl DosProtectionManager {
    ...
    pub fn new(config: DosProtectionConfig) -> Self {
        Self {
            ...
            connection_rate_limiter: Arc::new(RwLock::new(
                RateLimiter::new(config.connection_rate_limit, Duration::from_secs(60))
            )),
        }
    }
    ...
    pub async fn should_allow_connection(&self, ip: IpAddr) -> bool {
        ...
        if peer_stat.connection_count >= self.config.max_connections_per_ip {
            warn!("Connection limit exceeded for IP {}: {}", ip, peer_stat.connection_count);
    ...
    pub async fn register_connection(&self, ip: IpAddr) {
        if !self.is_whitelisted(&ip) {
            ...
            *global_count += 1;
            ...
            debug!("Connection registered for IP {}: count = {}", ip, peer_stat.connection_count);
        }
    }
        ...
        if !self.is_whitelisted(&ip) {
            ...
            if let Some(peer_stat) = stats.get_mut(&ip) {
                if peer_stat.connection_count > 0 {
                    ...
                    debug!("Connection unregistered for IP {}: count = {}", ip, peer_stat.connection_count);
    ...
    pub async fn should_allow_message(&self, ip: IpAddr, message_size: usize) -> bool {
        ...
        if message_size > self.config.max_message_size {
            warn!("Message from {} exceeds size limit: {} bytes", ip, message_size);
        ...
        if peer_stat.message_count >= self.config.max_messages_per_second {
            warn!("Message rate limit exceeded for IP {}: {} msg/s", 
    ...
    pub async fn get_stats(&self) -> DosProtectionStats {
        ...
        let banned_count = stats.values()
            .filter(|s| s.ban_until.is_some())
            .count();
        
        let active_peers = stats.values()
