...
use crate::{PeerManager, P2pNode, SyncManager, Error, Result};
...
impl Shutdown for SyncManager {
    ...
    async fn shutdown(&self) -> std::result::Result<(), ShutdownError> {
        ...
        info!("Final sync status: height={}/{}, state={}", 
              final_status.current_height,
              final_status.best_known_height,
              final_status.state);
...
mod tests {
    ...
    async fn test_shutdown_coordinator_integration() {
        ...
        let is_running = std::sync::Arc::new(tokio::sync::RwLock::new(true));
        let rpc_shutdown = std::sync::Arc::new(RpcServerShutdown::new(is_running.clone(), "test".to_string()));
