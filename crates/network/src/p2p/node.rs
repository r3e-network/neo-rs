...
impl P2PNode {
    ...
    pub async fn stop(&self) {
        ...
        *self.running.write().await = false;
        ...
        for address in connections.keys() {
            self.disconnect_peer(*address, "Node shutting down".to_string())
                .await;
        }
    ...
    pub async fn send_get_data(
        &self,
        address: SocketAddr,
        inventory: Vec<crate::InventoryItem>,
    ) -> Result<()> {
    ...
    pub async fn broadcast_inventory(
        ...
        info!(
            "Broadcasted inventory to {}/{} peers",
            success_count, total_count
        );
        Ok(())
    }
    ...
    async fn start_handshake(&self, address: SocketAddr) -> Result<()> {
        ...
        let version_message =
            ProtocolMessage::version(&self.node_info, self.config.listen_address.port(), true);
    ...
    pub async fn handle_message(&self, address: SocketAddr, message: NetworkMessage) -> Result<()> {
        ...
        match &message.payload {
            ProtocolMessage::Version {
                ...
                self.handle_version_message(address, *version, *nonce, user_agent, *start_height)
                    .await?;
            }
            ...
            _ => {
                ...
                self.handle_through_registered_handlers(address, &message)
                    .await?;
            }
        }
    ...
    async fn handle_version_message(
        ...
        info!(
            "Received Version from {}: v{}, user_agent={}, height={}",
    ...
    async fn handle_through_registered_handlers(
        &self,
        address: SocketAddr,
