...
use crate::{Error, NetworkMessage, Result};
...
pub trait MessageHandler: Send + Sync {
    ...
    async fn handle_message(&self, peer_address: SocketAddr, message: &NetworkMessage) -> Result<()>;
}
...
impl ProtocolUtils {
    ...
    pub async fn generate_peer_id(address: SocketAddr, nonce: u32, user_agent: &str) -> neo_core::UInt160 {
    ...
    pub fn validate_message(message: &NetworkMessage) -> Result<()> {
        ...
        if message.header.length > 0x02000000 { // 32MB limit
            return Err(Error::Protocol("Payload too large".to_string()));
        }

        // Validate checksum if payload exists
        if message.header.length > 0 {
            let message_bytes = message.to_bytes().map_err(|e| Error::Protocol(format!("Failed to serialize message: {}", e)))?;
            ...
            if message_bytes.len() > payload_start {
                ...
                if calculated_checksum != message.header.checksum {
                    return Err(Error::Protocol("Invalid checksum".to_string()));
                }
            }
        }
    ...
    pub fn is_sync_message(command: &str) -> bool {
        matches!(command, 
            "version" | "verack" | "getblocks" | "getdata" | 
            "block" | "headers" | "getheaders" | "inv"
        )
    }
...
impl MessageHandler for DefaultMessageHandler {
    async fn handle_message(&self, peer_address: SocketAddr, message: &NetworkMessage) -> Result<()> {
        tracing::debug!("Default handler received message from {}: {}", peer_address, message.header.command);
...
mod tests {
    ...
    fn test_expected_responses() {
        assert_eq!(ProtocolUtils::get_expected_response("version"), Some("verack"));
        assert_eq!(ProtocolUtils::get_expected_response("ping"), Some("pong"));
        assert_eq!(ProtocolUtils::get_expected_response("getblocks"), Some("inv"));
