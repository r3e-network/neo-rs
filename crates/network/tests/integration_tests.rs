...
use neo_network::*;
use neo_core::{Transaction, UInt160, UInt256, Signer, WitnessScope};
use neo_ledger::{Blockchain, Storage, StorageKey, StorageItem};
use std::sync::Arc;
use crate::{NetworkConfig, RpcConfig};

async fn create_test_blockchain() -> Arc<Blockchain> {
    Arc::new(Blockchain::new(neo_ledger::blockchain::NetworkType::TestNet).await.unwrap())
}
...
async fn test_complete_network_integration() {
    ...
    println!("   🔸 Blockchain initialized with {} blocks", blockchain.height().await);
