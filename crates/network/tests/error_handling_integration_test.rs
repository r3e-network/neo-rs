...
use neo_network::{
    NetworkConfig, PeerManager, NetworkErrorHandler, NetworkErrorEvent,
    Error, ErrorSeverity, RecoveryStrategy, OperationContext
};
...
async fn test_error_handler_integration() {
    ...
    let strategy1 = error_handler.handle_error(&connection_error, &mut context1).await;
    let strategy2 = error_handler.handle_error(&protocol_error, &mut context2).await;
    let strategy3 = error_handler.handle_error(&timeout_error, &mut context3).await;
    ...
    if let Some(partition_event) = error_handler.detect_network_partition().await {
        match partition_event {
            NetworkErrorEvent::NetworkPartitionDetected { affected_peers, .. } => {
                println!("Network partition detected with {} affected peers", affected_peers.len());
