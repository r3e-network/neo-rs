...
fn test_murmur_hash_functions() {
    ...
    assert_ne!((0, 0), murmur128_result, "Murmur128 should produce non-zero hash");
    
    // Test consistency - same input should produce same output
    let murmur32_result2 = murmur::murmur32(input, seed);
    assert_eq!(murmur32_result, murmur32_result2, "Murmur32 should be deterministic");
    
    let murmur128_result2 = murmur::murmur128(input, seed);
    assert_eq!(murmur128_result, murmur128_result2, "Murmur128 should be deterministic");
    
    // Test with different seed
    let different_seed = 123u32;
    let murmur32_different = murmur::murmur32(input, different_seed);
    assert_ne!(murmur32_result, murmur32_different, "Different seeds should produce different hashes");
}
...
fn test_sha256_hash() {
    ...
    assert_ne!(result, result2, "Different inputs should produce different hashes");
}
...
fn test_sha512_hash() {
    ...
    assert_ne!(sha512_result, result3, "Different inputs should produce different hashes");
}
...
fn test_hash160() {
    ...
    assert_eq!(result, manual_hash160, "Hash160 should be RIPEMD160(SHA256(data))");
...
fn test_hash256() {
    ...
    assert_eq!(result, manual_hash256, "Hash256 should be SHA256(SHA256(data))");
...
fn test_murmur_edge_cases() {
    ...
    assert_eq!(murmur32_empty, murmur32_empty2, "Murmur32 should be deterministic");
    assert_eq!(murmur128_empty, murmur128_empty2, "Murmur128 should be deterministic");
