...
impl SafeWitnessBuilder {
    ...
    pub fn build(self) -> Result<Witness, CoreError> {
        let invocation_script = self.invocation_script
...
mod tests {
    ...
    fn test_witness_validation() {
        ...
        assert!(result.unwrap_err().to_string().contains("Invocation script cannot be empty"));
        ...
        let result = SafeWitnessOperations::validate_witness(&witness);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Verification script cannot be empty"));
    }
    ...
    fn test_safe_witness_builder() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invocation script is required"));
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Verification script is required"));
    }
