...
fn bench_ecdsa_sign_verify(c: &mut Criterion) {
    ...
    for size in [32, 64, 128, 256, 512, 1024].iter() {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(keypair.sign(&large_message).unwrap())
                })
            },
        );
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    black_box(public_key.verify(&large_message, &large_signature).unwrap())
                })
            },
        );
    }
...
fn bench_hash_operations(c: &mut Criterion) {
    ...
    group.bench_function("sha256", |b| {
        b.iter(|| {
            black_box(sha256(test_data))
        })
    });
    
    group.bench_function("hash160", |b| {
        b.iter(|| {
            black_box(hash160(test_data))
        })
    });
    
    group.bench_function("hash256", |b| {
        b.iter(|| {
            black_box(hash256(test_data))
        })
    });
    ...
    for size in [32, 64, 128, 256, 512, 1024, 2048, 4096].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("sha256_data_size", size),
            size,
            |b, _| {
                b.iter(|| {
                    black_box(sha256(&large_data))
                })
            },
        );
        
        group.bench_with_input(
            BenchmarkId::new("hash160_data_size", size),
            size,
            |b, _| {
                b.iter(|| {
                    black_box(hash160(&large_data))
                })
            },
        );
        
        group.bench_with_input(
            BenchmarkId::new("hash256_data_size", size),
            size,
            |b, _| {
                b.iter(|| {
                    black_box(hash256(&large_data))
                })
            },
        );
    }
...
fn bench_key_serialization(c: &mut Criterion) {
    ...
    group.bench_function("public_key_to_bytes", |b| {
        b.iter(|| {
            black_box(public_key.to_bytes())
        })
    });
    
    group.bench_function("public_key_from_bytes", |b| {
        b.iter(|| {
            black_box(PublicKey::from_bytes(&public_key_bytes).unwrap())
        })
    });
    
    group.bench_function("public_key_to_hex", |b| {
        b.iter(|| {
            black_box(hex::encode(public_key.to_bytes()))
        })
    });
    ...
    group.bench_function("private_key_to_bytes", |b| {
        b.iter(|| {
            black_box(private_key.to_bytes())
        })
    });
    
    group.bench_function("private_key_from_bytes", |b| {
        b.iter(|| {
            black_box(PrivateKey::from_bytes(&private_key_bytes).unwrap())
        })
    });
