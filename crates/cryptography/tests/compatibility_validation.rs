...
fn test_hash_compatibility() {
    ...
    for (input, expected) in sha256_cases {
        ...
        assert_eq!(result_hex, expected, "SHA256 test failed for input '{}'", input);
        println!("✅ PASS\n");
    }
    ...
    for (input, expected) in ripemd_cases {
        ...
        println!("Expected:        {}", expected);
        assert_eq!(result_hex, expected, "RIPEMD160 test failed for input '{}'", input);
        println!("✅ PASS\n");
    }
...
fn test_ecdsa_compatibility() {
    ...
    assert_eq!(public_key.len(), 65, "Uncompressed public key should be 65 bytes");
    assert_eq!(public_key[0], 0x04, "Uncompressed key should start with 0x04");
    println!("✅ Public key derivation: PASS");
    
    let compressed_key = ECDsa::derive_compressed_public_key(&private_key).unwrap();
    assert_eq!(compressed_key.len(), 33, "Compressed public key should be 33 bytes");
    ...
    // Test key validation
    assert!(ECDsa::validate_private_key(&private_key), "Generated private key should be valid");
    assert!(ECDsa::validate_public_key(&public_key), "Derived public key should be valid");
    assert!(ECDsa::validate_public_key(&compressed_key), "Compressed key should be valid");
...
fn test_base58_compatibility() {
    ...
    assert!(!encoded_check.is_empty(), "Base58Check encoding should not be empty");
