...
fn test_hash_empty_input_consistency() {
    ...
    assert_eq!(ripemd160_result.len(), 20, "RIPEMD160 should produce 20 bytes");
    assert_eq!(sha256_result.len(), 32, "SHA256 should produce 32 bytes");
    
    // Results should be deterministic
    assert_eq!(hash160_result, hash160(empty_data), "Hash160 should be deterministic");
    assert_eq!(hash256_result, hash256(empty_data), "Hash256 should be deterministic");
}
...
fn test_hash_single_byte_variations() {
    ...
    for byte_value in 0u8..=255u8 {
        ...
        assert!(hash160_results.insert(hash160_result), 
               "Hash160 collision for byte {}", byte_value);
        assert!(hash256_results.insert(hash256_result), 
               "Hash256 collision for byte {}", byte_value);
    }
    
    // We should have 256 unique results
    assert_eq!(hash160_results.len(), 256, "All Hash160 results should be unique");
    assert_eq!(hash256_results.len(), 256, "All Hash256 results should be unique");
}
...
fn test_hash_boundary_conditions() {
    ...
    for (i, test_data) in test_cases.iter().enumerate() {
        ...
        assert_eq!(hash160_result.len(), 20, "Boundary case {} Hash160 length", i);
        assert_eq!(hash256_result.len(), 32, "Boundary case {} Hash256 length", i);
        
        // Verify deterministic behavior
        assert_eq!(hash160_result, hash160(test_data), 
                  "Boundary case {} Hash160 deterministic", i);
        assert_eq!(hash256_result, hash256(test_data), 
                  "Boundary case {} Hash256 deterministic", i);
    }
}
...
fn test_murmur_hash_variations() {
    ...
    for (input_idx, input) in test_inputs.iter().enumerate() {
        for (seed_idx, &seed) in test_seeds.iter().enumerate() {
            ...
            assert_eq!(hash_result, hash_again, 
                      "MurmurHash deterministic input:{} seed:{}", input_idx, seed_idx);
        }
    }
}
...
fn test_hash_avalanche_effect() {
    ...
    for byte_pos in 0..base_input.len() {
        ...
        assert_ne!(base_hash160, modified_hash160, 
                  "Hash160 avalanche failed at byte {}", byte_pos);
        assert_ne!(base_hash256, modified_hash256, 
                  "Hash256 avalanche failed at byte {}", byte_pos);
        ...
        assert!(diff_bytes_160 >= 8, "Hash160 should change at least 8 bytes");
        assert!(diff_bytes_256 >= 12, "Hash256 should change at least 12 bytes");
    }
}
...
fn test_hash_collision_resistance_basic() {
    ...
    for i in 0..ITERATIONS {
        ...
        assert!(hash160_set.insert(hash160_result), 
               "Hash160 collision at iteration {}", i);
        assert!(hash256_set.insert(hash256_result), 
               "Hash256 collision at iteration {}", i);
    }
    
    assert_eq!(hash160_set.len(), ITERATIONS, "All Hash160 should be unique");
    assert_eq!(hash256_set.len(), ITERATIONS, "All Hash256 should be unique");
}
...
fn test_large_input_processing() {
    ...
    assert_eq!(hash160_result.len(), 20, "Hash160 should handle large input");
    assert_eq!(hash256_result.len(), 32, "Hash256 should handle large input");
    
    // Should be deterministic
    assert_eq!(hash160_result, hash160(&large_input), "Large input Hash160 deterministic");
    assert_eq!(hash256_result, hash256(&large_input), "Large input Hash256 deterministic");
}
...
fn test_ecdsa_key_generation_uniqueness() {
    ...
    for i in 0..KEY_COUNT {
        ...
        assert_eq!(private_key.len(), 32, "Private key {} should be 32 bytes", i);
        assert!(!private_key.iter().all(|&b| b == 0), "Private key {} should not be zero", i);
        
        // Verify uniqueness
        assert!(private_keys.insert(private_key), "Private key {} should be unique", i);
    }
    
    assert_eq!(private_keys.len(), KEY_COUNT, "All private keys should be unique");
}
...
fn test_ecdsa_public_key_derivation_consistency() {
    for i in 0..10 {
        ...
        if let (Ok(pub1), Ok(pub2)) = (
            ...
            assert_eq!(pub1, pub2, "Public key derivation {} should be deterministic", i);
        ...
        if let (Ok(comp1), Ok(comp2)) = (
            ...
            assert_eq!(comp1, comp2, "Compressed key derivation {} should be deterministic", i);
            assert_eq!(comp1.len(), 33, "Compressed public key should be 33 bytes");
            assert!(comp1[0] == 0x02 || comp1[0] == 0x03, "Compressed key should start with 0x02 or 0x03");
        }
    }
}
...
fn test_ecdsa_signature_deterministic() {
    ...
    if let (Ok(signature1), Ok(signature2)) = (sig1, sig2) {
        ...
        assert!(signature1.len() >= 64, "Signature should be at least 64 bytes");
    }
}
...
fn test_ecdsa_key_format_validation() {
    ...
    if let Ok(valid_public) = ECDsa::derive_public_key(&valid_private) {
        // Basic format checks
        assert_eq!(valid_private.len(), 32, "Valid private key should be 32 bytes");
        assert_eq!(valid_public.len(), 65, "Valid public key should be 65 bytes");
        assert_eq!(valid_public[0], 0x04, "Uncompressed public key should start with 0x04");
    }
    
    // Test compressed key format
    if let Ok(compressed_public) = ECDsa::derive_compressed_public_key(&valid_private) {
        assert_eq!(compressed_public.len(), 33, "Compressed public key should be 33 bytes");
        assert!(compressed_public[0] == 0x02 || compressed_public[0] == 0x03, 
...
fn test_base58_encoding_decoding_roundtrip() {
    ...
    for (i, test_data) in test_cases.iter().enumerate() {
        ...
        if let Ok(decoded) = base58::decode(&encoded) {
            assert_eq!(test_data, &decoded, "Base58 round-trip failed for test case {}", i);
        }
        
        // Encoded string should not be empty unless input was empty
        if !test_data.is_empty() {
            assert!(!encoded.is_empty(), "Encoded string should not be empty for test case {}", i);
        }
    }
}
...
fn test_base58_invalid_character_handling() {
    ...
    for invalid_string in invalid_strings {
        ...
        assert!(result.is_err(), "Invalid Base58 string should be rejected: {}", invalid_string);
    }
}
...
fn test_base58_edge_cases() {
    ...
    if let Ok(decoded_empty) = base58::decode(&encoded_empty) {
        assert_eq!(empty_data, decoded_empty, "Empty data round-trip should work");
    }
    ...
    for byte_value in 0u8..=255u8 {
        ...
        if let Ok(decoded) = base58::decode(&encoded) {
            assert_eq!(single_byte, decoded, "Single byte {} round-trip failed", byte_value);
        }
    }
}
