...
fn test_try_catch_finally_flow() {
    ...
    let stack_items: Vec<_> = (0..result_stack.len()).map(|i| result_stack.peek(i as isize).unwrap()).collect();
...
fn test_exception_propagation() {
    ...
    let script_bytes = vec![
        ...
        OpCode::TRY as u8, 0x07, 0xFF, // 0xFF means no finally
        
        // Try block: Call function that throws
        OpCode::CALL as u8, 0x0A, // Call offset 10
