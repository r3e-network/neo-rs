...
fn test_debugger_breakpoints() {
    ...
    // Load the script
    debugger.engine_mut().load_script(script.clone(), 0, 0).unwrap();
    ...
    assert_eq!(debugger.engine().current_context().unwrap().instruction_pointer(), 2);
    ...
    assert_eq!(state, VMState::BREAK);
    assert_eq!(debugger.engine().current_context().unwrap().instruction_pointer(), 3);
...
fn test_debugger_step() {
    ...
    let state = debugger.step().unwrap();
    assert_eq!(state, VMState::BREAK);
    assert_eq!(debugger.engine().current_context().unwrap().instruction_pointer(), 1);
...
fn test_debugger_step_over() {
    ...
    let script_bytes = vec![
        ...
        OpCode::CALL as u8, 0x04, 0x00,     // Main: Call function at offset 4
    ...
    assert_eq!(state, VMState::BREAK);
    assert_eq!(debugger.engine().current_context().unwrap().instruction_pointer(), 4);
