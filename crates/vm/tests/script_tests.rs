...
fn test_script_creation() {
    let script_bytes = vec![OpCode::PUSH1 as u8, OpCode::PUSH2 as u8, OpCode::ADD as u8, OpCode::RET as u8];
...
fn test_script_get_jump_offset() {
    let script_bytes = vec![
        OpCode::JMP as u8, 0x02,                // JMP +2
        OpCode::PUSH1 as u8,                    // PUSH1 (skipped)
        OpCode::PUSH2 as u8,                    // PUSH2
        OpCode::JMP_L as u8, 0xFF, 0xFF, 0xFF, 0xFC, // JMP -4
...
fn test_script_get_jump_target() {
    let script_bytes = vec![
        ...
        OpCode::PUSH1 as u8,                    // PUSH1 (skipped)
        OpCode::PUSH2 as u8,                    // PUSH2
        OpCode::JMP_L as u8, 0xFC, 0xFF, 0xFF, 0xFF, // JMP -4
        OpCode::RET as u8,                      // RET (skipped)
    ];
    let script = Script::new_relaxed(script_bytes);
...
fn test_script_get_try_offsets() {
    let script_bytes = vec![
        OpCode::TRY as u8, 0x05, 0x00, 0x0A, 0x00, // TRY with catch at +5 and finally at +10
