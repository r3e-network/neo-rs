...
use neo_vm::{
    ExecutionEngine, VMState, TriggerType, Script, OpCode,
    stack_item::StackItem, evaluation_stack::EvaluationStack,
...
fn test_vm_arithmetic_operations() {
    ...
    assert_eq!(final_result.as_int().unwrap().to_string(), "7", "Final result should be 7");
...
fn test_vm_jump_operations() {
    let script_bytes = vec![
        ...
        OpCode::JMP as u8, 3,    // Jump forward 3 bytes
    ...
    let result_stack = engine.result_stack();
    assert_eq!(result_stack.len(), 2, "Should have 2 items (1 and 2, not 16)");
...
fn test_vm_conditional_jumps() {
    ...
    let script_bytes = vec![
        ...
        OpCode::JMPIF as u8, 4,  // Jump if true (4 bytes forward)
    ...
    let top = result_stack.peek(0).unwrap();
    assert_eq!(top.as_int().unwrap().to_string(), "8", "Should jump to 8, not 16 or 15");
...
fn test_vm_array_operations() {
    ...
    assert_eq!(size.as_int().unwrap().to_string(), "2", "Array size should be 2");
...
fn test_vm_complex_operations() {
    ...
    let final_result = result_stack.peek(0).unwrap();
    assert_eq!(final_result.as_int().unwrap().to_string(), "45", "Result should be 45");
...
fn test_vm_performance_scaling() {
    ...
    let mut engine = ExecutionEngine::new(None);
    engine.load_script(Script::new_relaxed(small_script), -1, 0).unwrap();
    engine.execute();
    ...
    let mut engine = ExecutionEngine::new(None);
    engine.load_script(Script::new_relaxed(medium_script), -1, 0).unwrap();
    engine.execute();
...
fn test_vm_instruction_handling() {
    ...
    let script_bytes = vec![
        ...
        OpCode::PUSHDATA1 as u8, 2, 0x12, 0x34, // Push data with length
