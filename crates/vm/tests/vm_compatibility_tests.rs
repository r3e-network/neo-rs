...
pub fn execute_and_verify(
    ...
    if let Ok(_context) = engine.load_script(script, 0, 0) {
        ...
        if engine.state() != expected.vm_state {
            println!("VM state mismatch: expected {:?}, got {:?}", expected.vm_state, engine.state());
            return false;
        }
        ...
        if result_stack.len() != expected.result_stack.len() {
            println!("Stack size mismatch: expected {}, got {}", expected.result_stack.len(), result_stack.len());
            return false;
        }
        ...
        if has_exception != expected.has_exception {
            println!("Exception state mismatch: expected {}, got {}", expected.has_exception, has_exception);
            return false;
        }
...
mod tests {
    ...
    fn test_simple_add_operation() {
        ...
        let expected = ExpectedExecutionResult {
            ...
            result_stack: vec![
                ExpectedStackItem::Integer("3".to_string()),
            ],
            has_exception: false,
        };
    ...
    fn test_boolean_operations() {
        ...
        let expected = ExpectedExecutionResult {
            vm_state: VMState::HALT,
            result_stack: vec![
                ExpectedStackItem::Boolean(false),
            ],
            has_exception: false,
        };
    ...
    fn test_vm_compatibility_basic_operations() {
        ...
        let test_cases = vec![
            ...
            (vec![OpCode::PUSH1 as u8, OpCode::PUSH2 as u8, OpCode::ADD as u8], vec!["3"]),
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH3 as u8, OpCode::SUB as u8], vec!["2"]),
            (vec![OpCode::PUSH4 as u8, OpCode::PUSH2 as u8, OpCode::MUL as u8], vec!["8"]),
            (vec![OpCode::PUSH8 as u8, OpCode::PUSH2 as u8, OpCode::DIV as u8], vec!["4"]),
        ];
        ...
        for (script_bytes, expected) in test_cases {
            ...
            if let Ok(_context) = engine.load_script(script, 0, 0) {
                ...
                for (i, expected_value) in expected.iter().enumerate() {
                    let stack_item = result_stack.peek(i as isize).expect("Stack item should exist");
    ...
    fn test_stack_operations_compatibility() {
        ...
        let test_cases = vec![
            ...
            // SWAP operation
            (vec![OpCode::PUSH1 as u8, OpCode::PUSH2 as u8, OpCode::SWAP as u8], vec!["1", "2"]),
    ...
    fn test_comparison_operations_compatibility() {
        let test_cases = vec![
            // EQUAL operation
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH5 as u8, OpCode::EQUAL as u8], vec!["1"]), // true
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH3 as u8, OpCode::EQUAL as u8], vec!["0"]), // false
            
            // NUMEQUAL operation
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH5 as u8, OpCode::NUMEQUAL as u8], vec!["1"]), // true
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH3 as u8, OpCode::NUMEQUAL as u8], vec!["0"]), // false
            
            // LT operation
            (vec![OpCode::PUSH3 as u8, OpCode::PUSH5 as u8, OpCode::LT as u8], vec!["1"]), // true
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH3 as u8, OpCode::LT as u8], vec!["0"]), // false
            
            // GT operation
            (vec![OpCode::PUSH5 as u8, OpCode::PUSH3 as u8, OpCode::GT as u8], vec!["1"]), // true
            (vec![OpCode::PUSH3 as u8, OpCode::PUSH5 as u8, OpCode::GT as u8], vec!["0"]), // false
        ];
