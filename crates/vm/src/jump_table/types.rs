...
fn convert(engine: &mut ExecutionEngine, instruction: &Instruction) -> Result<()> {
    ...
    let item_type = StackItemType::from_byte(type_byte).ok_or_else(|| Error::InvalidInstruction(format!("Invalid type: {}", type_byte)))?;
    ...
    let result = match (item, item_type) {
        ...
        (StackItem::InteropInterface(interface), StackItemType::InteropInterface) => StackItem::InteropInterface(interface),
        
        // Invalid conversions
        (item, target_type) => return Err(Error::InvalidType(format!("Cannot convert {:?} to {:?}", item.stack_item_type(), target_type))),
    };
...
fn is_type(engine: &mut ExecutionEngine, instruction: &Instruction) -> Result<()> {
    // Get the current context
    let context = engine.current_context_mut().ok_or_else(|| Error::InvalidOperation("No current context".into()))?;
...
fn is_null(engine: &mut ExecutionEngine, _instruction: &Instruction) -> Result<()> {
    // Get the current context
    let context = engine.current_context_mut().ok_or_else(|| Error::InvalidOperation("No current context".into()))?;
