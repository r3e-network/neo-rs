...
use super::{
    types::{SyscallDescriptor, ParameterType, InteropParameter},
    witness::validate_call_flags,
};
use num_traits::ToPrimitive;

/// Implements the SYSCALL operation.
pub fn syscall(engine: &mut ExecutionEngine, instruction: &Instruction) -> crate::Result<()> {
    ...
    let result = super::interop_services::invoke_interop_service(engine, &descriptor.name, parameters)?;
...
pub fn get_interop_descriptor(hash: u32) -> Option<SyscallDescriptor> {
    ...
    match hash {
        ...
        0x627D5B52 => Some(SyscallDescriptor {
            ...
            fixed_price: 32768, // 1 << 15
            required_call_flags: crate::call_flags::CallFlags::READ_STATES | crate::call_flags::CallFlags::ALLOW_CALL,
...
pub fn add_fee(engine: &mut ExecutionEngine, fee: u64) -> crate::Result<()> {
...
pub fn convert_parameter(item: StackItem, param_type: &ParameterType) -> crate::Result<InteropParameter> {
    match param_type {
                ...
                StackItem::Array(items) => {
                    ...
                    for array_item in items {
                        ...
                        let converted_param = match array_item {
                            StackItem::Integer(i) => InteropParameter::Integer(i.to_i64().unwrap_or(0)),
                            ...
                            StackItem::Array(nested_array) => {
                                ...
                                let nested_params: Vec<InteropParameter> = nested_array.iter().map(|nested_item| {
                                    match nested_item {
                                        StackItem::Integer(nested_i) => InteropParameter::Integer(nested_i.to_i64().unwrap_or(0)),
