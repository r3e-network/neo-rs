...
impl GasCalculator {
    ...
    pub fn check_gas(&self, additional_gas: i64) -> bool {
        let actual_cost = (additional_gas as u64).saturating_mul(self.exec_fee_factor as u64) as i64;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            GasError::OutOfGas { consumed, limit } => {
                write!(f, "Out of gas: consumed {} exceeds limit {}", consumed, limit)
            }
            GasError::InvalidGas { amount } => {
                write!(f, "Invalid gas amount: {}", amount)
            }
            GasError::GasLimitExceeded { required, available } => {
                write!(f, "Gas limit exceeded: required {} > available {}", required, available)
            }
        }
    }
}
