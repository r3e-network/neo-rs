...
use neo_wallets::*;
use neo_core::{UInt160, Transaction};
use std::sync::Arc;
use tokio;
use tempfile::TempDir;
use neo_core::{UInt256};
use neo_wallets::{
    Contract, Wallet, WalletAccount, ScryptParameters,
    WalletError, WalletResult, StandardWalletAccount, KeyPair,
};
...
async fn test_complete_wallet_workflow() {
    ...
    let watch_only = wallet.create_account_watch_only(UInt160::new()).await.unwrap();
...
async fn test_wallet_import_export_workflow() {
    ...
    assert!(wif_account.verify(test_data, &nep2_signature).await.unwrap());
    assert!(nep2_account.verify(test_data, &wif_signature).await.unwrap());
}
...
async fn test_wallet_persistence_workflow() {
    ...
    let account1 = wallet.create_account(&[1u8; 32]).await.unwrap();
    let account2 = wallet.create_account(&[2u8; 32]).await.unwrap();
    wallet.set_default_account(&account1.script_hash()).await.unwrap();
    ...
    assert_eq!(wallet.get_accounts().len(), saved_wallet.get_accounts().len());
...
async fn test_error_handling_workflow() {
    ...
    wallet.change_password("", "correct_password").await.unwrap();
