...
use crate::{
    ...
    contract::Contract,
    key_pair::KeyPair,
    Error, Result, Version, ContractParameterType,
};
...
impl WalletManager {
    ...
    pub async fn migrate_wallet(
        ...
        let old_factory = self.get_factory(old_path)
            .ok_or_else(|| WalletError::Other("No suitable factory for old wallet".to_string()))?;
        
        let new_factory = self.get_factory(new_path)
        ...
        // Create new wallet
        let mut new_wallet = new_factory.create_wallet(old_wallet.name(), new_path, password).await?;
        
        // Copy all accounts
        for account in old_wallet.get_accounts() {
            if let Some(key_pair) = account.get_key() {
                if let Some(contract) = account.get_contract() {
                    new_wallet.create_account_with_contract(contract.clone(), Some(key_pair)).await?;
                } else {
                    new_wallet.create_account(&key_pair.private_key()).await?;
                }
            } else {
                // Watch-only account
                new_wallet.create_account_watch_only(account.script_hash()).await?;
            }
        }
