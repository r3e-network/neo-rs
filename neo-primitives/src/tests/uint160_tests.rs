...
use crate::{UInt160, PrimitiveError};
...
mod uint160_tests {
    ...
    fn test_generator3() {
        ...
        assert_eq!("0xff00000000000000000000000000000000000001", uint160.to_hex_string());
    }

    #[test]
    fn test_generator4() {
        let uint160 = UInt160::parse("0x0102030405060708090a0b0c0d0e0f1011121314").unwrap();
        assert_eq!("0x0102030405060708090a0b0c0d0e0f1011121314", uint160.to_hex_string());
    }
    ...
    fn test_compare_to() {
        ...
        assert_eq!(std::cmp::Ordering::Equal, UInt160::zero().cmp(&UInt160::zero()));
        assert_eq!(std::cmp::Ordering::Less, UInt160::zero().cmp(&result));
        assert_eq!(std::cmp::Ordering::Greater, result.cmp(&UInt160::zero()));
        assert_eq!(std::cmp::Ordering::Equal, result.cmp(&UInt160::from_bytes(&temp).unwrap()));
    }
    ...
    fn test_equals() {
        ...
        assert_eq!(UInt160::zero(), UInt160::parse("0x0000000000000000000000000000000000000000").unwrap());
        assert_ne!(UInt160::zero(), UInt160::parse("0x0000000000000000000000000000000000000001").unwrap());
    }
    ...
    fn test_try_parse() {
        ...
        assert!(UInt160::try_parse("0000000000000000000000000000000000000000", &mut result));
        assert_eq!(result.unwrap(), UInt160::zero());
    }
    ...
    fn test_to_string() {
        let uint160 = UInt160::parse("0x0102030405060708090a0b0c0d0e0f1011121314").unwrap();
        assert_eq!("0x1413121110f0e0d0c0b0a090807060504030201", uint160.to_string());
    }
}
