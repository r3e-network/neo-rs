...
#[command(
    name = "neo-cli",
    version,
    about = "JSON-RPC client for Neo N3 nodes",
    long_about = "neo-cli is a command-line JSON-RPC client for Neo N3.\n\n\
It connects to a running neo-node (or any compatible Neo JSON-RPC endpoint) to query chain state and\n\
submit transactions.\n\n\
It does not run a node daemon; use neo-node to run the network/P2P sync and to host the RPC server.",
    after_help = "Examples:\n  neo-cli state\n  neo-cli block 1000\n  neo-cli --rpc-url http://localhost:10332 version\n"
)]
struct Cli {
    /// RPC server URL
    #[arg(
