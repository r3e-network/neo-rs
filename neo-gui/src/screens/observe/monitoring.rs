...
pub fn ui(app: &mut NeoGuiApp, ui: &mut Ui) {
    ...
    ui.horizontal_wrapped(|ui| {
        gauge(ui, "CPU", host.cpu_percent, format!("{:.0}%", host.cpu_percent));
        gauge(
            ui,
            "Memory",
            host.mem_percent(),
            format!("{} / {}", human_bytes(host.mem_used), human_bytes(host.mem_total)),
        );
        gauge(
            ui,
            "Disk",
            host.disk_percent(),
            format!("{} / {}", human_bytes(host.disk_used), human_bytes(host.disk_total)),
        );
    });
    ...
    widgets::card(ui, |ui| {
        alert_row(ui, "Node reachable", online, if online { "ok" } else { "node not responding" });
        alert_row(ui, "Disk < 90%", host.disk_percent() < 90.0, &format!("{:.0}% used", host.disk_percent()));
        alert_row(ui, "Memory < 90%", host.mem_percent() < 90.0, &format!("{:.0}% used", host.mem_percent()));
        let syncing = bps_hist.iter().rev().take(5).any(|b| *b > 0.0);
        alert_row(ui, "Sync progressing", syncing || !online, if syncing { "advancing" } else { "stalled?" });
    });
}
...
fn gauge(ui: &mut Ui, label: &str, percent: f32, sub: String) {
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            ui.add_space(6.0);
            let color = if percent > 90.0 { theme::ERR } else if percent > 75.0 { theme::WARN } else { theme::OK };
            ui.add(
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            let points: egui_plot::PlotPoints =
                data.iter().enumerate().map(|(i, v)| [i as f64, *v as f64]).collect();
            ...
            plot.show(ui, |plot_ui| {
                plot_ui.line(egui_plot::Line::new(points).color(color).width(2.0).fill(0.0));
            });
        });
}
...
fn alert_row(ui: &mut Ui, label: &str, ok: bool, detail: &str) {
    ui.horizontal(|ui| {
        let (glyph, color) = if ok { ("●", theme::OK) } else { ("●", theme::ERR) };
        ui.label(egui::RichText::new(glyph).color(color));
        ui.label(egui::RichText::new(label).color(theme::TEXT));
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(egui::RichText::new(detail).color(theme::TEXT_MUTED).size(12.0));
        });
    });
}
