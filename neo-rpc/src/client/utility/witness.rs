...
pub fn payload_witness_from_json(json: &JObject) -> Result<PayloadWitness, String> {
    let (invocation_bytes, verification_bytes) = parse_witness_scripts(json)?;
    Ok(PayloadWitness::new_with_scripts(invocation_bytes, verification_bytes))
}

pub fn scripts_to_witness_json(invocation: &[u8], verification: &[u8]) -> JObject {
    let mut json = JObject::new();
    json.insert("invocation".to_string(), neo_serialization::json::JToken::String(general_purpose::STANDARD.encode(invocation)));
    json.insert("verification".to_string(), neo_serialization::json::JToken::String(general_purpose::STANDARD.encode(verification)));
    json
}
...
fn parse_witness_scripts(json: &JObject) -> Result<(Vec<u8>, Vec<u8>), String> {
    ...
    let verification = json.get("verification").ok_or("Missing 'verification' field")?;
