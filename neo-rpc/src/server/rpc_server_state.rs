...
//!
//! The in-tree `neo_state_service::StateStore` is the state-root
//! verification cache (validated roots by index / by hash); it does not
//! persist the MPT trie itself. The endpoints split accordingly:
//!
//! - `getstateheight` / `getstateroot` are served from the state-root
//!   cache.
...
//! - `getproof` / `getstate` / `findstates` require a persisted MPT
//!   keyed by historical root hashes, which this build's state service
//!   does not maintain; they return a clean JSON-RPC error instead of
//!   guessing.
...
impl RpcServerState {
    ...
    fn get_proof(server: &RpcServer, _params: &[Value]) -> Result<Value, RpcException> {
        let _ = Self::state_store(server)?;
        Err(Self::proofs_unsupported())
   }
    ...
    fn get_state(server: &RpcServer, _params: &[Value]) -> Result<Value, RpcException> {
        let _ = Self::state_store(server)?;
        Err(Self::proofs_unsupported())
   }

    fn find_states(server: &RpcServer, _params: &[Value]) -> Result<Value, RpcException> {
        let _ = Self::state_store(server)?;
        Err(Self::proofs_unsupported())
   }
    ...
    /// [`Self::decode_proof_payload`]); used by tests to round-trip
    /// proof payloads.
    #[cfg(test)]
