...
//!
//! Replaces the previous HTTP routing glue. The dispatch logic
//! (`resolve_rpc_handler` / `invoke_rpc_handler`) now lives in
//! `super::dispatch`, so the same panic-safe handler invocation is reused by
//! this jsonrpsee module and any future transport.
