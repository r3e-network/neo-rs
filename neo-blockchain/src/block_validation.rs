...
//
// block_validation.rs file belongs to the neo project and is free
...
pub fn validate_transaction_count_raw(tx_count: usize) -> Result<(), BlockValidationError> {
    validate_transaction_count_raw_with_limit(tx_count, MAX_TRANSACTIONS_PER_BLOCK)
}
...
mod tests {
    ...
    fn validate_transaction_count_raw_with_limit_uses_effective_protocol_limit() {
        assert!(validate_transaction_count_raw_with_limit(200, 200).is_ok());
        assert_eq!(
            validate_transaction_count_raw_with_limit(201, 200),
            Err(BlockValidationError::TooManyTransactions {
                count: 201,
                max_count: 200,
            })
    ...
    fn validate_merkle_root_accepts_empty_block() {
        ...
        assert!(validate_merkle_root(&merkle_root, &tx_hashes).is_ok());
    }
    ...
    fn validate_merkle_root_rejects_wrong_root_for_empty() {
        ...
        let tx_hashes: Vec<UInt256> = vec![];
        assert!(validate_merkle_root(&wrong_root, &tx_hashes).is_err());
    }
    ...
    fn validate_no_duplicate_transactions_accepts_unique() {
        ...
        assert!(validate_no_duplicate_transactions(&tx_hashes).is_ok());
    }
    ...
    fn validate_no_duplicate_transactions_rejects_duplicates() {
        ...
        assert!(validate_no_duplicate_transactions(&tx_hashes).is_err());
    }
