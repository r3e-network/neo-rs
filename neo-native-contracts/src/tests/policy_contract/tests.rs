...
fn raw_argument_parsing_uses_shared_helpers() {
    let source = include_str!("../../policy_contract/mod.rs");
...
#[test]
fn max_transactions_per_block_snapshot_pre_gorgon_returns_settings() {
    let cache = DataCache::new(false);
    seed_current_block(&cache, 100);
    let mut settings = neo_config::ProtocolSettings::default();
    settings.max_transactions_per_block = 200;
    // HfGorgon not registered → fall back to ProtocolSettings.
    let value = PolicyContract::new()
        .get_max_transactions_per_block_snapshot(&cache, &settings)
        .unwrap();
    assert_eq!(value, 200);
}

#[test]
fn max_transactions_per_block_snapshot_post_gorgon_returns_storage() {
    let cache = DataCache::new(false);
    seed_current_block(&cache, 100);
    let mut settings = neo_config::ProtocolSettings::default();
    settings.max_transactions_per_block = 200;
    settings.hardforks.insert(Hardfork::HfGorgon, 0);
    // Storage key not written → fall back to ProtocolSettings.
    let value = PolicyContract::new()
        .get_max_transactions_per_block_snapshot(&cache, &settings)
        .unwrap();
    assert_eq!(value, 200);
    // Write a value and verify it overrides the settings default.
    seed_policy_setting_key(
        &cache,
        PolicyContract::max_transactions_per_block_key(),
        1200,
    );
    let value = PolicyContract::new()
        .get_max_transactions_per_block_snapshot(&cache, &settings)
...
#[test]
fn max_transactions_per_block_snapshot_pre_genesis_fallback() {
    let cache = DataCache::new(false);
    let mut settings = neo_config::ProtocolSettings::default();
    settings.max_transactions_per_block = 2048;
    settings.hardforks.insert(Hardfork::HfGorgon, 0);
    // No current block → pre-genesis fallback to settings.
    let value = PolicyContract::new()
        .get_max_transactions_per_block_snapshot(&cache, &settings)
        .unwrap();
...
#[test]
fn validate_max_transactions_per_block_range() {
    // Within range is ok.
    assert!(PolicyContract::validate_max_transactions_per_block(1).is_ok());
    assert!(PolicyContract::validate_max_transactions_per_block(512).is_ok());
    assert!(PolicyContract::validate_max_transactions_per_block(65_535).is_ok());
    // Out of range is rejected.
    assert!(PolicyContract::validate_max_transactions_per_block(0).is_err());
    assert!(PolicyContract::validate_max_transactions_per_block(65_536).is_err());
    assert!(PolicyContract::validate_max_transactions_per_block(-1).is_err());
}

#[test]
fn max_transactions_per_block_storage_round_trip() {
    let cache = DataCache::new(false);
    // First write: add (key doesn't exist yet).
    PolicyContract::new()
        .put_max_transactions_per_block(&cache, 700)
        .unwrap();
    assert_eq!(
        read_policy_setting_key(&cache, PolicyContract::max_transactions_per_block_key()),
        700
    );
    // Second write: update (key exists).
    PolicyContract::new()
        .put_max_transactions_per_block(&cache, 1_200)
        .unwrap();
    assert_eq!(
        read_policy_setting_key(&cache, PolicyContract::max_transactions_per_block_key()),
        1_200
...
#[test]
fn hf_gorgon_read_max_transactions_per_block_falls_back_to_settings() {
    let cache = DataCache::new(false);
    seed_current_block(&cache, 0);
    let mut settings = neo_config::ProtocolSettings::default();
    settings.max_transactions_per_block = 300;
    // HfGorgon not enabled → should return settings value.
    let engine = ApplicationEngine::new(
        neo_primitives::TriggerType::Application,
        None,
        std::sync::Arc::new(cache),
    ...
    .expect("engine builds");
    let value = PolicyContract::new()
        .read_max_transactions_per_block(&engine)
        .unwrap();
    assert_eq!(value, 300);
}

#[test]
fn default_max_transactions_per_block_constants() {
    // The PolicyContract default constant matches the C# protocol default (512).
    assert_eq!(DEFAULT_MAX_TRANSACTIONS_PER_BLOCK, 512);
    // Upper bound is u16::MAX.
    assert_eq!(MAX_MAX_TRANSACTIONS_PER_BLOCK, 65_535);
    // The pre-built MainNet settings default to 200 (governance override).
    // This confirms ProtocolSettings::default() reads .max_transactions_per_block
    // correctly; the PolicyContract default is the protocol-level constant, not
    // a network-specific override.
    assert_eq!(
        neo_config::ProtocolSettings::default().max_transactions_per_block,
        200
    );
}
