...
usage() {
  cat <<'USAGE'
Usage: scripts/validate-v3101-consistency.sh [--network <all|mainnet|testnet>] [--skip-baseline] [--profile <dev|release>]
...
Pipeline strength (review 2026-09-05 §4.3.4): local nodes run the memory
backend, so this validates protocol/RPC *vectors* and online parity probes —
it does NOT substitute for per-block RocksDB state replay. Gas tolerance
(VECTOR_GAS_TOLERANCE) is an environment allowance for online probes;
conformance claims must be backed by a zero-tolerance run
(VECTOR_GAS_TOLERANCE=0, exact per-vector gas equality).
...
Environment overrides:
  ...
  REPORT_ROOT               Output directory for reports (default: <repo>/reports/compat-v3101)
  VECTOR_GAS_TOLERANCE      Max gas delta per vector accepted by neo.tools.diff.cli;
                            set 0 for exact per-unit equality (recommended for conformance gates)
  ALLOW_POLICY_DEFAULT_VECTOR_MISMATCH  Reconcile policy vectors using live C# policy state when local node is unsynced (default: false)
  NEO_EXECUTION_SPECS_REF  Immutable git ref/commit for the v3.10.1 execution specs (required; no moving main branch)
...
SPEC_REF="${NEO_EXECUTION_SPECS_REF:-}"
if [[ -z "$SPEC_REF" || "$SPEC_REF" == "main" || "$SPEC_REF" == "master" ]]; then
  echo "NEO_EXECUTION_SPECS_REF must be an immutable v3.10.1 tag or commit SHA (moving branches are not allowed)." >&2
  exit 1
fi
REPORT_ROOT="${REPORT_ROOT:-$ROOT_DIR/reports/compat-v3101}"
...
prepare_specs_repo() {
  ...
  if [[ -d "$SPEC_DIR/.git" ]]; then
    git -C "$SPEC_DIR" fetch --depth 1 origin "$SPEC_REF"
  else
    rm -rf "$SPEC_DIR"
    git clone --no-checkout "$SPEC_REPO" "$SPEC_DIR"
    git -C "$SPEC_DIR" fetch --depth 1 origin "$SPEC_REF"
  fi
  git -C "$SPEC_DIR" checkout --detach FETCH_HEAD
  actual_ref="$(git -C "$SPEC_DIR" rev-parse HEAD)"
  echo "execution_specs_ref=$SPEC_REF" | tee "$RUN_DIR/execution-specs-ref.txt"
  echo "execution_specs_commit=$actual_ref" | tee -a "$RUN_DIR/execution-specs-ref.txt"
...
run_vector_diff() {
  ...
  if [[ "$rc" -ne 0 && "${ALLOW_POLICY_DEFAULT_VECTOR_MISMATCH:-false}" == "true" ]]; then
