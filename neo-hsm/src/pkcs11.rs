...
mod tests {
    ...
    /// Verify that the DER-octet-string decoder handles both the wrapped form
    /// (e.g. from GCP libkmsp11) and the already-stripped form correctly.
    #[test]
    fn decode_der_octet_string_wrapped_and_bare() {
        // Fake 65-byte uncompressed point.
        let point = vec![0x04u8; 65];

        // Short-form DER OCTET STRING wrapper (length 65 = 0x41).
        let mut wrapped = vec![0x04, 0x41];
        wrapped.extend_from_slice(&point);
        let decoded = decode_der_octet_string(&wrapped).unwrap();
        assert_eq!(decoded, point);

        // Already bare (no wrapper) — the function should pass it through.
        let decoded_bare = decode_der_octet_string(&point).unwrap();
        assert_eq!(decoded_bare, point);
    }

    /// Verify that the compression helper converts both 65-byte uncompressed
    /// and 33-byte compressed inputs correctly.
    #[test]
    ...
    /// Verify the single-sig redeem script length and opcode structure.
    #[test]
    fn signature_redeem_script_structure() {
        let pubkey = [0x02u8; 33]; // dummy compressed key
        let script = signature_redeem_script(&pubkey);
        // Expected layout: 0x0C 0x21 <33 bytes> 0x41 <4 bytes> = 40 bytes total
    ...
    /// Finalize: raw 64-byte r||s round-trips through low-s normalization.
    ///
    /// We use the `sign_prehash` path so we can construct a known signature,
    /// then verify that `finalize_signature` with RawRs produces valid low-s output.
    #[test]
    fn finalize_raw_rs_low_s() {
        ...
        // Result must verify against the original data.
        let pubkey = Secp256r1Crypto::derive_public_key(&private_key).unwrap();
        // Verification uses full data (not prehash), so we replicate the hashing.
        // neo-crypto verify hashes internally, so verify the prehash path:
        use p256::{
