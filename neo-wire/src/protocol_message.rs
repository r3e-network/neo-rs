...
//! [`crate::Message`]: it carries a `command` discriminator plus the
...
use neo_p2p::payloads::{
    AddrPayload, FilterAddPayload, FilterLoadPayload, GetBlockByIndexPayload,
    GetBlocksPayload, InvPayload, PingPayload, VersionPayload,
};
use neo_p2p::MessageCommand;
use neo_payloads::{
    Block, ExtensiblePayload, HeadersPayload, MerkleBlockPayload, Transaction,
};
...
mod tests {
    ...
    fn protocol_message_command_matches_variant() {
        ...
        assert_eq!(
            ProtocolMessage::pong(42).command(),
            MessageCommand::Pong
        );
    }
    ...
    fn empty_command_round_trip() {
        let payload = ProtocolMessage::Verack.serialize_payload().expect("serialize");
