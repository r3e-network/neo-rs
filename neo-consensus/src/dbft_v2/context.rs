...
impl ConsensusContext {
    ...
    pub fn commit_count(&self) -> usize {
        self.commits.iter().filter(|commit| commit.is_some()).count()
    }
    ...
    pub fn new_recovery_message(&self, meta: MessageMeta) -> Message<RecoveryMessage> {
        ...
        let preparations =
            self.prepares.iter().filter_map(|prepares| prepares.to_preparation_compact()).collect();
        ...
        let req =
            self.prepares.iter().filter_map(|prepares| prepares.request.as_ref()).find(|_req| true);
