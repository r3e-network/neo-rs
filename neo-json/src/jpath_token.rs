...
use crate::jtoken::{JToken, JsonError};
    ...
    fn parse(expr: &str) -> Result<Vec<JPathToken>, JsonError> {
        ...
        while let Some(c) = chars.next() {
            match c {
                ...
                '\'' => {
                    let content: String = chars.by_ref()
                        .take_while(|&ch| ch != '\'')
                        .collect();
...
impl JPath {
    ...
    fn process_json_path(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>) -> Result<Vec<JToken>, JsonError> {
        let max_depth = 6;
    ...
    fn process_dot(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>, max_objects: usize) -> Result<(), JsonError> {
        let token = tokens.pop_front().ok_or(JsonError::FormatError)?;
        match token.token_type {
            ...
            JPathTokenType::Identifier => Self::descent_by_name(objects, token.content.unwrap(), max_objects),
    ...
    fn descent_by_name(objects: &mut Vec<JToken>, name: String, max_objects: usize) -> Result<(), JsonError> {
        let mut new_objects = Vec::new();
        for obj in objects.iter() {
        ...
        *objects = new_objects;
        ...
        fn process_bracket(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>, max_objects: usize) -> Result<(), JsonError> {
            let token = tokens.pop_front().ok_or(JsonError::FormatError)?;
            match token.token_type {
                ...
                JPathTokenType::Number => {
                    let start = token.clone().content.unwrap().parse::<i32>().map_err(|_| JsonError::FormatError)?;
                    match tokens.pop_front().map(|t| t.token_type) {
                        Some(JPathTokenType::Colon) => Self::process_slice(objects, tokens, start, max_objects),
                        Some(JPathTokenType::Comma) => Self::process_union(objects, tokens, token, max_objects),
                        Some(JPathTokenType::RightBracket) => Self::descent_by_index(objects, start, max_objects),
                        _ => Err(JsonError::FormatError),
                    }
                },
        ...
        fn process_recursive_descent(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>, max_objects: usize) -> Result<(), JsonError> {
        ...
        fn process_slice(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>, start: i32, max_objects: usize) -> Result<(), JsonError> {
            ...
            for obj in objects.iter() {
                if let JToken::Array(arr) = obj {
                    let i_start = if start >= 0 { start as usize } else { arr.len().saturating_sub((-start) as usize) };
                    let i_end = if end > 0 { end as usize } else { arr.len().saturating_sub((-end) as usize) };
                    new_objects.extend(arr[i_start.min(arr.len())..i_end.min(arr.len())].iter().cloned());
                }
            }
            *objects = new_objects;
        ...
        fn process_union(objects: &mut Vec<JToken>, tokens: &mut VecDeque<JPathToken>, first: JPathToken, max_objects: usize) -> Result<(), JsonError> {
            ...
            match first.token_type {
                JPathTokenType::Number => {
                    let indices: Result<Vec<i32>, _> = items.iter()
                ...
                JPathTokenType::String => {
                    let keys: Vec<String> = items.into_iter()
        ...
        fn descent_by_index(objects: &mut Vec<JToken>, index: i32, max_objects: usize) -> Result<(), JsonError> {
            let mut new_objects = Vec::new();
            for obj in objects.iter() {
                if let JToken::Array(arr) = obj {
                    let i = if index >= 0 { index as usize } else { arr.len().saturating_sub((-index) as usize) };
