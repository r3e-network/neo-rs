/*!
The `mpt` module implements MPT (Merkle-Patricia Trie).

An MPT stores key-value pairs and is a trie over a 16-symbol alphabet. https://en.wikipedia.org/wiki/Trie
A trie is a tree where values are stored in leaves and keys are paths from the root to the leaf node.
...
regardless of the order they were added/removed in.
The actual trie structure can vary because of node -> HashNode compressing.

There is also one optimization which costs us almost nothing in terms of complexity but is quite beneficial:
