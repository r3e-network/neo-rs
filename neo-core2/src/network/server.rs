...
const DEFAULT_MIN_PEERS: usize = 5;
const DEFAULT_ATTEMPT_CONN_PEERS: usize = 20;
const DEFAULT_MAX_PEERS: usize = 100;
const DEFAULT_EXTENSIBLE_POOL_SIZE: usize = 20;
const DEFAULT_BROADCAST_FACTOR: usize = 0;
const MAX_BLOCK_BATCH: usize = 200;
const PEER_TIME_FACTOR: u64 = 1000;
...
#[derive(Clone)]
struct Config {
    extensible_pool_size: usize,
    min_peers: usize,
    max_peers: usize,
    attempt_conn_peers: usize,
    broadcast_factor: usize,
    addresses: Vec<SocketAddr>,
    seeds: Vec<String>,
    dial_timeout: Duration,
    neofs_block_fetcher_cfg: NeoFSBlockFetcherConfig,
    user_agent: String,
    net: u32,
    relay: bool,
    time_per_block: Duration,
    proto_tick_interval: Duration,
    ping_interval: Duration,
}

#[derive(Clone)]
struct NeoFSBlockFetcherConfig {
    enabled: bool,
    bqueue_size: usize,
}
...
struct Server {
    config: Config,
    id: u32,
    transports: Vec<Transporter>,
    discovery: Discoverer,
    chain: Arc<dyn Ledger>,
    bqueue: Arc<Mutex<BlockQueue>>,
    bsync_queue: Arc<Mutex<BlockQueue>>,
    bfetcher_queue: Arc<Mutex<BlockQueue>>,
    mempool: Arc<Mutex<Mempool>>,
    notary_request_pool: Option<Arc<Mutex<Mempool>>>,
    extensible_pool: Arc<Mutex<ExtensiblePool>>,
    notary_feer: Option<NotaryFeer>,
    block_fetcher: Option<BlockFetcher>,
    services: Arc<RwLock<HashMap<String, Arc<dyn Service>>>>,
    extens_handlers: Arc<RwLock<HashMap<String, Box<dyn Fn(&Extensible) -> Result<(), Box<dyn Error>>>>>>,
    tx_callback: Arc<RwLock<Option<Box<dyn Fn(&Transaction)>>>>,
    txin: UnboundedSender<Transaction>,
    txin_map: Arc<Mutex<HashMap<Uint256, ()>>>,
    peers: Arc<Mutex<HashMap<Peer, bool>>>,
    last_requested_block: AtomicU32,
    last_requested_header: AtomicU32,
    register: UnboundedSender<Peer>,
    unregister: UnboundedSender<PeerDrop>,
    handshake: UnboundedSender<Peer>,
    quit: Arc<AtomicBool>,
    relay_fin: Arc<AtomicBool>,
    run_fin: Arc<AtomicBool>,
    broadcast_tx_fin: Arc<AtomicBool>,
    run_proto_fin: Arc<AtomicBool>,
    block_fetcher_fin: Arc<AtomicBool>,
    transactions: UnboundedSender<Transaction>,
    sync_reached: Arc<AtomicBool>,
    state_sync: Arc<dyn StateSync>,
    log: Arc<Mutex<Logger>>,
    started: Arc<AtomicBool>,
    tx_handler_loop_wg: Arc<Mutex<()>>,
}
...
impl Server {
    ...
    fn new(config: Config, chain: Arc<dyn Ledger>, state_sync: Arc<dyn StateSync>, log: Arc<Mutex<Logger>>) -> Result<Self, Box<dyn Error>> {
        let (txin_tx, txin_rx) = unbounded_channel();
        let (register_tx, register_rx) = unbounded_channel();
        let (unregister_tx, unregister_rx) = unbounded_channel();
        ...
        let server = Server {
            ...
            id: Self::random_id(),
            transports: vec![],
            discovery: Discoverer::new(config.seeds.clone(), config.dial_timeout, None),
            chain: chain.clone(),
            bqueue: Arc::new(Mutex::new(BlockQueue::new(chain.clone(), log.clone()))),
            bsync_queue: Arc::new(Mutex::new(BlockQueue::new(state_sync.clone(), log.clone()))),
            bfetcher_queue: Arc::new(Mutex::new(BlockQueue::new(chain.clone(), log.clone()))),
            mempool: Arc::new(Mutex::new(Mempool::new())),
            notary_request_pool: None,
            extensible_pool: Arc::new(Mutex::new(ExtensiblePool::new(chain.clone(), config.extensible_pool_size))),
            notary_feer: None,
            block_fetcher: None,
            services: Arc::new(RwLock::new(HashMap::new())),
            extens_handlers: Arc::new(RwLock::new(HashMap::new())),
            tx_callback: Arc::new(RwLock::new(None)),
            txin: txin_tx,
            txin_map: Arc::new(Mutex::new(HashMap::new())),
            peers: Arc::new(Mutex::new(HashMap::new())),
            last_requested_block: AtomicU32::new(0),
            last_requested_header: AtomicU32::new(0),
            register: register_tx,
            unregister: unregister_tx,
            handshake: handshake_tx,
            quit: Arc::new(AtomicBool::new(false)),
            relay_fin: Arc::new(AtomicBool::new(false)),
            run_fin: Arc::new(AtomicBool::new(false)),
            broadcast_tx_fin: Arc::new(AtomicBool::new(false)),
            run_proto_fin: Arc::new(AtomicBool::new(false)),
            block_fetcher_fin: Arc::new(AtomicBool::new(false)),
            transactions: transactions_tx,
            sync_reached: Arc::new(AtomicBool::new(false)),
            state_sync: state_sync.clone(),
            log: log.clone(),
            started: Arc::new(AtomicBool::new(false)),
            tx_handler_loop_wg: Arc::new(Mutex::new(())),
        };
    ...
    async fn start(&self) {
        if self.started.compare_and_swap(false, true, Ordering::SeqCst) {
            info!(self.log.lock().await, "node server already started");
            return;
        }
        info!(self.log.lock().await, "node started");

        self.try_start_services().await;
        self.init_stale_mem_pools().await;

        let tx_threads = optimal_num_of_threads();
        for _ in 0..tx_threads {
            let server = self.clone();
            tokio::spawn(async move {
                server.tx_handler_loop().await;
            });
        }

        let server = self.clone();
        tokio::spawn(async move {
            server.broadcast_tx_loop().await;
        });

        let server = self.clone();
        tokio::spawn(async move {
            server.relay_blocks_loop().await;
        });

        let server = self.clone();
        tokio::spawn(async move {
            server.bqueue.lock().await.run().await;
        });

        let server = self.clone();
        tokio::spawn(async move {
            server.bsync_queue.lock().await.run().await;
        });

        let server = self.clone();
        tokio::spawn(async move {
            server.bfetcher_queue.lock().await.run().await;
        });

        if self.config.neofs_block_fetcher_cfg.enabled {
            if let Some(block_fetcher) = &self.block_fetcher {
                if let Err(err) = block_fetcher.start().await {
                    error!(self.log.lock().await, "skipping NeoFS BlockFetcher: {:?}", err);
                }
            }
        }
    ...
    async fn shutdown(&self) {
        ...
        info!(self.log.lock().await, "shutting down server");

        if self.config.neofs_block_fetcher_cfg.enabled {
            if let Some(block_fetcher) = &self.block_fetcher {
                block_fetcher.shutdown().await;
            }
        }
        ...
        for peer in self.get_peers(None).await {
            peer.disconnect(ServerError::ServerShutdown).await;
        }

        self.bqueue.lock().await.discard().await;
        self.bsync_queue.lock().await.discard().await;
        self.bfetcher_queue.lock().await.discard().await;
        ...
        if self.chain.p2p_sig_extensions_enabled() {
            if let Some(notary_request_pool) = &self.notary_request_pool {
                notary_request_pool.lock().await.stop_subscriptions().await;
            }
        }

        self.quit.store(true, Ordering::SeqCst);
        self.relay_fin.store(true, Ordering::SeqCst);
        self.run_fin.store(true, Ordering::SeqCst);
        self.broadcast_tx_fin.store(true, Ordering::SeqCst);
        self.run_proto_fin.store(true, Ordering::SeqCst);
        self.block_fetcher_fin.store(true, Ordering::SeqCst);

        self.tx_handler_loop_wg.lock().await;
    }

    async fn add_service(&self, svc: Arc<dyn Service>) {
        let mut services = self.services.write().unwrap();
        services.insert(svc.name(), svc);
    }

    async fn get_block_queue(&self) -> Arc<Mutex<BlockQueue>> {
        self.bqueue.clone()
    }

    async fn add_extensible_service(&self, svc: Arc<dyn Service>, category: String, handler: Box<dyn Fn(&Extensible) -> Result<(), Box<dyn Error>>>) {
        let mut extens_handlers = self.extens_handlers.write().unwrap();
        extens_handlers.insert(category.clone(), handler);

        let mut services = self.services.write().unwrap();
        services.insert(svc.name(), svc);
    }

    async fn add_consensus_service(&self, svc: Arc<dyn Service>, handler: Box<dyn Fn(&Extensible) -> Result<(), Box<dyn Error>>>, tx_callback: Box<dyn Fn(&Transaction)>) {
        let mut tx_callback_lock = self.tx_callback.write().unwrap();
        *tx_callback_lock = Some(tx_callback);

        self.add_extensible_service(svc, "consensus".to_string(), handler).await;
    }

    async fn del_service(&self, svc: Arc<dyn Service>) {
        let mut services = self.services.write().unwrap();
        services.remove(&svc.name());
    }

    async fn del_extensible_service(&self, svc: Arc<dyn Service>, category: String) {
        let mut extens_handlers = self.extens_handlers.write().unwrap();
        extens_handlers.remove(&category);

        self.del_service(svc).await;
    }

    async fn del_consensus_service(&self, svc: Arc<dyn Service>) {
        let mut tx_callback_lock = self.tx_callback.write().unwrap();
        *tx_callback_lock = None;

        self.del_extensible_service(svc, "consensus".to_string()).await;
    }

    async fn get_notary_pool(&self) -> Option<Arc<Mutex<Mempool>>> {
        self.notary_request_pool.clone()
    }

    async fn unconnected_peers(&self) -> Vec<String> {
        self.discovery.unconnected_peers().await
    }

    async fn bad_peers(&self) -> Vec<String> {
        self.discovery.bad_peers().await
    }

    async fn connected_peers(&self) -> Vec<PeerInfo> {
        let peers = self.peers.lock().await;
        peers.iter().map(|(peer, _)| PeerInfo {
            address: peer.peer_addr().to_string(),
            user_agent: peer.version().user_agent.clone(),
            height: peer.last_block_index(),
        }).collect()
    }

    async fn run(&self) {
        let peer_check_time = self.config.time_per_block * PEER_TIME_FACTOR;
        let mut addr_check_timeout = false;
        let mut addr_timer = tokio::time::interval(peer_check_time);
        let mut peer_timer = tokio::time::interval(self.config.proto_tick_interval);

        loop {
            let net_size = self.discovery.network_size().await;
            let optimal_n = self.discovery.get_fan_out() * 2;
            let peer_n = self.handshaked_peers_count().await;
            let mut peer_t = peer_check_time;

            if peer_n < self.config.min_peers {
                self.discovery.request_remote(self.config.attempt_conn_peers).await;
                peer_t = self.config.proto_tick_interval;
            } else if self.config.min_peers > 0 && optimal_n > peer_n && optimal_n < self.config.max_peers && optimal_n < net_size {
                self.discovery.request_remote(min(self.config.attempt_conn_peers, optimal_n - peer_n)).await;
            }

            if addr_check_timeout || self.discovery.pool_count().await < self.config.attempt_conn_peers {
                self.broadcast_hp_message(NewMessage::new(CMDGetAddr, NullPayload::new())).await;
                addr_check_timeout = false;
            }

            tokio::select! {
                _ = self.quit.load(Ordering::SeqCst) => {
                    break;
                }
                _ = addr_timer.tick() => {
                    addr_check_timeout = true;
                }
                _ = peer_timer.tick() => {
                    peer_t = peer_check_time;
                }
                Some(peer) = self.register.recv() => {
                    let mut peers = self.peers.lock().await;
                    peers.insert(peer.clone(), true);
                    let peer_count = self.peer_count().await;
                    info!(self.log.lock().await, "new peer connected: {:?}", peer.remote_addr());

                    if peer_count > self.config.max_peers {
                        let mut rng = rand::thread_rng();
                        let random_peer = peers.keys().nth(rng.gen_range(0..peers.len())).unwrap().clone();
                        random_peer.disconnect(ServerError::MaxPeers).await;
                    }
                }
                Some(drop) = self.unregister.recv() => {
                    let mut peers = self.peers.lock().await;
                    if peers.remove(&drop.peer).is_some() {
                        if matches!(drop.reason.downcast_ref::<ServerError>(), Some(ServerError::InvalidInvType) | Some(ServerError::StateMismatch) | Some(ServerError::BlocksRequestFailed)) {
                            warn!(self.log.lock().await, "peer disconnected: {:?}", drop.peer.remote_addr());
                        } else {
                            info!(self.log.lock().await, "peer disconnected: {:?}", drop.peer.remote_addr());
                        }

                        if matches!(drop.reason.downcast_ref::<ServerError>(), Some(ServerError::IdenticalID)) {
                            self.discovery.register_self(drop.peer.clone()).await;
                        } else {
                            self.discovery.unregister_connected(drop.peer.clone(), matches!(drop.reason.downcast_ref::<ServerError>(), Some(ServerError::AlreadyConnected))).await;
                        }
                    }
                }
                Some(peer) = self.handshake.recv() => {
                    let version = peer.version();
                    info!(self.log.lock().await, "started protocol: {:?}", peer.remote_addr());

                    self.discovery.register_good(peer.clone()).await;
                    self.try_init_state_sync().await;
                    self.try_start_services().await;
                }
                _ = self.block_fetcher_fin.load(Ordering::SeqCst) => {
    ...
    async fn run_proto(&self) {
        let mut ping_timer = tokio::time::interval(self.config.ping_interval);

        loop {
            let prev_height = self.chain.block_height().await;
            ...
            tokio::select! {
                ...
                _ = ping_timer.tick() => {
                    if self.chain.block_height().await == prev_height {
                        self.broadcast_message(NewMessage::new(CMDPing, Ping::new(self.chain.block_height().await, self.id))).await;
                    }
                }
            }
        }
    ...
    async fn try_start_services(&self) {
        if self.sync_reached.load(Ordering::SeqCst) {
            return;
        }

        if self.is_in_sync().await && self.sync_reached.compare_and_swap(false, true, Ordering::SeqCst) {
            info!(self.log.lock().await, "node reached synchronized state, starting services");

            if self.chain.p2p_sig_extensions_enabled() {
                if let Some(notary_request_pool) = &self.notary_request_pool {
                    notary_request_pool.lock().await.run_subscriptions().await;
                }
            }
    ...
    async fn subscribe_for_notary_requests(&self, ch: UnboundedSender<MempoolEvent>) {
        if !self.chain.p2p_sig_extensions_enabled() {
            panic!("P2PSigExtensions are disabled");
        }

        if let Some(notary_request_pool) = &self.notary_request_pool {
            notary_request_pool.lock().await.subscribe_for_transactions(ch).await;
        }
    }

    async fn unsubscribe_from_notary_requests(&self, ch: UnboundedSender<MempoolEvent>) {
        if !self.chain.p2p_sig_extensions_enabled() {
            panic!("P2PSigExtensions are disabled");
        }
