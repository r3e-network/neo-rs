...
use crate::fakechain::{FakeChain, FakeStateSync};
use crate::random;
use crate::config::{self, ProtocolConfiguration};
use crate::consensus::Service;
use crate::core::{self, block, interop, mpt, transaction};
...
struct FakeConsensus {
    started: AtomicBool,
    stopped: AtomicBool,
    payloads: Mutex<Vec<payload::Extensible>>,
    txlock: Mutex<()>,
    txs: Mutex<Vec<transaction::Transaction>>,
}

impl Service for FakeConsensus {
    fn name(&self) -> &str {
        "fake"
    }
    ...
    fn on_payload(&self, p: payload::Extensible) -> Result<(), Box<dyn std::error::Error>> {
        let mut payloads = self.payloads.lock().unwrap();
        payloads.push(p);
        Ok(())
    }

    fn on_transaction(&self, tx: transaction::Transaction) {
        let _lock = self.txlock.lock().unwrap();
        let mut txs = self.txs.lock().unwrap();
        txs.push(tx);
    }

    fn get_payload(&self, _h: util::Uint256) -> Option<payload::Extensible> {
        panic!("implement me");
    }
}
...
#[test]
fn test_new_server() {
    let bc = Arc::new(FakeChain {
        blockchain: config::Blockchain {
            protocol_configuration: ProtocolConfiguration {
                p2p_state_exchange_extensions: true,
    ...
    let (s, err) = new_server_from_constructors(ServerConfig::default(), bc.clone(), Arc::new(FakeStateSync::default()), None, new_fake_transp, new_test_discovery);
    assert!(err.is_some());

    let mut s = new_test_server(ServerConfig { min_peers: -1, ..Default::default() });
    assert!(s.id() != 0);
    assert_eq!(default_min_peers, s.server_config.min_peers);
    assert_eq!(default_max_peers, s.server_config.max_peers);
    assert_eq!(default_attempt_conn_peers, s.server_config.attempt_conn_peers);

    let cfg = ServerConfig {
        min_peers: 1,
        max_peers: 2,
        attempt_conn_peers: 3,
        ..Default::default()
    };
    s = new_test_server(cfg);
    assert!(s.id() != 0);
    assert_eq!(1, s.server_config.min_peers);
    assert_eq!(2, s.server_config.max_peers);
    assert_eq!(3, s.server_config.attempt_conn_peers);
}

#[test]
fn test_server_start_and_shutdown() {
    let mut s = new_test_server(ServerConfig::default());

    s.start();
    let p = new_local_peer(&s);
    s.register.send(p.clone()).unwrap();
    assert!(s.started.load(Ordering::SeqCst));
    assert!(s.transports[0].started.load(Ordering::SeqCst));
    assert!(s.tx_callback.is_none());

    s.shutdown();
    assert!(!s.started.load(Ordering::SeqCst));
    assert!(s.transports[0].closed.load(Ordering::SeqCst));
    let err = p.dropped_with.load().unwrap();
    assert_eq!(err, err_server_shutdown);

    let cons = Arc::new(FakeConsensus {
    ...
    s.add_consensus_service(cons.clone(), cons.on_payload, cons.on_transaction);

    s.start();
    let p = new_local_peer(&s);
    s.register.send(p.clone()).unwrap();
    assert!(cons.started.load(Ordering::SeqCst));
    assert!(s.started.load(Ordering::SeqCst));

    s.shutdown();
    assert!(!s.started.load(Ordering::SeqCst));
    assert!(cons.stopped.load(Ordering::SeqCst));
...
#[test]
fn test_server_register_peer() {
    const PEER_COUNT: usize = 3;

    let mut s = new_test_server(ServerConfig { max_peers: 2, ..Default::default() });
    let mut ps = Vec::with_capacity(PEER_COUNT);
    for i in 0..PEER_COUNT {
        let mut p = new_local_peer(&s);
        p.netaddr.port = i as u16 + 1;
        p.version = Some(payload::Version {
            nonce: i as u32,
            user_agent: b"fake".to_vec(),
            ..Default::default()
        });
        ps.push(p);
    }
    ...
    s.start();
    s.register.send(ps[0].clone()).unwrap();
    assert_eq!(1, s.peer_count());
    s.handshake.send(ps[0].clone()).unwrap();

    s.register.send(ps[1].clone()).unwrap();
    s.handshake.send(ps[1].clone()).unwrap();
    assert_eq!(2, s.peer_count());

    assert_eq!(0, s.discovery.unconnected_peers().len());
    s.register.send(ps[2].clone()).unwrap();
    assert!(s.discovery.unconnected_peers().len() > 0);

    let mut index = -1;
    let addrs = s.discovery.unconnected_peers();
    for addr in addrs {
        for (j, p) in ps.iter().enumerate() {
            if p.peer_addr().to_string() == addr {
                index = j as i32;
                break;
            }
        }
    ...
    assert!(index >= 0);
    let err = ps[index as usize].dropped_with.load().unwrap();
    assert_eq!(err, err_max_peers);

    index = (index + 1) % PEER_COUNT as i32;
    s.unregister.send(peer_drop(ps[index as usize].clone(), err_identical_id)).unwrap();
    assert!(s.bad_peers().contains(&ps[index as usize].peer_addr().to_string()));
}

#[test]
fn test_get_blocks_by_index() {
    test_get_blocks_by_index(CMD_GET_BLOCK_BY_INDEX);
}

fn test_get_blocks_by_index(cmd: CommandType) {
    let mut s = new_test_server(ServerConfig { user_agent: "/test/".to_string(), ..Default::default() });
    let start = s.chain.block_height();
    if cmd == CMD_GET_HEADERS {
        s.state_sync.request_headers.store(true, Ordering::SeqCst);
    }
    let mut ps = Vec::with_capacity(10);
    let mut expects_cmd = vec![cmd; 10];
    let mut expected_height = vec![vec![start + 1]; 10];
    for i in 0..10 {
        let mut p = new_local_peer(&s);
        p.message_handler = Some(Box::new(move |t, msg| {
            assert_eq!(expects_cmd[i], msg.command);
            if expects_cmd[i] == cmd {
                let p = msg.payload.downcast_ref::<payload::GetBlockByIndex>().unwrap();
                assert!(expected_height[i].contains(&p.index_start));
                expects_cmd[i] = CMD_PONG;
            } else if expects_cmd[i] == CMD_PONG {
                expects_cmd[i] = cmd;
            }
        }));
        ps.push(p);
    ...
    let mut nonce = 0;
    let check_ping_respond = |t, peer_index, peer_height, hs: &[u32]| {
        nonce += 1;
        expected_height[peer_index] = hs.to_vec();
        s.handle_ping(&ps[peer_index], payload::Ping::new(peer_height, nonce)).unwrap();
    };

    check_ping_respond(0, 5000, &[1]);
    check_ping_respond(1, 5000, &[1 + payload::MAX_HASHES_COUNT]);
    check_ping_respond(2, 5000, &[1 + 2 * payload::MAX_HASHES_COUNT]);
    check_ping_respond(3, 5000, &[1 + 3 * payload::MAX_HASHES_COUNT]);

    s.chain.block_height.store(2123, Ordering::SeqCst);

    check_ping_respond(5, 5000, &[2124]);
    check_ping_respond(6, 5000, &[2624]);
    check_ping_respond(7, 3100, &[2124]);
    check_ping_respond(8, 5000, &[3124]);
    check_ping_respond(9, 5000, &[3624]);
    check_ping_respond(1, 5000, &[2124, 2624, 3124, 3624]);
    check_ping_respond(2, 5000, &[2124, 2624, 3124, 3624]);
    check_ping_respond(3, 5000, &[2124, 2624, 3124, 3624]);
}

#[test]
fn test_send_version() {
    let s = new_test_server(ServerConfig { user_agent: "/test/".to_string(), ..Default::default() });
    let p = new_local_peer(&s);
    s.transports[0].accept();
    p.message_handler = Some(Box::new(move |t, msg| {
        let (_, prt) = s.transports[0].host_port();
        let port = prt.parse::<u16>().unwrap();
        assert_eq!(CMD_VERSION, msg.command);
        let version = msg.payload.downcast_ref::<payload::Version>().unwrap();
        assert_ne!(0, version.nonce);
        assert_eq!(1, version.capabilities.len());
        assert_eq!(vec![capability::Capability {
            type_: capability::TCP_SERVER,
            data: capability::Server { port },
        }], version.capabilities);
        assert_eq!(0, version.version);
        assert_eq!(b"/test/".to_vec(), version.user_agent);
    }));

    p.send_version().unwrap();
}

#[test]
fn test_verack_after_handle_version_cmd() {
    let s = new_test_server(ServerConfig::default());
    let p = new_local_peer(&s);
    let na = TcpListener::bind("0.0.0.0:3000").unwrap();
    p.netaddr = na.local_addr().unwrap();

    p.message_handler = Some(Box::new(move |t, msg| {
        assert_eq!(CMD_VERACK, msg.command);
    }));
    let capabilities = vec![
        capability::Capability {
            type_: capability::FULL_NODE,
            data: capability::Node { start_height: 0 },
        },
        capability::Capability {
            type_: capability::TCP_SERVER,
            data: capability::Server { port: 3000 },
        },
    ];
    let version = payload::Version::new(0, 1337, b"/NEO-GO/".to_vec(), capabilities);

    s.handle_version_cmd(&p, version).unwrap();
}

#[test]
fn test_server_not_sends_verack() {
    let s = new_test_server(ServerConfig { max_peers: 10, net: 56753, ..Default::default() });
    let p = new_local_peer(&s);
    let p2 = new_local_peer(&s);
    s.id = 1;
    s.start();

    let na = TcpListener::bind("0.0.0.0:3000").unwrap();
    p.netaddr = na.local_addr().unwrap();
    p2.netaddr = na.local_addr().unwrap();
    s.register.send(p.clone()).unwrap();
    ...
    let mut version = payload::Version::new(56753, 1, b"/NEO-GO/".to_vec(), capabilities.clone());
    let err = s.handle_version_cmd(&p, version.clone()).unwrap_err();
    assert_eq!(err, err_identical_id);

    version.nonce = 2;
    version.magic = 56752;
    let err = s.handle_version_cmd(&p, version.clone()).unwrap_err();
    assert_eq!(err, err_invalid_network);

    version.magic = 56753;
    s.handle_version_cmd(&p, version.clone()).unwrap();
    p.handle_version_ack().unwrap();
    assert!(p.handshaked());

    s.register.send(p2.clone()).unwrap();
    let err = s.handle_version_cmd(&p2, version).unwrap_err();
    assert_eq!(err, err_already_connected);
}

impl Server {
    fn test_handle_message(&self, t: &mut testing::T, p: Option<Peer>, cmd: CommandType, pl: payload::Payload) -> &Self {
        let p = p.unwrap_or_else(|| {
            let mut p = new_local_peer(self);
            p.handshaked = 1;
            p
        });
        let msg = Message::new(cmd, pl);
        self.handle_message(&p, msg).unwrap();
        self
    }
}

fn start_test_server(t: &mut testing::T, protocol_cfg: Option<fn(&mut config::Blockchain)>) -> Arc<Server> {
    let srv_cfg = ServerConfig { user_agent: "/test/".to_string(), ..Default::default() };
    let s = if let Some(protocol_cfg) = protocol_cfg {
        new_test_server_with_custom_cfg(srv_cfg, protocol_cfg)
    } else {
        new_test_server(srv_cfg)
    };
    s.start();
    t.cleanup(move || {
        s.shutdown();
...
fn start_with_cleanup(t: &mut testing::T, s: Arc<Server>) {
    s.start();
    t.cleanup(move || {
        s.shutdown();
    });
...
#[test]
fn test_block() {
    let s = start_test_server(&mut testing::T::default());

    s.chain.block_height.store(12344, Ordering::SeqCst);
    assert_eq!(12344, s.chain.block_height());

    let mut b = block::Block::new(false);
    b.index = 12345;
    s.test_handle_message(&mut testing::T::default(), None, CMD_BLOCK, b);
    assert_eq!(12345, s.chain.block_height());
}
...
fn test_consensus() {
    ...
    s.chain.block_height.store(4, Ordering::SeqCst);
    let p = new_local_peer(&s);
    p.handshaked = 1;
    s.register.send(p.clone()).unwrap();
    assert_eq!(1, s.peer_count());

    let new_consensus_message = |start, end| {
        let mut pl = payload::Extensible::new();
        pl.category = payload::CONSENSUS_CATEGORY;
        pl.valid_block_start = start;
        pl.valid_block_end = end;
        Message::new(CMD_EXTENSIBLE, pl)
    };

    s.chain.verify_witness = Box::new(|| Err("invalid".into()));
    let msg = new_consensus_message(0, s.chain.block_height() + 1);
    assert!(s.handle_message(&p, msg).is_err());

    s.chain.verify_witness = Box::new(|| Ok(0));
    let msg = new_consensus_message(0, s.chain.block_height() + 1);
    s.handle_message(&p, msg).unwrap();
    assert!(cons.payloads.lock().unwrap().contains(&msg.payload.downcast_ref::<payload::Extensible>().unwrap()));

    let msg = new_consensus_message(0, s.chain.block_height());
    s.handle_message(&p, msg).unwrap();
    assert!(!cons.payloads.lock().unwrap().contains(&msg.payload.downcast_ref::<payload::Extensible>().unwrap()));
    ...
    let msg = new_consensus_message(s.chain.block_height() + 1, s.chain.block_height() + 2);
    assert!(s.handle_message(&p, msg).is_err());
}
...
fn test_transaction() {
    ...
    let tx = new_dummy_tx();
    s.request_tx(tx.hash());
    let p = new_local_peer(&s);
    p.is_full_node = true;
    p.message_handler = Some(Box::new(move |t, msg| {
        if msg.command == CMD_INV {
            let inv = msg.payload.downcast_ref::<payload::Inventory>().unwrap();
            assert_eq!(payload::TX_TYPE, inv.type_);
            assert_eq!(vec![tx.hash()], inv.hashes);
        }
    }));
    s.register.send(p.clone()).unwrap();

    s.test_handle_message(&mut testing::T::default(), None, CMD_TX, tx);
    assert!(cons.txs.lock().unwrap().contains(&tx));
