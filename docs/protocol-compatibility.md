...
| `9f4795ab` / #4571 | Adds `HF_Huyao` after `HF_Gorgon`. Empty `Hardforks: {}` now backfills every known hardfork through Huyao at height 0. | `neo-primitives::Hardfork::HfHuyao`, `neo-config::HardforkSchedule`, and config tests define Huyao and the C# empty-object backfill rule. Built-in MainNet/TestNet presets stay explicit operational schedules unless a loaded network config schedules later forks. |
