...
Practical checks and routines for running `neo-node` in production.
...
- Check sync status with `getblockcount` and compare height to trusted explorers/seeds.
- Inspect peer count with `getconnectioncount`; investigate sustained zero-peer periods.
- Review logs: `journalctl -u neo-node -p warning..alert --since "1 hour ago"` (or tail the path configured in `[logging]` / `/data/Logs` depending on your deployment).
...
## Data and storage
- Location: see `--storage-path` or TOML `storage.path` / `storage.data_dir` (`/var/neo/...` recommended; `/data/...` in Docker).
...
- Network markers: the node writes `NETWORK_MAGIC` into the data dir; ensure you use a matching directory per network.
...
- When exposing RPC, configure `[rpc]` in TOML to bind to loopback, disable CORS, require auth, and disable wallet-mutating methods. Put any public endpoint behind TLS, auth, and rate limits.
- Use `getversion` / `getblockcount` RPC probes for liveness and readiness checks.
...
- For Docker/compose, update `NEO_NETWORK`, `NEO_CONFIG`, `NEO_STORAGE`, or the mounted TOML, then run `docker compose up -d` to re-create the container.
...
If the node is running with RPC authentication, keep the validator on the node host so it can reach `http://127.0.0.1:10332` without a proxy. You can either allow an unauthenticated loopback interface or wrap the validator in a helper that injects the `Authorization: Basic` header before each request; the script itself uses `http.client` so it must see the same request headers you would hit the RPC with manually.
...
- If RPC is overloaded: place a reverse proxy with rate limits in front of the node and consider moving RPC to a dedicated instance.
- If disk is full: expand the volume, prune old backups/logs, and keep RocksDB on fast, durable storage.
- If plugin state looks off: inspect the TOML/plugin config on disk. The `listplugins` RPC is disabled by default in production; only enable it behind auth/proxy if you need remote visibility.
