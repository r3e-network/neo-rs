...
| Priority | Change | Benefit |
|----------|--------|---------|
| P0 | Staged sync pipeline | 3-5x sync speed, crash resume |
| Done | Import queue boundary with bounded concurrent `check` | Reusable preverification surface; concrete downloader integration remains next |
| P1 | Commit policy per stage | Tunable memory/i-o |
...
Current status: the shared trait and bounded queue exist. The remaining work is
to integrate block downloaders/sync stages with the queue while preserving the
single ordered import path.
...
block downloader — blocks arrive via inventory messages.
...
```rust
// Proposed BlockDownloader
pub trait BlockDownloader: Stream<Item = Vec<Block>> + Send + Sync {
    fn with_concurrency(n: usize) -> Self;
    fn with_peer_bias(peer: PeerId) -> Self;
}
// Concurrent requests, retry on different peers, stream interface
```
...
`NativeContractProvider` global seam with `install()`.
...
2. Make `NativeContractProvider` part of `NodeBuilder` config instead of global
   `install()` seam.
...
|--------|-------|---------|-------------|------------|--------|
| Staged sync pipeline | ★★★★★ | ★★ | ★★★★ | ★★★★ | Large |
| Hot/Cold/Static tiering | ★★★ | ★★★★★ | ★★★ | ★★★ | Medium |
| Import queue + concurrent verify | ★★★★ | - | ★★★ | ★★★ | Boundary done; integration medium |
| Compact derive macro | ★★ | ★★★★ | - | ★★ | Small |
| Task supervision | - | - | ★★★★★ | ★★ | Done |
| BlockDownloader as Stream | ★★★ | - | ★★ | ★★★ | Medium |
...
3. **Block import queue with concurrent verification** — reusable runtime boundary implemented; downloader/sync integration remains next.
4. **Hot/Cold/Static tiering** (medium, big storage win)
5. **BlockDownloader as Stream** (medium, sync speed)
6. **Staged sync pipeline** (large, biggest overall impact)
