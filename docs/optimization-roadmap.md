...
| Bounded optimistic header/standard-transaction signature pool | **OPT-IN FOUNDATION 2026-07-20** — typed receipts, ordered publication/commit fences, staged-batch rollback, synchronous fallback, fixed-cardinality counters; no end-to-end node speedup established | `neo-blockchain::pipeline::signature_verification`; `reports/performance/optimistic-signature-verification-20260720.md`; standard transaction jobs run only for deferred verified sync batches, while non-standard witnesses remain canonical |
