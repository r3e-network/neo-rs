...
| Bounded optimistic header signature pool | **OPT-IN FOUNDATION 2026-07-20** — bounded parallel header witness verification, ordered receipt publication, synchronous fallback, fixed-cardinality counters; no end-to-end node speedup established | `neo-blockchain::pipeline::signature_verification`; `reports/performance/optimistic-signature-verification-20260720.md`; canonical block import keeps Neo N3 transaction acceptance semantics |
