...
| `neo-contract` | [neo-contract-parity](./neo-contract-parity.md) | 🟧 Manifest/ABI/NEF validation complete; runtime host enforces storage contexts + call flags for storage/notify and now exposes storage iterator handles (KeysOnly/ValuesOnly/RemovePrefix/Backwards). | `System.Storage.Find` is not yet wired to VM syscalls and still lacks `DeserializeValues`/`PickField*`; call flags aren’t propagated through contract invocation/deploy flows; native contract plumbing missing. | Wire the iterator APIs into `System.Storage.Find/Next`, add deserialization/pick semantics, and propagate call flags via `System.Contract.Call`. | `cargo +nightly test --manifest-path neo-contract/Cargo.toml` (fails early until `neo-core` drops edition 2024) |
