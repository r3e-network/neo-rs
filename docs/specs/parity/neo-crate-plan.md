...
| `neo-contract` | [neo-contract-parity](./neo-contract-parity.md) | 🟧 Manifest/ABI/NEF parity largely in place (NEF + manifest validated); runtime host enforces storage contexts, call flags for storage/notify, and exposes iterator handles feeding VM syscalls. | `System.Storage.Find` lacks `DeserializeValues`/`PickField*`; call flags not yet propagated through contract invocation/deploy flows; native contract plumbing missing. | Implement StorageFind deserialization/pick semantics and thread call flags through `System.Contract.Call`/deployment paths. | `cargo +nightly test --manifest-path neo-contract/Cargo.toml` (fails early until `neo-core` drops edition 2024) |
...
| `neo-vm` | [neo-vm-parity](./neo-vm-parity.md) | 🟧 Opcode surface expanded (stack ops, iterators, storage syscalls, call flag query). | No StackItem hierarchy, gas accounting, TRY/CATCH, or iterator registry; storage iterators currently emit ad-hoc arrays. | Port StackItem model + gas accounting, then swap iterator values to real StackItems. | `cargo test --manifest-path neo-vm/Cargo.toml` |
