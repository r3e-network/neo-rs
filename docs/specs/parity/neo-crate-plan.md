...
| `neo-contract` | [neo-contract-parity](./neo-contract-parity.md) | 🟧 Manifest/ABI/NEF validation complete; runtime host enforces storage contexts + call flags for storage/notify and Storage.Find/Next now flows through the VM syscall path with deterministic iterator ordering + array results. | BinarySerializer-backed `DeserializeValues`/`PickField*` semantics are still missing and call flags aren’t propagated through contract invocation/deploy flows; native contract plumbing remains TODO. | Implement find-option deserialization/pick behaviour and thread call flags via `System.Contract.Call`, then tackle native contract plumbing. | `cargo +nightly test --manifest-path neo-contract/Cargo.toml` (fails early until `neo-core` drops edition 2024) |
