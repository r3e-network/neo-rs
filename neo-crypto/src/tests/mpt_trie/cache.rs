...
impl MptStoreSnapshot for DeferredBatchProbeStore {
    ...
    fn try_get_nodes_with_source(
        &self,
        keys: &[Vec<u8>],
    ) -> MptResult<Vec<crate::mpt_trie::MptStoreLookup<Node>>> {
        if keys.is_empty() {
            return Ok(Vec::new());
        }
        self.batch_reads
        ...
        {
            return Err(crate::mpt_trie::MptError::storage(
        ...
        Ok(keys
            .iter()
            .map(|key| {
                crate::mpt_trie::MptStoreLookup::Backing(
                    data.get(key).map(|bytes| deserialize_node(bytes)),
                )
            })
            .collect())
    }
