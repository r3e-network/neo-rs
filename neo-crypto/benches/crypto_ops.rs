...
use neo_crypto::{
    Crypto, ECDsa, ECCurve, Ed25519Crypto, HashAlgorithm, NeoHash, Secp256k1Crypto, Secp256r1Crypto,
};
...
fn bench_secp256r1_sign(c: &mut Criterion) {
    ...
    for size in [32, 256, 1024].iter() {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let result = Secp256r1Crypto::sign(
                        black_box(&message),
...
fn bench_secp256k1_sign(c: &mut Criterion) {
    ...
    for size in [32, 256, 1024].iter() {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let result = Secp256k1Crypto::sign(
                        black_box(&message),
                        black_box(&private_key),
                    ).unwrap();
                    black_box(result)
