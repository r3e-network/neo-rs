...
#[derive(Copy, Clone, Debug, PartialEq, Eq, Hash, FromPrimitive, EnumIter, EnumString)]
#[repr(u8)]
pub enum OpCode {
	PushInt8 = 0x00,
	PushInt16 = 0x01,
...
impl OpCode {
	pub fn operand_size(&self) -> Result<u8, VMError> {
		match OPERAND_SIZES.get(self) {
			Some(size) => Ok(size.size),
			None => Err(VMError::InvalidOpCode(*self)),
		}
	}

	pub fn operand_prefix(&self) -> Result<u8, VMError> {
		match OPERAND_SIZES.get(self) {
