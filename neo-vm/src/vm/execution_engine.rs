...
use crate::vm::{
    EvaluationStack, ExecutionContext, ExecutionEngineLimits, Instruction, OpCode, VMError, VMState,
};
...
impl NeoVm {
    ...
    fn execute_instr(&mut self, instr: Instruction) -> Result<VMState, VMError> {
        match instr.opcode {
            ...
            | OpCode::PushInt64
            | OpCode::PushInt128
            | OpCode::PushInt256 => self.push(Rc::new(RefCell::new(Integer::new(
                &BigInt::from_be_bytes(instr.operand.as_slice()),
            )))),
            ...
            OpCode::PushA => {
                let position = (self.current_context?.get_mut().instruction_pointer as i32)
                    ...
                    .unwrap();
                if position < 0 || position > self.current_context?.get_mut().script().len() as i32
                {
            ...
            OpCode::CallL => self.execute_call(
                (self.current_context?.get_mut().instruction_pointer + instr.token_i32()) as i32,
            ),
            OpCode::CallA => {
            ...
            OpCode::EndFinally => {
                ...
                if self.uncaught_exception.is_none() {
                    self.current_context?.get_mut().instruction_pointer =
                        current_try.get_mut().EndPointer;
                } else {
            ...
            OpCode::Ret => {
                ...
                let stack_eval = match self.invocation_stack.len() == 0 {
                    ...
                    false => {
                        self.invocation_stack.last().unwrap().borrow().evaluation_stack().clone()
                    }
                };
                // }
                ...
                if context_pop.borrow().evaluation_stack() != stack_eval {
                    if context_pop.borrow().rv_count >= 0
                        && context_pop.get_mut().evaluation_stack().get_mut().len()
            ...
            // Stack ops
            OpCode::Depth => {
                self.push(self.current_context?.get_mut().evaluation_stack().borrow().len())
            }
            ...
            OpCode::Tuck => {
                self.current_context?.get_mut().evaluation_stack().get_mut().Insert(2, self.peek(0))
            }
            OpCode::Swap => {
            ...
            OpCode::InitSSLot => {
                ...
                self.current_context?.get_mut().set_fields() = Some(&Slot::new_with_count(
            ...
            OpCode::Le => {
                ...
                if x1.get_type() == StackItemType::Any || x2.get_item_type() == StackItemType::Any {
                    self.push(StackItemTrait::from(false).into())
                } else {
                    self.push(StackItemTrait::from(x1.get_integer() <= x2.get_integer()).into())
                }
            ...
            OpCode::PackMap => {
                ...
                if size < 0
                    || size * 2 > self.current_context?.get_mut().evaluation_stack().borrow().size()
                {
                    return Err(VMError::InvalidOpcode(
            ...
            OpCode::PackStruct => {
                ...
                if size < 0
                    || size > self.current_context?.get_mut().evaluation_stack().borrow().size()
                {
                    return Err(VMError::InvalidOpcode(
                        "The value {size} is out of range.".parse().unwrap(),
                    ));
            ...
            OpCode::Pack => {
                let size = self.pop().get_integer().to_usize().unwrap();
                if size < 0 || size > self.current_context.unwrap().evaluation_stack().len() {
                    return Err(VMError::InvalidOpcode(
                        "The value {size} is out of range.".parse().unwrap(),
                    ));
                }
                ...
                if instr.opcode == OpCode::NewArrayT {
                    ...
                    item = match _type as StackItemType {
                        ...
                        StackItemType::ByteString => {
                            StackItemTrait::from(ByteString::new(Vec::new()))
                        }
            ...
            OpCode::NewStruct0 => self.push(
                StackItemTrait::from(Struct::new(None, Some(self.reference_counter.clone())))
                    .into(),
            ),
            OpCode::NewStruct => {
            ...
            OpCode::Size => {
                ...
                match x {
                    StackItem::VMArray(array) => {
                        self.push(StackItemTrait::from(array.Count).into())
                    }
                    ...
                    StackItem::VMByteString(array) => {
                        self.push(StackItemTrait::from(array.Size).into())
                    }
                    StackItem::VMBuffer(buffer) => {
                        self.push(StackItemTrait::from(buffer.Size).into())
                    }
