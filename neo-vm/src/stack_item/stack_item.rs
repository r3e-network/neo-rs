...
impl StackItem {
    ...
    pub fn as_bytes(&self) -> VmResult<Vec<u8>> {
        match self {
            ...
            Self::Integer(i) => Ok(normalize_bigint_bytes(&i.to_bigint())),
    ...
    pub fn into_bytes(self) -> VmResult<Vec<u8>> {
        match self {
            Self::Null => Ok(Vec::new()),
            Self::Boolean(b) => Ok(vec![u8::from(b)]),
            Self::Integer(i) => Ok(normalize_bigint_bytes(&i.into_bigint())),
            ...
            match item {
                ...
                StackItem::Integer(i) => {
                    if i.is_zero() {
                        ...
                        Some(i.to_signed_bytes_le()) // VmInteger has this method
                    }
                }
