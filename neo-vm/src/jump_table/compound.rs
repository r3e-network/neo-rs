...
fn has_key(engine: &mut ExecutionEngine, _instruction: &Instruction) -> VmResult<()> {
    ...
    let result = match &collection {
        ...
        StackItem::Buffer(buffer) => {
            byte_sequence_has_key(neo_vm_rs::StackValue::Buffer(0, buffer.data()), &key)?
        }
        _ => {
            return Err(VmError::invalid_type_simple(
...
// ===================== Pre-HF_Gorgon (neo-vm#543) compound variants =====================
//
// The neo-vm#543 fix changed index validation for HASKEY/PICKITEM/SETITEM/REMOVE
// to bounds-check the full `BigInteger` index. The pre-543 handlers (restored for
// the NotGorgon / NotEchidna tables) instead cast the index to a 32-bit `int`
// first, so an index outside `i32` range faults UNCATCHABLY (C# `OverflowException`)
// rather than producing a catchable out-of-range fault. The collection mutation and
// reference-counting are otherwise identical (encapsulated in the collection types).
...
pub(crate) fn pick_item_before543(
    ...
    let result = match collection {
        ...
        item @ (StackItem::Integer(_) | StackItem::Boolean(_)) => {
            // C# treats Integer/Boolean as PrimitiveType GetSpan(); the i32 index
            // overflow faults uncatchably, then the byte read bounds-checks.
            let index = before543_index(&key)?;
            if index < 0 {
                return Err(VmError::catchable_exception_msg(format!(
