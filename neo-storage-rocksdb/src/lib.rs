...
//!
//! Implements neo-storage's `Store`/`ReadOnlyStore`/`WriteStore`/`StoreProvider`
...
//! (the same seam where an alternate `neo-storage-mdbx` backend can plug in).
...
pub use write_batch_buffer::{
    AutoFlushBatchBuffer, WriteBatchBuffer, WriteBatchConfig, WriteBatchStats,
