...
use neo_core::{rpc::RpcException, ProtocolSettings, Signer, Transaction};
...
fn parse_plugins(result: &JToken) -> Result<Vec<RpcPlugin>, RpcException> {
    let array = result
        .as_array()
        .ok_or_else(|| RpcException::new(-32603, "getplugins returned non-array"))?;
    ...
    array
        ...
        .map(|item| {
            let token = item
                .as_ref()
                .ok_or_else(|| RpcException::new(-32603, "plugin entry was null"))?;
            let obj = token
                .as_object()
                .ok_or_else(|| RpcException::new(-32603, "plugin entry was not an object"))?;
