...
use neo_core::{Block, BlockHeader, ProtocolSettings, Signer, Transaction};
use neo_json::{JArray, JObject, JToken};

use crate::utility::attributes::attribute_from_json;
use crate::utility::parsing::{oracle_response_code_to_str, parse_i64_token, parse_u32_token};
use crate::utility::witness::{
...
pub fn block_to_json(block: &Block, protocol_settings: &ProtocolSettings) -> JObject {
    ...
    json.insert(
        ...
        JToken::String(header.next_consensus.to_address()),
    );
    json.insert(
        "witnesses".to_string(),
        JToken::Array(
...
pub fn transaction_to_json(tx: &Transaction, protocol_settings: &ProtocolSettings) -> JObject {
    ...
    json.insert(
        "sender".to_string(),
        tx.sender()
            .map(|sender| JToken::String(sender.to_address()))
...
pub fn transaction_from_json(
    ...
    if let Some(signers_token) = json.get("signers").and_then(|t| t.as_array()) {
        ...
        for entry in signers_token.children() {
            ...
            let signer_json = crate::utility::parsing::jtoken_to_serde(signer_token)?;
            parsed_signers.push(
