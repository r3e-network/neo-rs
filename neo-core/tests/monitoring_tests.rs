...
async fn test_performance_thresholds_and_alerts() {
    ...
    system
        ...
        .register_alert_callback(move |alert| {
            if matches!(
                alert.level,
                neo_core::monitoring::performance::AlertLevel::Critical
            ) {
...
async fn test_profiler() {
    ...
    let profiler =
        Profiler::start_with_monitor("test_operation", system.performance_monitor.clone());
...
async fn test_exporter_factory() {
    let system = init_monitoring("2.0.0".to_string()).await.unwrap();
    ...
    let otlp_exporter =
        ExporterFactory::create_otlp("http://localhost:4317".to_string(), "neo-node".to_string());
    assert_eq!(otlp_exporter.content_type(), "application/json");

    let payload = system.export("otlp-json").await.unwrap();
    let parsed: Value = serde_json::from_str(&payload).expect("valid OTLP json");
    let metrics = parsed["resourceMetrics"][0]["scopeMetrics"][0]["metrics"]
        .as_array()
        .expect("metrics array");
    assert!(!metrics.is_empty(), "OTLP payload should contain metrics");
}

#[tokio::test]
async fn test_push_otlp_sends_payload() {
    ...
    let system = init_monitoring("3.0.0".to_string()).await.unwrap();
                    ...
                    Ok(n) => {
                        ...
                        if let Some(header_end) = find_header_end(&buffer) {
                            if let Some(content_length) =
                                parse_content_length(&buffer[..header_end])
                            {
                                let total_len = header_end + content_length;
                                if buffer.len() < total_len {
                                    continue;
                                }
                                let body = String::from_utf8_lossy(&buffer[header_end..total_len])
                                    .to_string();
                                let mut guard = received_clone.lock().await;
                                *guard = Some(body);
                                let _ = socket
    ...
    system
        .push_otlp(&endpoint, "neo-test-service")
        .await
        .unwrap();

    let payload = received.lock().await.clone().expect("payload captured");
    assert!(
        payload.contains("neo.health.status"),
        "OTLP body should include health metric"
    );
}
