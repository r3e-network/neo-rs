...
impl Instruction {
    ...
    fn compute_size(opcode: OpCode, operand_len: usize) -> u16 {
        let total = 1 + opcode.operand_prefix() + operand_len;
        total as u16
    }
    ...
    pub fn parse(script: &[u8], position: usize) -> VmResult<Self> {
        ...
        let operand = if matches!(
            ...
            let operand_end = operand_start + operand_size;
    ...
    pub fn parse_from_neo_io_reader(
        reader: &mut crate::neo_vm::io::MemoryReader,
    ) -> VmResult<Self> {
        ...
        let operand = if matches!(
            ...
            let operand_size = opcode.operand_size();
            if operand_size > 0 {
                SmallVec::from_vec(reader.read_bytes(operand_size)?)
            } else {
                SmallVec::new()
            }
        };
    ...
    pub fn parse_from_reader(reader: &mut crate::neo_vm::io::MemoryReader) -> VmResult<Self> {
        let pointer = reader.position();
...
mod tests {
    ...
    fn test_instruction_parsing_from_reader() {
        ...
        let mut reader = crate::neo_vm::io::MemoryReader::new(&script);
    ...
    fn test_operand_size() {
        ...
        assert_eq!(OpCode::PUSHNULL.operand_size(), 0);

        // PUSH instructions with fixed operand sizes
        assert_eq!(OpCode::PUSHINT8.operand_size(), 1);
        assert_eq!(OpCode::PUSHINT16.operand_size(), 2);
        assert_eq!(OpCode::PUSHINT32.operand_size(), 4);
        assert_eq!(OpCode::PUSHINT64.operand_size(), 8);
        assert_eq!(OpCode::PUSHINT128.operand_size(), 16);
        assert_eq!(OpCode::PUSHINT256.operand_size(), 32);
        assert_eq!(OpCode::PUSHA.operand_size(), 4);

        assert_eq!(OpCode::PUSHDATA1.operand_size(), 1);
        assert_eq!(OpCode::PUSHDATA2.operand_size(), 2);
        assert_eq!(OpCode::PUSHDATA4.operand_size(), 4);

        // Jump instructions with 1-byte offset
        assert_eq!(OpCode::JMP.operand_size(), 1);
        assert_eq!(OpCode::JMPIF.operand_size(), 1);
        assert_eq!(OpCode::JMPIFNOT.operand_size(), 1);
        assert_eq!(OpCode::CALL.operand_size(), 1);

        // Jump instructions with 4-byte offset
        assert_eq!(OpCode::JMP_L.operand_size(), 4);
        assert_eq!(OpCode::JMPIF_L.operand_size(), 4);
        assert_eq!(OpCode::JMPIFNOT_L.operand_size(), 4);
        assert_eq!(OpCode::CALL_L.operand_size(), 4);
