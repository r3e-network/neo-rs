...
fn init_slot(engine: &mut ExecutionEngine, instruction: &Instruction) -> VmResult<()> {
    ...
    if local_count > 0 {
        ...
        ctx.set_local_variables(Some(crate::neo_vm::slot::Slot::new(local_count, rc)));
    }
    ...
    if argument_count > 0 {
        ...
        let rc = ctx.evaluation_stack().reference_counter().clone();
        ctx.set_arguments(Some(crate::neo_vm::slot::Slot::with_items(arg_items, rc)));
    }
...
fn init_static_slot(engine: &mut ExecutionEngine, instruction: &Instruction) -> VmResult<()> {
    ...
    if static_count > 0 {
        let rc = ctx.evaluation_stack().reference_counter().clone();
        ctx.set_static_fields(Some(crate::neo_vm::slot::Slot::new(static_count, rc)));
    }
