...
impl NefFile {
    ...
    pub fn to_json(&self) -> JObject {
        JObject::new()
            .set("magic", Self::MAGIC)
            .set("compiler", self.compiler.clone())
            .set("source", self.source.clone())
            .set("tokens", self.tokens.iter().map(|t| t.to_json()).collect::<Vec<_>>())
            .set("script", base64::encode(&self.script))
            .set("checksum", self.checksum)
    }
}
