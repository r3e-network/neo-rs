    ...
    ([$($callback:tt)+]; $($args:tt)*) => {
        $($callback)+! {
            ...
            ;
            {
                ...
                safe "verifyWithECDsa", fee = 1 << 15, flags = [], params = [ByteArray, ByteArray, ByteArray, Integer], returns = Boolean, active = HfCockatrice, deprecated = HfGorgon, names = ["message", "pubkey", "signature", "curveHash"] => engine verify_with_ecdsa;
                safe "verifyWithECDsa", fee = 1 << 15, flags = [], params = [ByteArray, ByteArray, ByteArray, Integer], returns = Boolean, active = HfGorgon, names = ["message", "pubkey", "signature", "curveHash"] => engine verify_with_ecdsa;
                safe "verifyWithEd25519", fee = 1 << 15, flags = [], params = [ByteArray, ByteArray, ByteArray], returns = Boolean, active = HfEchidna, deprecated = HfGorgon, names = ["message", "pubkey", "signature"] => engine verify_with_ed25519;
                safe "verifyWithEd25519", fee = 1 << 15, flags = [], params = [ByteArray, ByteArray, ByteArray], returns = Boolean, active = HfGorgon, names = ["message", "pubkey", "signature"] => engine verify_with_ed25519;
