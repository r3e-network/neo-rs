...
impl SafeArithmetic {
    ...
    pub fn validate_total_supply_change(
        current: &BigInt,
        delta: &BigInt,
    ) -> CoreResult<BigInt> {
...
impl PermissionValidator {
    ...
    pub fn validate_account_hash(data: &[u8]) -> CoreResult<()> {
        if data.len() != 20 {
            return Err(CoreError::native_contract(
                format!("Account hash must be 20 bytes, got {}", data.len())
            ));
        }
        Ok(())
    }
    ...
    pub fn validate_public_key(data: &[u8]) -> CoreResult<()> {
        if data.len() != 33 {
            return Err(CoreError::native_contract(
                format!("Public key must be 33 bytes, got {}", data.len())
            ));
        }
...
impl StateValidator {
    ...
    pub fn validate_total_supply(
        ...
        if let Some(max) = max_supply {
            if total_supply > max {
                return Err(CoreError::native_contract(
                    format!("Total supply {} exceeds maximum {}", total_supply, max)
                ));
            }
        }
        Ok(())
...
mod tests {
    ...
    fn test_state_validator() {
        ...
        assert!(StateValidator::validate_account_state(
            &BigInt::from(100),
            100,
            200
        ).is_ok());
        ...
        assert!(StateValidator::validate_total_supply(&BigInt::from(100), Some(&BigInt::from(1000))).is_ok());
        assert!(StateValidator::validate_total_supply(&BigInt::from(-100), None).is_err());
        assert!(StateValidator::validate_total_supply(&BigInt::from(1000), Some(&BigInt::from(100))).is_err());
    }
}
