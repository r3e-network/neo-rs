...
impl NativeContract for NeoToken {
    ...
    fn post_persist(&self, engine: &mut ApplicationEngine) -> CoreResult<()> {
        ...
        if Self::should_refresh_committee(block.index(), committee_count) {
            let m = i64::try_from(committee_count)
                .map_err(|_| CoreError::native_contract("Committee count exceeds i64 capacity"))?;
            let n = i64::try_from(validators_count)
                .map_err(|_| CoreError::native_contract("Validators count exceeds i64 capacity"))?;
