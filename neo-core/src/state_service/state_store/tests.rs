...
impl StateStoreBackend for FailingStateStoreBackend {
    ...
    fn commit(&self) -> Result<(), String> {
        Err("injected state store commit failure".to_string())
    }
}
...
impl WriteStore<Vec<u8>, Vec<u8>> for FailingSnapshotStore {
    fn delete(&mut self, _key: Vec<u8>) -> Result<(), neo_storage::StorageError> {
        Ok(())
    }

    fn put(&mut self, _key: Vec<u8>, _value: Vec<u8>) -> Result<(), neo_storage::StorageError> {
        Ok(())
    }
}
...
impl StoreSnapshot for FailingCoreSnapshot {
    ...
    fn try_commit(&mut self) -> Result<(), StorageError> {
        Err(StorageError::CommitFailed(
            "injected snapshot commit failure".to_string(),
        ))
    }
}
...
fn ledger_storage_is_excluded_from_state_trie() {
    ...
    store
        .update_local_state_root_snapshot(height, changes.into_iter())
...
#[test]
fn state_snapshot_commit_propagates_backend_commit_failure() {
    let backend = Arc::new(FailingStateStoreBackend);
    let settings = StateServiceSettings::default();
    let mut snapshot = StateSnapshot::new(backend, settings);

    let err = snapshot.commit().expect_err("commit should fail");

    assert!(err.contains("injected state store commit failure"));
}

#[test]
fn state_store_transaction_commit_propagates_backend_commit_failure() {
    let backend = Arc::new(FailingStateStoreBackend);
    let mut tx = StateStoreTransaction::new(backend);
    tx.put(b"key".to_vec(), b"value".to_vec());

    let err = tx.commit().expect_err("transaction commit should fail");

    assert!(err.contains("injected state store commit failure"));
}

#[test]
fn update_local_state_root_returns_commit_error() {
    let store = StateStore::new(
...
fn snapshot_backed_backend_commit_propagates_snapshot_try_commit_failure() {
    ...
    let err = backend.commit().expect_err("snapshot commit should fail");

    assert!(err.contains("injected snapshot commit failure"));
}
...
fn snapshot_backed_backend_keeps_pending_on_commit_failure() {
    ...
    let _ = backend.commit().expect_err("snapshot commit should fail");
