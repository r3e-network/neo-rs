...
use crate::constants::{MAX_STORAGE_KEY_SIZE, MAX_STORAGE_VALUE_SIZE};
...
impl Node {
    ...
    pub fn hash(&self) -> UInt256 {
        self.try_hash().expect("failed to compute node hash")
    }
    ...
    fn extension_size(&self) -> usize {
        if self.next.is_none() {
            panic!("extension node missing child during size computation");
        }
