...
pub enum UnhandledExceptionPolicy {
    /// Ignore the exception (continue running).
    Ignore,
    /// Stop only the offending plugin.
    StopPlugin,
    /// Halt the entire node.
    StopNode,
    /// Continue execution but log the failure.
    Continue,
    /// Terminate the RPC server or process immediately.
    Terminate,
}
...
macro_rules! register_plugin {
    ($plugin_type:ty) => {
        inventory::submit! {
            $crate::extensions::plugin::PluginRegistration(|| {
