...
impl<'a> MemoryReader<'a> {
    ...
    #[inline(always)]
    fn ensure_position(&self, move_by: usize) -> Result<(), IOError> {
        if self.pos + move_by > self.memory.len() {
            Err(IOError::UnexpectedEof(format!("Unexpected end of data at position {}", self.pos)))
        } else {
            Ok(())
        }
    }
    ...
    #[inline(always)]
    pub fn peek(&self) -> Result<u8, IOError> {
    ...
    pub fn read_bool(&mut self) -> Result<bool, IOError> {
        match self.read_u8()? {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(IOError::InvalidData("Invalid boolean value".to_string())),
        }
    }
    ...
    #[inline(always)]
    pub fn read_i16_big_endian(&mut self) -> Result<i16, IOError> {
        self.ensure_position(size_of::<i16>())?;
    ...
    #[inline(always)]
    pub fn read_u16_big_endian(&mut self) -> Result<u16, IOError> {
        self.ensure_position(size_of::<u16>())?;
    ...
    #[inline(always)]
    pub fn read_i32_big_endian(&mut self) -> Result<i32, IOError> {
        self.ensure_position(size_of::<i32>())?;
    ...
    #[inline(always)]
    pub fn read_u32_big_endian(&mut self) -> Result<u32, IOError> {
        self.ensure_position(size_of::<u32>())?;
    ...
    #[inline(always)]
    pub fn read_i64_big_endian(&mut self) -> Result<i64, IOError> {
        self.ensure_position(size_of::<i64>())?;
    ...
    #[inline(always)]
    pub fn read_u64_big_endian(&mut self) -> Result<u64, IOError> {
        self.ensure_position(size_of::<u64>())?;
    ...
    pub fn read_var_int(&mut self, max: u64) -> Result<u64, IOError> {
        ...
        if value > max {
            return Err(IOError::InvalidData(format!("VarInt exceeds maximum value: {}", value)));
        }
        Ok(value)
    }

    #[inline(always)]
    pub fn read_fixed_string(&mut self, length: usize) -> Result<String, IOError> {
        ...
        for j in i..end {
            if self.memory[j] != 0 {
                return Err(IOError::InvalidData(format!("Invalid fixed string format at position {}", j)));
            }
        }
        self.pos = end;
        String::from_utf8(data.to_vec()).map_err(|_| IOError::InvalidData(format!("Invalid UTF-8 sequence at position {}", i)))
    }

    #[inline(always)]
    pub fn read_var_string(&mut self, max: usize) -> Result<String, IOError> {
        let length = self.read_var_int(max as u64)? as usize;
        self.ensure_position(length)?;
        let data = &self.memory[self.pos..self.pos + length];
        self.pos += length;
        String::from_utf8(data.to_vec()).map_err(|_| IOError::InvalidData(format!("Invalid UTF-8 sequence at position {}", self.pos)))
    }

    #[inline(always)]
    pub fn read_memory(&mut self, count: usize) -> Result<&'a [u8], IOError> {
    ...
    #[inline(always)]
    pub fn read_var_memory(&mut self, max: usize) -> Result<&'a [u8], IOError> {
        let length = self.read_var_int(max as u64)? as usize;
